...
A Faucet is generally defined in the blockchain world as an vehicle for distributing small amounts of tokens. Lisk has a faucet plugin for the Lisk Framework that provides a user interface to receive an certain amount of tokens on request.
In addition, the Lisk faucet plugin has the ability to send tokens to different accounts from a faucet.
...
The validator is essentially a validation library according to the Lisk Protocol. The @liskhq/lisk-validator package is a wrapper of validation packages, that includes custom validation utilities related to the Lisk Protocol.
