...
The optional function `validate()` validates the data of a xref:{url_transaction_asset}[transaction asset], before it is passed to the `apply()` function.
...
The most important part of the xref:{url_module_asset}[module asset] is the `apply()` function.
