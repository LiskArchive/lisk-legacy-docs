...
* [x] Enable block generation on a node (for DPoS blockchains)
* [x] Register a delegate
* [x] Create the delegate data and add it to the node configuration
====

== Registering a delegate

Before it is possible to enable block generation on a node, it is required to register as a delegate.
This is achieved by choosing a unique delegate name, and posting a corresponding "Delegate Registration" transaction to the network.

There are many possibilities to post the transaction, for example: {url_lisk_wallet}[Lisk Desktop^], xref:{url_elements}[Lisk Elements], the node CLI or the Dashboard plugin.
// TODO: Add link to dashboard plugin reference, once it is updated for v6
// There are many possibilities to post the transaction, for example: {url_lisk_wallet}[Lisk Desktop^], xref:{url_elements}[Lisk Elements], or the xref:{url_framework_dashbobardplugin}[].

In the following examples, the CLI is used to communicate with the node.
...
For this, you only need the account passphrase, and a unique delegate name.
...
[source,bash]
----
 % ./bin/run transaction:create dpos registerDelegate 1100000000 \
...
== Add delegate data to the node config

IMPORTANT: This step only needs to be performed when a delegate is enabling block generation on a node for the first time.

To enable your node to forge for a particular delegate, firstly it is required to insert the following data into the node config file:
...
It is possible to generate all relevant config values to enable block generation from the account passphrase.

To do so, use the command xref:{url_references_cli_forgingconfig}[keys create].
...
It will be required everytime you wish to enable block generation, in order to decrypt the delegates' passphrase in the config.
...
. Use as passphrase the passphrase of the delegate account.
...
? Please enter passphrase:  [hidden]
? Please re-enter passphrase:  [hidden]
Warning: Passphrase contains 1 words instead of expected 12. Passphrase contains 0 whitespaces instead of expected 11.
...
This will create the following keys associated with the account:
...
Add the generated keys into the node configuration file under `generator.keys`:
...
== Checking the block generation status

To check the block generation status of a Lisk Core node, execute the following command:
...
The command returns a list of delegates, based on the list under `generator.keys` in the node config, which was added to the config in the previous step, <<add-the-generator-keys>>).
