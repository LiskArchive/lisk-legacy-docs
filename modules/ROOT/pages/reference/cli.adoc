...
VERSION
  lisk-core/3.0.0-rc.3 darwin-x64 node-v12.22.3
...
The configuration for Lisk Core is stored in `~/.lisk/lisk-core/config` footnoteref:[snap_footnote,Snap versions of Lisk Core store their everything in `~/snap/lisk-core/current/.lisk/lisk-core` instead of `~/.lisk/lisk-core`].
