...
Using dedicated commands that are part of the Interoperability module, users can recover their assets from a terminated sidechain directly on the native chain.
...
==== Recover Message
The {url_typedoc_interop_message_recovery}[Recover Message^] command is used to recover pending messages from the outbox of a terminated sidechain.
