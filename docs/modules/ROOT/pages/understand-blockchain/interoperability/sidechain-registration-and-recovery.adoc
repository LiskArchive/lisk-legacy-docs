...
This command creates the <<chain-account,sidechain account>> on the mainchain with the initial `status` set to `registered`.
...
== Interoperability data stores
=== Chain account

The properties of sidechain- and mainchain accounts and their default values.
...
** a <<chain-account,sidechain account>>,
** a <<channel-data,sidechain channel>>,
** and the <<validator-data,validators>> for the sidechain.
* Each sidechain maintains:
** a <<chain-account,mainchain account>>,
** a <<channel-data,mainchain channel>>,
** and the <<validator-data,validators>> for the mainchain.
...
This xref:{url_build_regsidechain_sccommand}[Sidechain Registration command] assigns a unique bytes value as the chain ID that identifies the sidechain in the ecosystem.
