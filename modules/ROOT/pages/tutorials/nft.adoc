...
You will learn how to create a custom module which adds the feature to create and purchase NFTs to a blockchain application, and a custom plugin which provides additional NFT-related features.
...
The module name should be unique within the network as well.
Otherwise it will lead to confusion, for example when subscribing to events or invoking actions of that module.
...
You can get more information about how to build a React application for example at the {url_react_docs}[official React documentation^].
...
Here you see a list of all transactions, which have been posted in the network so far, including a few details such as their respective module and asset name, the sender address and the transaction ID.
...
[TIP]
====
As you may be aware, there are two different representations of the account address in the Lisk protocol: the **binary address** and the **base32 address**.

For *backend*, it is always recommended to work with the **binary address**, as this is how the Lisk SDK handles addresses internally.
