...
:url_delegate_misbehavior: pages/consensus.adoc#how_to_discover_misbehavior_of_a_delegate
:url_protocol_transactions_pom: protocol:transactions.adoc#pom
:url_dpos: pages/consensus.adoc#delegated_proof_of_stake_(dpos)
:url_delegate_registration: pages/consensus.adoc#registering_as_delegate
:url_genesis_block: pages/architecture/index.adoc#genesis_block
...
A server having Lisk Core installed and therefore being part of a Lisk network, e.g. Lisk’s mainnet. It can be described as a resource for interacting with the node endpoint provided by the Lisk public API. A node can be setup with Lisk core using the following 4 distribution methods: Application, Commander application, Docker image and Source code.
