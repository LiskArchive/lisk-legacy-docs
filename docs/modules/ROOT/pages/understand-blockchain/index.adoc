...
Each node runs on an instance of the blockchain client and also stores node-specific on-chain and off-chain data.
...
**If the nodes of a blockchain follow the Lisk protocol, they are called "blockchain applications"**.
...
More specifically, within the Lisk documentation, whenever blockchain applications are discussed, this is directly referring to applications running on blockchains that are compatible with Lisk Protocol. 
...
Application domain is where logic written by the developers live.
