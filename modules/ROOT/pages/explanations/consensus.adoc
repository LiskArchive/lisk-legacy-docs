...
In a DPoS system, each account that has an adequate enough balance to send a xref:{url_protocol_transactions_delegate}[register delegate transaction] can <<delegate_registration,register a new delegate>> on the network.
...
During a forging round, delegates forge blocks one after another in a random order, which was defined in the beginning of each forgig round.
...
Furthermore delegates can be punished, if they forge a block with consensus votes that contradict with the Lisk BFT (see the xref:{url_protocol_punishment}[Punishment of Lisk-BFT protocol violations] section of the Lisk Protocol).
