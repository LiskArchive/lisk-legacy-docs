...
<9> Initial Reward.
<10> Milestone 1. <11> Milestone 2. <12> Milestone 3. <13> Milestone 4. <14> Start rewards at block (n).
...
Min: 0 (default), Max: 15. <35> If true, enables cache.
Default: false.
<36> Redis host IP. Default: 127.0.0.1 <37> Redis host port.
Default: 6380 <38> Contains configurations related to modules.
...
<63> Define who can access the forging related API end points of the node.
...
Default: 1000 <73> Contains forging options for delegates.
...
<95> The maximum number of outbound peers/connections which the node should have.This should be an integer.
The node will keep trying to connect to new peers until it reaches the maximum outbound connections.
<96> The number of milliseconds to ban a peer for, in the case whereby its reputation score falls bellow 0. <97> How frequently, (in milliseconds) the outbound list is checked and repopulated, (as peers can drop out over a period of time).
...
<102> The maximum size in bytes, (integer) of any kind of message from a peer.
