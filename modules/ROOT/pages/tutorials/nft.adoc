...
Next, we will define all required properties for the plugin one after another.

=== Database related functions
For a better overview, we will first create another file which will contain the functions related to the plugin database.
...
* `getDBInstance(dataPath,dbName)`: Create a new key-value store `nft_plugin.db` for the NFT plugin.
