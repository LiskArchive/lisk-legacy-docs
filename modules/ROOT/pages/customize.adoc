...
The transaction that you’re currently processing is the function’s context, (i.e. `this.amount`).
Invalidate the transaction by pushing an error into the result array.
...
As your application matures you can provide the custom ways of how your transaction’s signature is derived: `sign`, `getBytes`, `assetToBytes`.

== Default transaction types

IMPORTANT: The first 10 transaction types are reserved for the https://lisk.io/documentation/lisk-protocol[Lisk protocol]. Do not use them to register custom transactions.
...
This can be done by a third party tool (like `curl` or `Postman`), but also using Lisk Commander, Lisk Desktop or Mobile.
...
When this happens, the block containing all of the included transactions get reverted in favour of a competing block.
