...
The API checks whether the specified `blsKey` is registered on the chain or not and whether the `proofOfPossession` for the BLS key is valid.
The API will return `false` if the `blsKey` is already registered.
...
If there is no BLS key in the store it would return as `true`. If the input passed to the API is invalid, or the `blsKey` already exist, the response will be `false`.
====
