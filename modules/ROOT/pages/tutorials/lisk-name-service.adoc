...
Click on the button kbs:[Generate new account] in the top right corner of the page to create a new account:
...
This should return the corresponding LNS object, which was just created by registering the domain name in the steep before:
...
This is important, because a single account can register many different domain names at onnce.
...
Finally, let's check if the reveerse lookup of the account address is working as expected.
In the section `Call action`, select `lns:lookupAddress` from the dropdown menu and provide the address in the inpuut field, again, the address needs to be in hexadecimal representation.
