...
:url_admin_binary_rebuild_from_genesis: setup/binary.adoc#rebuild_from_genesis
:url_configuration_forging: configuration.adoc#forging
...
[IMPORTANT]
====
*Important note for delegates:* After an automated upgrade is performed, it will still be necessary to manually enable forging again, as described in the xref:{url_configuration_forging}[configuration section][[Forging]].
====
...
Counting from the migration height, there is 2 full forging rounds time to upgrade your node manually by following the steps described in <<migrate_manually, Migrate manually>>.
If 2 full forging rounds have already passed since migration, your Node will probably be on a fork after the upgrade.
In order to resolve this, rebuild your version of the blockchain xref:{url_snapshots}[from snapshot][[Snapshot]] or xref:{url_admin_binary_rebuild_from_genesis}[from the genesis block].
