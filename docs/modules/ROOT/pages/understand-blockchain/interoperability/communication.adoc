...
In fact, a cross-chain transaction uses the identical schema as normal transactions, but it refers to a cross-chain command instead of a regular command.
Unlike regular commands, cross-chain commands can create CCMs.
...
If the validation is completed successfully, the cross-chain command is performing the intended state changes on the receiving chain, i.e. adding 5 LSK to Bobs account.
--
:!sectnums:
