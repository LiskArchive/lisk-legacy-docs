...
On the other hand, if the percentage of rewards shared equally among active validators is set to 100%, it will be the previous proof-of-stake (PoS) system where each validator received the same reward for generating a block irrespective of their weights.
+
...
If a validator misses a block then a validator generating the block second time will get the reward proportional to their weight.
