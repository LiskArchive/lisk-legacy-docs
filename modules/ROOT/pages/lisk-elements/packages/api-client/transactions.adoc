...
Submits a signed transaction JSON for processing by the transaction
pool.
...
`+transaction+`: The JSON representation of the transaction ; ie.
returned by `+Transaction.toJSON()+`;
