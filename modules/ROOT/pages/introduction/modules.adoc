...
The most important parts of a module are the parts which contain the logic to do state mutation on the blockchain.
...
To add support for a new transaction tot he application, it is required to implement a new <<assets,transaction asset>> and to add it to the module.
...
The `reducerHandler` allows modules and <<assets>> to invoke reducers of other modules.
