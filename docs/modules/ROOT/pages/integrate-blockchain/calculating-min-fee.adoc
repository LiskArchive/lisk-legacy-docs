...
One way to retrieve the xref:{url_understand_tx_fee}[minimum fee] for a transaction, is to dry-run the transaction with no fees.

=== Prepare the transaction

First, create the transaction for which the minimum required fee shall be calculated.

Use `0` as value for the fee.

[source,bash]
----
./bin/run transaction:create token transfer 0
