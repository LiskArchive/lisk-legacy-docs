...
In case you still want to create a new genesis block with the current release, check out the {url_github_corbifex}[Genesis block generator by Corbifex].
====
...
The user who will later deploy the node on a server can then add the credentials to the config, to enable forging for a particular delegate on this node.
====
...
The client node is talking via the API with the `client` app, and over the WebSocket connection to the seed node.
