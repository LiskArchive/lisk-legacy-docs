...
[source,bash]
----
    pm2 start npm --name "lisk-service" -- run start
----
...
The market service is required to be started separately:

Navigate into the directory `services/market` and install all dependencies for the market component separately:

[source,bash]
----
npm install
pm2 start npm --name "lisk-service-market" -- run start
----
