...
The main purpose of deploying sidechains in the Lisk ecosystem is to increase scalability. Therefore, the more sidechains that exist in the ecosystem mean the more transactions that can be processed per second.
It is intended that each blockchain serves one application, which means, for instance, there could be a sidechain dedicated to a decentralized exchange application.
...
The `name` property sets the name of the sidechain as a string of characters, and has to be unique within the Lisk ecosystem, and contains only characters from the set *a-z0-9!@$&_.*
