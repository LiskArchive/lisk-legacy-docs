...
The on-chain architecture of the `Application` consists two things. One is the `Node` and others are the `Modules`.
...
The `Node` is private abstraction layer of above mentioned activities so why not visible to the SDK user. User is only exposed with `Module` interface for defining any on-chain logic.
...
IMPORTANT: All of the logic implemented using a custom module / asset must be “deterministic” and executable within block time.
