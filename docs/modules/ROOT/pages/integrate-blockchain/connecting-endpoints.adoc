...
:url_subscribing_to_events: /subscribing-to-events.adoc
...
NOTE: To interact with the node APIs through IPC, WS, or HTTP, it is necessary to enable the corresponding APIs. This can be done by configuring them in the node settings, using command line flags, or setting up the environment when launching the blockchain application.
