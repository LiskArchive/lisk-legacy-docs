...
After every node on the network has validated the transaction, it is then verified by checking how many blocks now exist on top of the validated block. In other words the block of data containing the transaction (this is also known as block confirmations), will then be added to the distributed ledger. +
<4> Block creation +
