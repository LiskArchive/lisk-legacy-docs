...
. Stop the old node.
. Dump the data in the `forger_info` table of the db of your node by executing the following command below:
+
[source,bash]
----
...
. Restore the `forger_info` table by executing the following command below:
+
[source,bash]
----
...
The height of your node should be equal to the current network height. This can be achieved by executing the command below:
+
[source,bash]
----
lisk-core node:info
