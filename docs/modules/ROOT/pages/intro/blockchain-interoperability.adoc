...
The Lisk interoperability solution has been designed with the following objectives in mind, scalability, security, flexibility, and efficiency:

 - Scalability

* There are no limits on the amount of sidechains that can be connected to the mainchain.
...
In essence, it is designed to facilitate communication via the concept of xref:{url_cross_chain_cert}[cross-chain] certification.
...
The future of blockchain interoperability is continuing to evolve, and the technological advances already have made a revolutionary impact on unlocking the full potential of blockchain.
...
The key to the growth of the {url_web3}[Web3^] ecosystem lies in blockchain interoperability.
Interoperability enables different blockchain networks to share data transactions, unlocking a plethora of possibilities.
The significance of blockchain interoperability becomes evident in the creation of innovative applications accessible through conventional user interfaces, accelerating the adoption of {url_web3}[Web3^].

The outlook for blockchain interoperability appears highly promising.
