...
:description: How to publish a blockchain application,including the components, seed nodes, creating a new genesis block & config objects, and adding additional nodes.
:sectnums:
:toc:
...
:url_github_hello_index: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/hello_world/index.js
:url_github_hello_client: https://github.com/LiskHQ/lisk-sdk-examples/tree/v4/archive/3.x/hello_world/react-client
:url_github_hello_package: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/hello_world/package.json
:url_github_hello_transaction: https://github.com/LiskHQ/lisk-sdk-examples/tree/v4/archive/3.x/hello_world/transactions
...
:url_protocol_genesis_block: ROOT::understand-blockchain/lisk-protocol/blocks.adoc#_genesis_block
...
As shown in the diagram above, the plan is to connect a second node with a public API which has forging disabled.
...
TIP: When using this architecture it is convenient to test if the WebSocket connection between the seed node and the new node is established.
...
Therefore, the seed node is the only node at this point that can forge new blocks, because all of the genesis delegates are actively forging on it.

TIP: How to replace the genesis delegates with real delegates is covered in the next section <<more_nodes, Add more nodes>>.

To set up the node and install the node application on a new server, just follow the instructions of the `README` file, that was created in the previous step.
...
[NOTE]
.Broadcast irregularities are no cause for concern.
====
...
At this point the node realizes it is not in synchronization with the other nodes and starts the synchronization process.
This can also be seen in the above logs, `Starting sync`.
During the synchronization process the missing blocks are received from the peers and added to the database of the node.
====
