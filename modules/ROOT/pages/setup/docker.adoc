...
This document covers how to prepare a system to run Lisk Core as a Docker-based container.
...
The `lisk` user itself does not need any `sudu` rights to run the Lisk Core.
...
To fully verify that your node is synchronised with the network, go to the {url_lisk_explorer_betanet}[Lisk Explorer(Betanet)] and compare the network height in the explorer with the height of your node.
