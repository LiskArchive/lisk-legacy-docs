...
=== Usage
Each namespace has its own set of endpoints. To invoke an endpoint, combine the name of an endpoint with its namespace like this: `namespace_endpoint`. For details, check out the cURL requests for endpoints discussed below.
...
Events exposed by Lisk RPC node can be subscribed as explained in the xref:{url_subscribe_event}[] section. To subscribe to an event, it is necessary that a WebSocket or Inter-Process Communication protocol should be used. 
