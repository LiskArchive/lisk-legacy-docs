...
After reaching the offset height, rewards for each validator are credited to their account on generating new blocks.
...
[source,bash]
----
lisk-core transaction:create pos changeCommission 134000 --json --pretty  
----
--
Sidechain Node::
+
...
[source,bash]
----
./bin/run transaction:create pos changeCommission 134000 --json --pretty 
----
--
=====
+
...
. Once the transaction is executed, the staker's account balance will increase as per the earned reward and the corresponding amount is reduced from the validator's locked balance.
