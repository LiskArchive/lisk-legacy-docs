...
:description: How to create & register custom transactions including the BaseTransaction interface & the Store.
:toc:
:v_sdk: v3.0.2
:v_protocol: master
...
:url_protocol_transactions: {v_protocol}@lisk-sdk:protocol:transactions.adoc
:url_lisk_protocol_transactions_pool: {v_protocol}@lisk-sdk:protocol:network.adoc#pool
...
The Lisk SDK provides a <<interface,BaseTransaction interface>> from which developers can extend from, in order  to create a custom transaction.
