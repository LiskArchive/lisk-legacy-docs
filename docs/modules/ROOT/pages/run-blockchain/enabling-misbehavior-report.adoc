...
// External URLs
// Project URLs
:url_plugin_report_misbehavior: lisk-sdk::plugins/report-misbehavior-plugin.adoc

// TODO: Update the page by uncommenting the hyperlinks once the updated pages are available. 


// xref:{url_plugin_report_misbehavior}["Report Misbehavior" plugin]
...
TIP: See more available configuration options at the "Report Misbehavior" plugin reference.
// xref:{url_plugin_report_misbehavior}[]
...
For example, use the `lisk-client` package and write a small script that invokes the `reportMisbehavior:authorize` action on the node.
