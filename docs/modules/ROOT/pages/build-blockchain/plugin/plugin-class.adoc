...
The Plugin class contains the starting, ongoing, and concluding logic of a plugin. In our case, we want HelloInfoPlugin to update the database with NewHelloEvents after regular intervals.
...
* [x] Storing NewHello event in the database.
====
...
But, how can we see that data, for that, let's create an endpoint and test our plugin in the next guide.