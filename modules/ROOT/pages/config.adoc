...
----
NOTE: The `DEFAULT_KEY_DERIVATION_PATH`` points to the path for deriving the private key of the n-th generator key pair, for the chain with `chainID` is `m/25519'/134'/chainID'/n'`. If `chainID` is not known, use the path `m/25519'/134'/0'/n'` for the n-th generator key derived for chains without known chain ID. For more information, see {url_key_derivation_lip66}[Introduce tree based key derivation and account recovery].
