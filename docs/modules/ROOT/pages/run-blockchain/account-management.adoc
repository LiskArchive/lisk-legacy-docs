...
* [x] Methods manage account credentials
====
...
[#generator-keys]
An extra key pair, the generator keys, is added to a validator account to sign blocks.
...
[#bls-keys]
An extra key pair, the BLS keys, is added to a validator account that is used for creating aggregated signatures.
BLS keys are used by validators, among other things, to sign commits in Lisk chains.
