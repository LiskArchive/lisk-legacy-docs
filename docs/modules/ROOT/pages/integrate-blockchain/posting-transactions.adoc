...
:url_integrate_createtx_hex: integrate-blockchain/create-send-transactions.adoc#hex-string
:url_integrate_createtx_json: integrate-blockchain/create-send-transactions.adoc#json-compatible-object
:url_sdk_cli_txsend: v6@lisk-sdk::client-cli.adoc#transactionsend

This guide gives an overview of the different ways how to send a transaction to a node.

TIP: If the existing data needs to be encoded/decoded before posting, check the guide xref:{url_integrate_decoding}[]

== Via the node CLI

If you are managing your own node, it is possible to send transaction directly via the node CLI with the xref:{url_sdk_cli_txsend}[transaction:send] command.
[source,bash]
----
...
It can be used inside of JS or TS script to connect to a blockchain node API.
...
== HTTP node API example with cURL

If a node has HTTP API enabled, it is possible to post HTTP requests to the node like shown in the below example.
