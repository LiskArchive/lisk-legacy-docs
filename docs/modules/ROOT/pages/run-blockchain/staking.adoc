...
====
On this page, you'll learn how to (un-)stake tokens for a validator on a blockchain that uses the Lisk PoS module.
...
Create a transaction to stake and un-stake tokens for one or multiple validators.

* Up to *20 stakes and/or un-stakes* can be included in a single Stake transaction.
* A *positive* `amount` of tokens indicates a *stake*, whereas a *negative* `amount` of tokens indicates an *un-stake*.
...
There are various different services and tools to stake tokens, for example: {url_lisk_wallet}[Lisk Desktop^], xref:{url_sdk_elements}[Lisk Elements], or the node CLI.
