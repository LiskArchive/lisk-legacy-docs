...
== Creating a Plugin
Lisk provides an extremely simple method to create a plugin using xref:{url_lisk_commander}[Lisk Commander]. 
...
The structure of the plugins folder can be similar to the one mentioned below:
...
For a plugin to work correctly, functions like `nodeModulePath()`, `load()`, and `unload()` are mandatory to have in a plugin. These functions are automatically created during the bootstrapping process. 

The following code snippet shows a bare-bone plugin class.
...
The `datapath()` function returns this path to the plugin so that data specific to the application can be retrieved.
...
For more information about default configuration, see xref:{url_references_config_default}[Configuration].
...
==== Schemas
A Schemas of a plugin contains the structural information of the data accepted by a plugin. 
...
Usually, schemas are present inside the *schemas.ts* file inside a plugin's folder.
...
Whilst schemas describe the schema of data, the types.ts file contains the definitions of all the variables that are used by a plugin.
...
A {url_github_faucet_endpoints}[sample endpoint class] and one of its endpoints are available in the following code snippet:
