...
:description: All log levels and associated messages which can be used for monitoring or debugging a node.
:toc:
...
All logs that have equal or higher log levels than in the log level specified in the config are saved in a `.log`-file for further analysis.
...
After customizing and saving the config to fit your requirements, it is recommended to test it by performing a dry run using the following command below:
