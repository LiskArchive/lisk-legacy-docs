...
If you do so, the installation will be aborted, as it is not necessary and not recommended to install Lisk Core for a user with `sudo` rights.
====
...
Just be aware that it is necessary to refer to your custom name instead of `lisk-mainnet` or `lisk-testnet` when running the commands.
====
...
It is also required to have the checksum for the TAR file with the format 2.0.0-Linux-x86_64.tar.gz.SHA256 for the Commander to check the integrity of the binary.
...
It is also recommended to set up a xref:{url_config_logrotation}[log rotation].
...
Assuming all of the above steps have been successfully completed, then you are ready to move on to the configuration documentation. If you wish to enable forging or SSL, please see the xref:{url_config}[General configuration].
