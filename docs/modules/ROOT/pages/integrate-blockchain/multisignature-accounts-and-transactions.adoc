...
As mentioned in the <<parameters, parameters>> section, we need three accounts.
Each account will be used to sign the registerMultiSignature transaction individually.
We will be using the first account among the three as the sender of the registerMultisignature transaction.
...
+
--
Let's prepare a script which takes the accounts data and uses it to create and later sign a `registerMultisignature` transaction.
...
----
+
. Add details such as sender of the transaction, all the optional keys and mandatory keys in sorted form.
+
[source,js]
----
...
----
+
. Retrieve `nonce` of the sender account from the xref:{url_getAuthAccount}[auth_getAuthAccount], and create an `unSignedData` object.
+
[source,js]
----
...
+
--
. Use each optional account's details to add signature for each account.
...
----
+
. Using the `Transaction` class, initialize a new object which takes all the details mentioned earlier as part of the registration transaction.
+
[source,js]
----
...
+
--
The transaction:sign command is used to sign a multi-sig transaction. 
...
+
--
To collect the signature for a multi-signature transaction, run the create-multiSig-transaction.js file.
