...
To quickly connect to a specific network with Lisk Core, specify the desired network with the `--network` or `-n` flag.
...
NOTE: If the node will connect to the Mainnet, change `testnet` to `mainnet` in the snippet above.
