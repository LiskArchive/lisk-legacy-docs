...
It is used as prefix in the xref:{url_advanced_communication_alias}[alias] of events and actions, and as key label to to add the properties of the <<account-schema>> to the user accounts footnoteref:[account_footnote,For more information about accounts, check the xref:{url_protocol_accounts}[\] page of the Lisk protocol.].
...
NOTE: The properties `token`, `sequence`, `keys`, `dpos` exist in the user account, because the blockchain application has already several modules xref:{url_intro_bapps_defaultmodules}[registered by default].
