...
====
The following tools maintained by Lisk communinty members can be used to create automatic backups of the <<forger_info-data>>.

* The {url_forger_sync}[forger-sync^] is a collection of bash scripts by delegate Gr33nDrag0n.
...
A plugin that can be registered to a Lisk blockchain application.
Automatically backups the `forging_info` data after each block that the respective delegate has forged.
