...
In case you created custom configurations inside the `lisk-core/` folder, make sure to copy or move them out of the `lisk-core/` folder.

* back up `custom-config.json`: backup the custom configuration for Lisk Core
* back up `pm2.conf.json`: backup the pm2 config file
