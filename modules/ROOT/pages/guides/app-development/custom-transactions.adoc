...
:url_guides_interact: guides/app-development/interact.adoc
...
Transaction fees are a specific amount of tokens that is added to every transaction which is sent to the network.
The specified fee is automatically deducted from the account of the sender, after the transaction is included in a block.
When a transaction is included in a block, the delegate who forged the block will receive all the transaction fees of all the transactions which are included in the forged block.

The fees for every transaction can be defined dynamically.
Transactions with higher fees will often be included quicker into the blockchain, as forging delegates favor to include transaction in their block, that give them a big amount of transaction fees.
This is often the case, if so many transactions are in the transaction pool, that it is not possible to include all of them in a single block.
...
In case that a transaction remains in the transaction pool for a long time because of a fee that was chosen too low, it is possible to send a new transaction with identical values, except the fee which should be increased of course.
...
=== Is is possible to have zero transaction fees?

Yes, it is still possible to develop custom transactions with no transaction fees by overriding the `MIN_FEE_PER_BYTE` property to `MIN_FEE_PER_BYTE=0`.
