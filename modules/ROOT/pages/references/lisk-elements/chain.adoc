...
The following command will install the Lisk-chain:
...
The `account` store holds state per account, the `chain` store holds blockchain wide state information, and the `consensus` store holds the internal state related to BFT.
...
* The `consensus` store contains the finalized height, BFT voting ledger, and a list of the validator's information.

=== Data access

The data access module provides simplified access to the data storage, (blockchain database).
