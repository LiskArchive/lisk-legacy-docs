...
To implement the LNS module, take a look again at the module skeleton, which was generated in step <<bootstrapping-the-default-application>>:
...
Before we go on with developing the frontend part, let's quickly check if the blockchain application is working as expected by enabling the Dashboard plugin in the next chapter.

== First start of the LNS application

Start the LNS blockchain application with the following command:
...
If the application start is successfull, you will see

== Connecting the Dashboard and Faucet plugins

The Dashboard plugin provides a web interface that allows developers to innteract with their blockchain application during development.
...
* ... send actions to the LNS application to ...
...
<1> Choose a simple password to encrypt the passphrasee symmetrically.
...
The xref:{url_recaptcha_keys}[free site key and secret key for reCAPTCHA^] are used below for testing purposes.
...
image:tutorials/lns/faucet-authorize-success.png[]

It is now possible to use the faucet under {url_faucet} .

image:tutorials/lns/faucet.png[]

=== Checking the functionality of the LNS module

Now let's use the dashboard under {url_dashboard} .

==== Create new account

image:tutorials/lns/new-account.png[]

==== Fund new account

image:tutorials/lns/faucet-send.png[]

image:tutorials/lns/faucet-fund-success.png[]

image:tutorials/lns/transactions1.png[]

==== Register new domain

image:tutorials/lns/send-tx-lns-register-dropdown.png[]

image:tutorials/lns/send-tx-lns-register.png[]

image:tutorials/lns/send-tx-lns-register-success.png[]

==== Test resolveName and resolveNode

image:tutorials/lns/call-action-lns-resolvename.png[]
image:tutorials/lns/call-action-lns-resolvename-success.png[]

---

image:tutorials/lns/call-action-lns-resolvenode.png[]

=== Define a reverse lookup address

image:tutorials/lns/send-tx-lns-reverselookup-dropdown.png[]
image:tutorials/lns/send-tx-lns-reverselookup.png[]
image:tutorials/lns/send-tx-lns-reverselookup-success.png[]
image:tutorials/lns/transactions2.png[]

==== Update records for a name

image:tutorials/lns/send-tx-lns-update-records.png[]
image:tutorials/lns/send-tx-lns-update-records-success.png[]
image:tutorials/lns/transactions3.png[]

==== Perform a reverse lookup for an address

image:tutorials/lns/call-action-lns-lookupaddress.png[]
image:tutorials/lns/call-action-lns-resolvenode-success.png[]
