...
:url_binary_pre_install: binary.adoc
:url_config_api: management/configuration.adoc#_api_access_control
:url_core_config: management/configuration.adoc
:url_core_releases: https://github.com/LiskHQ/lisk-core/releases
:url_docker_setup: docker.adoc
:url_environment_variables: docker.adoc#_list_of_available_environment_variables
// :url_environment_variables: docker.adoc#_set_environment_variables
...
+
--
We recommend using a Node version manager such as {url_nvm}[NVM].
...
To perform the command below successfully, install Docker like described in the Setup page of xref:{url_docker_setup][Lisk Core Docker distribution].
...
Don’t forget to update this password in the xref:{url_core_config}[Lisk Core configuration] later on.
...
If you are looking to upgrade your current Lisk Core installation, please see xref:{url_upgrade_source}[Upgrade from Source].

=== Login as the Lisk user

This user was created in the xref:{url_binary_pre_install}[binary-pre-install,Pre-Installation steps].
...
NOTE: Please check for latest release on {url_core_releases}[core releases].
...
Alternatively, the user may define a custom `config.json`, like described in xref:{url_core_config}[Configuration of Lisk Core] .
...
For details on how to manage or stop your Lisk node, please have a look in xref:{url_admin}[setting up your node].

If you are not running Lisk locally, you will need to follow the xref:{url_config_api}[Configuration - API][[_api_access_control]] document to enable access.

That’s it!
You are ready to move on to the xref:{url_core_config}[configuration] documentation in case you wish to configure your node further, e.g. if you wish to enable forging.
