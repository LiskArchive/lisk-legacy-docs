...
* If the `rescuer` is not the account that has initiated a recovery for the `lostAccount` then throw an error.
* If the account that vouches for the rescuer is not listed under `friends` in the recovery config of the `lostAccount`, then throw an error.
* If the friend has vouched before, throw an error.
...
* The asset contains the property `lostAccount`, which is the address of the account that is intended to be recovered.
* If not enough blocks have passed since initialization of the recovery, then throw an error.
...
* If not enough friends have vouched for the rescuer account, throw an error.
...
It will contain the logic for the transaction asset which allows to close an active recovery, for example, in case when the lost credentials were found.
...
* {url_github_srs_component_sidemenu}[SideMenu()^]: Renders the side navigation which allows to switch between the different pages of the frontend application.
...
That's it!
You should now have a complete blockchain application running which allows users to recover their accounts in case they lost their credentials.
...
The original account will get warned in case someone malicious starts recovery on their account to steal their funds, and if it wasn't themselves who initiated the recovery, then they can quickly close the recovery process before the delay period is passed and the set threshold number of friends already vouched for it.
