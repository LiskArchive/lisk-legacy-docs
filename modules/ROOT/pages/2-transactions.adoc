...
For example, in Lisk Mainnet, for a link:2-transactions.adoc#balance-transfer[balance transfer] transaction with a size of 133 bytes, the minimum fee is 10^-5^ LSK/byte × 133 bytes = 0.00133 LSK. In the case of link:2-transactions.adoc#delegate-registration[delegate registration] transaction with a size of 124 bytes, the minimum fee of the transaction is 10 LSK + 10^-5^ LSK/byte × 124 bytes = 10.00124 LSK.
