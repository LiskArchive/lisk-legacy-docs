...
Add the code for the customized `node` application (including the custom transaction objects), to a public code repository.
...
* {url_github_transport_transactions}[transactions/^]: A folder containing all required custom transaction objects.
...
== Write tests for custom transaction objects

The more complex the logic inside the custom transaction objects, the more complicated it becomes to verify that the custom transaction object logic is working as expected.
