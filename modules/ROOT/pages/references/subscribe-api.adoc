...
The blockchain update API can be accessed by the following path wss://service.lisk.io/blockchain.

If you are interested in accessing the testnet network, this can be accomplished by using the wss://testnet-service.lisk.io/blockchain endpoint.
...
const connection = io.connect('wss://service.lisk.io/blockchain', { transports: ['websocket'] });
