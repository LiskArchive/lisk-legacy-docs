...
The xref:{url_sdk_docs}[Lisk SDK] is an open-source software development kit which enables developers to easily build scalable sidechains using JavaScript & Typescript.
...
The consensus algorithm used by Lisk mainchain and sidechain is a hybrid version of DPoS and PoS algorithms.
...
If a validator uses their tokens to increase their own <<validator-weight>>, this is called a self-stake.
...
A standby validator, or inactive validator, is classed as a validator that does not have the sufficient validator weight and therefore is not eligible to generate new blocks. A standby validator can become an <<active-validator>> if their <<validator-weight>> increases.
...
The registration process is performed by posting a validator registration transaction to the network.
...
A transaction type on the mainnet which submits stakes for validators for staking purposes.
If the stakes are positive it adds to the staking weight, if they are negative it removes the staking weight again.
If staking weight was added, the corresponding LSK tokens are locked and cannot be used for any other transaction.
...
It is computed from the <<self-stakes>> and the total amount of received <<un-staking, stakes>>.
