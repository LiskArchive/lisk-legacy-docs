...
:description: This part of the Lisk supply chain tutorial describes how to publish the application & make it portable including further improvements.
:toc: preamble
...
:url_github_transport_app: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/transport/client/app.js#L14
:url_github_transport_node_index: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/transport/node/index.js
:url_github_transport_node_packagejson: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/transport/node/package.json
:url_github_transport_transactions: https://github.com/LiskHQ/lisk-sdk-examples/tree/v4/archive/3.x/transport/transactions
...
:url_core_config_forging_enable: lisk-core::management/forging.adoc#forging_enable_disable
...
Secondly, it is necessary for the tracking script to be automatically enabled after the Raspberry Pi boot process has been executed.

This can easily be achieved on the Raspberry Pi by installing `pm2` globally using the following commands listed below:
...
Copy and paste it in the terminal again to complete the setup of the `pm2` startup script.
...
After completion of part 4, the set up should appear as shown below in the following diagram: xref:{url_tutorials_transport_2}[Part 3: A simple supply chain tracking system].
...
Most of the configurations can stay the same as defined in the default config options.
...
The transactions will be visible in the logs, (if the log level is at least `info`).
...
These new blocks are broadcast again to the new node, and the client application can display the data based on the API calls that it sends to the new node.
