...
To connect to another network, specify the `network` when starting the Lisk Core, as described in <<_command_line_options,Source Administration>>.
...
* xref:{v_sdk}@lisk-sdk::lisk-elements/packages/cryptography.adoc[Cryptography module of Lisk Elements]   **Comment: please note this alternative does not seem to be described in this section, and both links are inactive - we need to check this.. (Chris - 13 Dec)

Firstly, regarding the Lisk Commander, please ensure the Lisk Commander is installed in a secure environment.
...
Both the private and public keys will be needed in a location that is accessible to the Lisk core.
====
...
<4> Replace from 'path_to_key' to the actual path to key file.
<5> Replace from 'path_to_cert' to the actual path to certificate file.
...
For monitoring or debugging your node, the Lisk Core tracks all activity that occurs within the node by creating applicable log messages.
