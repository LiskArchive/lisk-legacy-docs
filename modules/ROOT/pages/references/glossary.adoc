...
The Base32 format is a user-friendly, human readable representation of addresses in a Base32 encoding and includes a checksum which detect typing errors. It is used to represent addresses on user interfaces. An address in this format is always 41 characters in length and contains decimal digits and lower-case letters, and finally starts with the prefix 'lsk'. An example address in this format will look like the following: ‘lsk24cd35u4jdq8szo3pnsqe5dsxwrnazyqqqg5eu’.
...
Addresses are used to identify accounts, and are derived from a public key, (pubkey), by computing SHA-256(pubkey) and truncating the output to its first 160 bits.
Having a length of 160 bits makes it computationally infeasible that two independent users would end up with the same address. A binary address example used at protocol level is shown here: 0xc247a42e09e6aafd818821f75b2f5b0de47c8235.
