...
// Project URLs
:url_sdk_guides_forging: ROOT::run-blockchain/forging.adoc
:url_sdk_protocol_puninshment: ROOT::understand-blockchain/lisk-protocol/consensus-algorithm.adoc#punishment
...
The command will ask for the delegate passphrase, and in order to symmentrically encrypt the passphrase for the config, it will also ask for a password.
...
<1> Set the console log level to `info` to view all relevant logs in the console.
