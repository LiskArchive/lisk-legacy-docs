...
* The payload hash of the genesis block. (Example: {url_github_devnet_genesis_payload}[Devnet genesis block payload hash^]).
...
Due to the changes that have been introduced for the structure of transaction objects, the genesis block and it's transactions were newly created for the Lisk SDK version 3.0.
...
|{set:cellbgcolor!}
**os** +
__optional__|The operating system, that the peer node is running on. +
**Example** : `"debian"`|string
|**protocolVersion** +
__optional__|The protocol version of Lisk Core that the peer node is running on. +
...
|**version** +
__optional__|The version of Lisk Core that the peer node is running on. +
...
The rank is defined by the voters weight/ approval of a delegate, in relation to all other delegates. +
...
|**epoch** +
__required__|Timestamp of the first block on the network. +
**Example** : `"2016-05-24T17:00:00.000Z"`|string (date-time)
|**fees** +
__required__||<<_fees,Fees>>
|**milestone** +
__required__|The reward that each forger receives for forging a block at the current slot.
...
For example, the nethash describes the Mainnet or the Testnet, that the node is connecting to.|string
| **nonce** +
__required__|Unique identifier of the node. Random string. +
MinLength: 16, MaxLength: 16, Example: `cJmXK66xxz644d67`|string
...
|**syncing** +
__required__|True if the node synchronizes with other peers. +
...
|**signatures** +
__optional__|If the transaction is a multisignature transaction, all signatures by the members of the corresponding multisignature group will be listed here.| string (signature) array
|**timestamp** +
...
|{set:cellbgcolor!}
**blockId** +
__optional__|The Id of the block which is included in this transaction. +
**Length** : `1 - 20` +
**Example** : `"6258354802676165798"`|string (id)
|**confirmations** +
__optional__|Number of times that this transaction has been confirmed by the network.
By forging a new block on a chain, all former blocks and their transactions contained in the chain are confirmed by the forging delegate. +
**Minimum value** : `0`|integer
|**fee** +
...
|**senderId** +
__optional__|Lisk Address of the sender's account. +
**Example** : `"12668885769632475474L"`|string (address)
|{set:cellbgcolor!}
**senderPublicKey** +
__required__|The public key of the sender's account. +
