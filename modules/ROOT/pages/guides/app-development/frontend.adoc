...
This frontend can be built with any technology stack you feel most comfortable with.
...
A page for generating new accounts allows to conveniently create credentials that can be used in the application.
...
The faucet is a component that allows accounts to receive tokens from the genesis account, which holds majority of initial tokens at the start of the Devnet.

In a new file `api.js`, the `apiClient` from package `lisk-client` provides an interface for the faucet and other React components to connect to the blockchain application via websocket on port 8888.
...
<6> After submitting the transaction and getting back the response, the state of the Faucet component is updated with the transaction object and the API response.
<7> If the transaction object is present in the state, it is displayed below the form on the same page.

=== Send transaction

Now that we are able to create a new account and receive some initial tokens for it, let's build a new component that allows to send tokens from an account to another.

To do this, create a new file `Transfer.js`.
The contents of `Transfer.js` are similar to `Faucet.js`, as we send a transfer transaction on both pages.
...
<1> Retrieves account details from the blockchain application, based on the address provided.
<2> If `state.account` is not empty, it will display the account details on the same page.

=== Index and navigation

Now that all the basic components for the frontend are created, let's add a small component for the landing page.
...
It is now possible to use the app in browser to create new accounts, fund accounts, see account details of a specific account, and send tokens from one account to another.
