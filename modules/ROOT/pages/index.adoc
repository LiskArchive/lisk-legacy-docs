...
The {url_github_sdk}[Lisk SDK] aims to provide an easy and reliable software development kit for building blockchain applications compatible with the Lisk Protocol.
...
The codebase is written entirely in JavaScript and TypeScript, which means for a majority of developers no significant change of tools is required to get started.
...
To get started with the Lisk SDK and the development of a blockchain application, check out the following sections int he documentation:

=== How-To Guides

The xref:{url_guides}[guides] section offers dedicated how-to guides about all relevant topics about building a blockchain application with the Lisk SDK.

The guides are sorted in a chronological order but can also be read seperately, or be used as sources to look up certain commands or snippets.

=== Tutorials

The xref:{url_tutorials}[tutorials] explain in detail how to build a specific blockchain application.
The examples in the tutorials each implement a simple, but valid industry use case.

The tutorials overview page provides an informative overview about all existing tutorials, the estimated time and the skill level to complete a particular tutorial.

TIP: All code for the example apps that are used in the tutorials is also available in the {url_github_sdk_examples}[lisk-sdk-examples repository] on GitHub.
