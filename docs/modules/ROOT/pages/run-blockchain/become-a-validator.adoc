...
The validator info data is required for enabling block generation for the corresponding delegate.
...
To not miss any rewards, it is of high interest for a validator to set the hash onion, before enabling forging on the node.
...
[source,bash]
----
./bin/run endpoint invoke random_getHashOnionSeeds
----
...
.. Never use outdated <<validator-info-data>> to enable block generation.
...
Being selected directly depends on the *validator weight*, and this is computed directly from the stakes and self-stakes of a delegate, as defined in the Lisk PoS protocol{fn_lip23}.
...
. <<checking-the-validator-status, Check the validator status>>
