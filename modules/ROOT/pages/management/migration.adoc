...
====
All actively forging delegates on the Lisk Mainnet and the Lisk Testnet need to follow this guide to correctly migrate their nodes to the new network, in order to don't miss any blocks after the network hard fork.

Optionally, every one else having running a Lisk Core 2.1.6 who is not an actively forging delegate can participate in the migration process.
In this case you can skip the steps 1.4, 2.7, 2.8 and 2.9 in the guide, as they are only relevant for delegates.
====
...
[source,bash]
----
curl -o lisk-migrator.tar.gz https://downloads.lisk.io/lisk/main/lisk-migrator.tar.gz
----
--
Testnet::
+
...
[source,bash]
----
curl -o lisk-migrator.tar.gz https://downloads.lisk.io/lisk/test/lisk-migrator.tar.gz
----
--
====
...
[source,bash]
----
curl -o lisk-migrator.tar.gz.SHA256 https://downloads.lisk.io/lisk/main/lisk-migrator.tar.gz.SHA256
----
--
Testnet::
+
...
[source,bash]
----
curl -o lisk-migrator.tar.gz.SHA256 https://downloads.lisk.io/lisk/test/lisk-migrator.tar.gz.SHA256
----
--
====

{counter:seq3}) Run the following command in the terminal and ensure the output is `sha256sum: <file name>: OK` (Note: The command name and output may vary on macOS).
...
Make the `lisk-migrator` command available in the PATH, e.g. by executing
...
<1> Replace `$HOME` with the absolute path of where the `lisk-migrator` folder is located, in case you extracted it someewhere else than your home directory.
...
The height is needed as a parameter of the `lisk-migrator` script in the next step.
...
If you have added the Lisk Migrator to the PATH as described in section <<setting-up-the-lisk-migrator>> you can start the migration script by running the following command in the terminal:
...
[source,bash]
----
lisk-migrator --snapshot-height ${snapshotHeight} --output /home/lisk/.lisk/lisk-core/config/mainnet/genesis_block.json --lisk-core-path /home/lisk/lisk-main
----
--
Testnet::
+
...
[source,bash]
----
lisk-migrator --snapshot-height ${snapshotHeight} --output /home/lisk/.lisk/lisk-core/config/testnet/genesis_block.json --lisk-core-path /home/lisk/lisk-test
----
--
====
...
* `--output`:
The absolute path to the directory, where the newly generated genesis block should be saved to.
* `--lisk-core-path`:
...
After the snapshot height is reached, delegates have approximately 2 hours time to start their Lisk Core 3.0.0 and enable forging, to ensure they won't miss any blocks after the hardfork.

In case the node is started at a later point in time, then the node will simply sync to the current network height.
For delegates, this might mean they miss a block, for everyone else it will not have any further impact.
...
Run the following command in the terminal and check the logs in the console to verify Lisk Core starts successfully:
...
Create a pm2 config as shown below for example:
...
After creating the config, to start it execute the following command:
...
As the last step, enable forging on the node for your delegate.
...
These three variables need to be set to `0` for every delegate who starts forging for the first time in the network, which is the case for every delegate participating in the migration.

[TIP]

====
When a delegate has already forged in the network, it is necessary to check which values to use for `HEIGHT`, `MAXHEIGHTPREVIOUSLYFORGED`, `MAXHEIGHTPREVOTED`.

A delegate can check these values by running the following command:
