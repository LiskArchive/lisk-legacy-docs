...
NOTE: To get a good overview about the general workflow we are going to implement, take a look at the xref:transport.adoc#_general_procedure[diagram on the Introduction page] of the workshop.
...
Start or restart the `node`, `client` and `iot` application, like you did in xref:transport1.adoc#_13_the_client_application[Step 1.3 in Part 1] of the workshop.
...
.Transport fail
image:9-fail.png[finish transport]
