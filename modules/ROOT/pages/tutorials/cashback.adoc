...
At this point it is now possible to start both the node and network, in order to to verify that the setup was successful by executing the following commands below:
...
<4> If a different user name than 'lisk' was used to access the database lisk_dev, then it will be necessary to update the username in the config.
...
Checking the account balance of the recipient can be performed by executing the following command below:
