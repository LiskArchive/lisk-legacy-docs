...
:url_protocol_blocks: understand-blockchain/blocks-txs.adoc
:url_protocol_consensus: understand-blockchain/blocks-txs.adoc
:url_protocol_transactions: understand-blockchain/blocks-txs.adoc
...
More specifically, within the Lisk documentation, whenever blockchain applications are discussed, this is directly referring to applications that run by using blockchains, which are compatible with Lisk Protocol. 
...
Unlike smart contract based platforms, where smart contracts have to share the same chain for their blockchain needs, Lisk provides the opportunity for each application to have its own sidechain.
...
The state machine, which is part of the application domain contains code written by the developer for a blockchain application.
...
*Each node has an instance of the Lisk blockchain client and together, these nodes form a chain*. The on-chain logic present in each instance of the Lisk blockchain application must be the same for a chain. However, the off-chain logic for each node can differ within a chain.
...
As the name suggests, a xref:{url_understand_state_machine}[state machine] is relevant to the states of a machine. A blockchain application relies heavily on its state machine to mutate the state of a blockchain. 
...
TIP: Lisk provides a range of default modules out of the box. These modules are created automatically whenever a blockchain application is bootstrapped via Lisk Commander. For more information, see the modules reference page.
...
However, the Genesis configuration and the configuration for Modules must be the same across the network of a blockchain application.
