...
:v_core: 3.0.0
:experimental:
:url_github_transport_client: https://github.com/LiskHQ/lisk-sdk-examples/blob/development/transport/client/app.js#L14
...
:url_github_hello_transaction: https://github.com/LiskHQ/lisk-sdk-examples/tree/development/hello_world/hello_transaction.js
...
:url_reference_config: reference/configuration.adoc
...
For example, for a proof of concept, in order to provide the already activated forging delegates inside the config; please use the Devnet genesis delegates in {url_github_sdk_configDevnet}[configDevnet] or create your own genesis delegates and add them to the config.
====
...
Once a new node is set up, update the API endpoint in the {}[client] app, to target the new node:
