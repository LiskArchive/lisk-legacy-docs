...
A reference of all endpoints and events that are provided by the Lisk Framework for a blockchain application.
...
Modules and plugins often expose additional events and endpoints to the application.
All endpoints and events registered with the application are available as RPC endpoints.

The endpoints and events of the official modules and plugins are described on their respective reference pages, see xref:{url_modules}[] and xref:{url_plugins}[].

== Endpoints

Endpoints are called to receive specific data from the blockchain application. Endpoints are part of the public request / response API, and are invoked via RPCs. The endpoints mentioned below are categorized based on various namespaces as follows:
...
=== Usage
Each endpoint can be called by mentioning its namespace and endpoint's name together like this: `namespace_endpointName`. For details, check out cURL requests for endpoints mentioned below.
...
==== `system_getSchema`
Returns schema for the entire application.
...
Returns `transactionId` of the transaction being posted.
...
Returns transactions present in transaction pool.
...
==== `txpool_dryRunTransaction` (Incomplete)
...
==== `consensus_getBFTParameters` (Incomplete)
...
==== `Generator`
==== `generator_getStatus`
Returns the status for all accounts generated.
