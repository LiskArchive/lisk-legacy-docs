...
[TIP]
====
{url_github_transport_alarmtx_solution}[Please see the solutions at /transactions/solutions^] for the custom transaction objects.
...
<4> Contains the custom transaction objects, that are used by the node and client app.
...
TIP: Please see the Lisk documentation for an xref:{url_customize_interface}[overview about the required methods for custom transaction objects]
...
This holds the core business logic of your custom transaction objects.
...
As a new custom transaction, `LightAlarmTransaction` has been created, this needs to be registered with the node application.
