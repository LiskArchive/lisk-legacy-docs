...
:description: The Lisk Transport tutorial teaches how to build a blockchain application with IoT devices communicating with the network. It uses a raspberry pi with a light sensor to track the transport of a packet on the blockchain as part of a supply chain tracking sytem. It further details how to implement a simple trust system and how to finally publish the application.
:toc:
...
The diagram shown below describes the general workflow of the supply chain tracking system.
A detailed description of each step can be seen here.
...
* `minTrust`: The minimum amount of trust a carrier requires in order to to transport this packet.
* `recipientId`: The ID of the desired recipient of the packet. Only the recipient can send the `FinishTransaction` later, which will complete the transport.
...
During the transportation of the packet, it will track itself with different sensors in order to check if it is manipulated during the transportation phase.
If this occurs, the packet will send a self-signed alarm transaction to the network.
...
NOTE: xref:tutorials/transport0.adoc[Click here to go to the installation guide]
