...
The default database name is `lisk_dev`, so for the development purposes, a command `createdb lisk_dev --owner lisk` must be used for the set up.
...
Lisk SDK is an all-in-one package that provides you with tools to create, run and maintain blockchain applications in JavaScript.
...
TIP: If the API of the node is public, it is also possible to send requests and receive live responses from the node using Swagger UI.
