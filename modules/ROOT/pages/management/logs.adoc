...
:description: A guide that explains the log levels and associated messages which can be used for monitoring or debugging a node.
:toc:
...
All logs that have equal or higher log levels than in the `config.json` specified file log level are saved in a `.log`-file for further analysis.
