...
In the "Understand Lisk blockchain" category you learn the basic concepts of Lisk blockchain, Lisk Network, and Lisk Sidechain application{fn_sidechain_glossary} (sApp).
The page discusses the topics and terms that are important to know when developing within the Lisk ecosystem.
...
A *sidechain application* (or, in short, *sApp*) refers to a software that contains a lisk based *sidechain client*, a *middleware* and a *user interface*.
...
Sidechain client::
A software that runs a node for a particular sidechain, and follows the Lisk protocol.
For example, xref:{url_lisk_core}[] is the blockchain client of the Lisk Mainnet.
The xref:{url_lisk_sdk}[] offers a lot of helpful tools to build a fitting sidechain client for your application, check out xref:{url_build_index}[] for more information.

Middleware::
Although this component is optional, it will become essential for bigger blockchain applications in the long run.
A middleware can prepare, aggregate and enrich data from the blockchain network, so it can be easily used and digested by the application frontend.
Lisk provides a ready-to-use middleware, which can be easily configured for any sidechain application:
...
User Interface (UI)::
The frontend, or User Interface of sidechain applications is highly customizable, just use your preferred exising Frameworks to develop a nice UI which fits your individual use case.

An highlevel overview of the Lisk ecosystem can be seen below in the following diagram.
...
== Network topology
The Lisk blockchain ecosystem consists of various interconnected sApps.
...
NOTE: Other components of the application that are not part of the three domains, such as the Transaction Pool, etc. can be implemented differently by the developer if desired, without breaking the protocol.
...
* *Application* layer (represents the <<app-domain,application domain>>)
...
A sApp relies heavily on its state machine to mutate the state of a sidechain.

* *States:* A state machine is deterministic and can have multiple states, but only one state at any given time.
For example, a basic analogy could be whereby a padlock represented as a state machine would have the following two states "Unlocked" and "Locked".

* *Transitions*: A transition is defined as the instantaneous transfer from one state to another state.
For example, using our padlock analogy, to change from the state "Unlocked" to "Locked", a transition would be "Close padlock", and then from "Locked" to "Unlocked", it would be "Insert key & turn".
...
These modules are used automatically, whenever a sidechain application is bootstrapped via Lisk Commander.
...
The communication architecture of a sidechain client allows internal application components and external services to communicate to the client via various channels.
