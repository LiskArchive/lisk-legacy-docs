...
And, when the HTTP mode is enabled the API can be accessed via the `/rpc` path.
...
NOTE: When creating the transaction against a sidechain node, same command (`transaction:create`) can be used. Please replace `lisk-core` with the corresponding application's CLI tool or use `./bin/run` as the CLI tool, when building the application binary from the source code.
