...
After completing the above steps, the very first version of the public testnet may look like this:
...
The new genesis block shall have, for example, the following properties:
...
To define a node as a seed node, simply add the IP and port of the seed node to the config as described in section <<update-the-configuration-file>>.
...
One account is always connected to the faucet, to provide the tokens to bbe distributed through the faucet.
To prevent the faucet from running dry, faucet account should always have a high enough balance to be ablee to provide the requested tokens to users.
