...
Most of the configurations can stay the same to what is defined in the default config options However, please note that there is one option that should be updated: **The seed node(s)**.
...
.. Send some funds(at least enough to register as a delegate) to the newly generated address, to initialize your account in the network.
. Register a delegate
...
[source,bash]
----
lisk transaction:create:delegate lightcurve | tee >(curl -X POST -H "Content-Type: application/json" -d @- 1.2.3.4:4000/api/transactions) <1>
----
<1> Replace `1.2.3.4` with the IP of a node with a public API.
+
...
Or let the network know the current location of the packet by transmitting the GPS location in a certain time interval.
