...
rm -rf ~/.nvm
----
. Add a new session and set the iTerm to open in the **Intel** mode. Now set a shortcut key for opening the iTerm in the aforementioned mode.
Following that, open a terminal in Intel mode.
...
=== Install Python (M1 Macs only)
To install Python on an M1 Mac, execute the following commands:
[source,bash]
----
brew install pyenv
