...
:description: This section covers the config object, and the default values including a description of each value.
:toc:
...
They will be used automatically when starting the application without designating a specific configuration.
...
<49> Port to host the Lisk Wallet on. The default is 443, however it is recommended to use a port above 1024 with IP tables.
...
<59> Time to wait for a response from the server before timing out.
...
<77> If true, enables synchronizing, (fallback for broadcasts).
