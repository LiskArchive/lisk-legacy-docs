...
[loweralpha]
. Plugins can be used to develop a UI for a blockchain application, in case of a very simple and small UI.
...
Please be aware, every node has only a few basic API endpoints(here link to RPC endpoints page), which might not be ideally suited for more complex UIs. 
In this case, we recommend to use the enriched API of Lisk Service as middleware to communicate between frontend and blockchain application.
