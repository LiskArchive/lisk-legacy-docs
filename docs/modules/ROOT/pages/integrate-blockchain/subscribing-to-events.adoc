...
That means, *blockchain events can only be received for the 300 latest blocks by default.*
This can be changed by updating the `*keepEventsForHeights*` property in the sidechain client's config.

TIP: To store the blockchain events on a node forever, set `keepEventsForHeights` to `0` in the sidechain client's config.
...
NOTE: Different to the RPC events, blockchain events cannot be directly subscribed.
