...
+
--
Prepare a JSON object that contains all required <<parameters>> for sending a multi-signature registration.
...
+
--
Create a transaction like normally for example with the API client, like explained here: xref:{url_integrate_createtxs_apiclient}[How to create a transaction via API client].
--
=====
...
The main thing that needs to updated in the script, is the transaction object to be signed.
Here, you should put the transaction that was created in the previous step <<how-to-create-a-multi-signature-transaction>>.

Once all required signatures for the respective multi-sig transaction are collected, it can be sent like a normal transaction, like described here: xref:{url_integrate_posttxs_apiclient}[].
--
=====
