...
:description: Covers all Docker image commands relevant to manage Lisk Core with a Docker image.
:toc:
:page-next: /lisk-core/3.0.0/management/configuration.html
:page-previous: /lisk-core/3.0.0/setup/docker-image.html
...
:url_docker_compose_file: https://docs.docker.com/compose/compose-file/#ports
:url_docker_compose_environment: https://docs.docker.com/compose/compose-file/#environment

:url_lisk_snapshots: http://snapshots.lisk.io

:url_config: management/configuration.adoc
:url_config_cl: reference/config.adoc#command_line_options
:url_setup_docker_install: setup/docker.adoc#install

This section describes how to work with a Docker image-based Lisk Core installation, and covers the available basic commands which can be used to manage your Docker image-based Lisk node.
...
It will read both the `docker-compose.yml` and your customized `docker-compose.override.yml` file, which can be executed with the following command shown below:
...
(Please see: {url_docker_compose_file}[Docker ports])
...
(Please see: {url_docker_compose_environment}[Docker environment])
