...
Now import the `ReactInteroperableMethod` into the module and use it to create a new member `crossChainMethod` inside the module.
...
Now go the file `react/commands/react_command.ts` and import the constants, schemas and types defined above.
...
Go back to the file `react/module.ts` and update it ass described in the code comments.
...
To keep the example simple, we only check if the `receivingChainID` parameters doesn't equal the parameter of the sending chain.

You can extend the `verify()` hook to include more checks for the other parameters as well, as desired.
...
For this, adjust the `execute()` hook as shown in the snipet below.

Use the `.send()` methods of the interoperability module to send the prepared CCM.
...
The CCM will now be sent to the mainchain by a xref:{}[relayer node], where it will be forwarded to the designated receiving sidechain.
...
To learn how to implement cross-chain comands on the receiving chain, check out the next guide: xref:{url_build_execccm}[]