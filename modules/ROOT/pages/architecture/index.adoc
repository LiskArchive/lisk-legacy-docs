...
It could be a simple use case such as creating an HTTP API for a blockchain or providing some aggregation service.
...
It could be a frontend, a CLI or any app developed independently in any other technology stack.
...
The framework architecture is constructed of three different layer of abstractions: `Node`, `Modules` and `Plugins` (see the <<architecture-diagram>>).
