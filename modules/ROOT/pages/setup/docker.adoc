...
+
--
Please refer to {url_docker_install_linux}[docker installation for linux].
...
Where `<network>` refers to the Lisk network you wish to establish a connection to.
...
When your node is synchronizeded, the values of `networkHeight` and `height` should be either equal or almost equal.
...
To proceed to the next step, please see the xref:{url_admin_docker}[Docker Administration] to learn how to manage your Node.
