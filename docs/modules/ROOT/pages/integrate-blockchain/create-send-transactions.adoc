...
NOTE: Please note that when WS is enabled, the API can be accessed via the `/rpc-ws` path.
In addition, when HTTP is enabled the API can be accessed via the `/rpc` path.
...
This can be achieved by just replacing `lisk-core` with `./bin/run`.
