...
An account can be classed as any data associated with an address.
...
The API can be defined as the interface for the module-to-module communication.
...
Furthermore, an updated Multisignature registration is now used, based upon new registration params and emit events.
...
The interoperability module provides basic functionalities that will allow the transmission of information between interoperable chains in the Lisk ecosystem using cross-chain messages.
...
Furthermore, it also implements a command allowing delegates without a BLS key to register one.
...
Furthermore, the NFT module ensures the NFT unique identifiers are unable to be modified.
...
Proof-of-authority (PoA) is a consensus mechanism that is dependent on reputable and trustworthy pre-defined validators to produce blocks,
...
Validators in Lisk <<DPoS - Delegated Proof of Stake>> and Lisk PoA chains share many common properties, for example, such as the BLS and generator keys.
