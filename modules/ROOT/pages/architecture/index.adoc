...
The Lisk Framework is an application framework that integrates the Lisk Elements packages in order to form a functioning Lisk blockchain application.
...
The framework architecture is constructed of three different layers of abstractions.
The `Application` object encapsulates all together to provide a user-facing interface.

== On-Chain Architecture
This layer of abstraction provides collection features to configure and run any business logic on the blockchain.
...
In short, the on-chain logic has the following attributes:

* It can introduces state mutations through blocks or transactions.
* It is part of the blockchain protocol.
* It is verifiable by anyone just by synchronizing with the blockchain.
* It is handled in "modules".

More information can be found in the section xref:{url_explanations_on_chain}[On-chain architecture]

== Off-Chain Architecture
Some business flows or requirements are necessary to be implemented to perform certain computations, which are based on the blockchain data but are not actually required to change or alter any blockchain states.
It could be a simple use case such as creating an HTTP API for your blockchain or providing an aggregation service.
The off-chain architecture provides features to cover all such use cases.

In short, the off-chain logic has the following attributes:
...
You can connect either to a IPC or WS interface to communicate with the blockchain.
...
It can be instantiated by deploying either of the two following methods shown below:
