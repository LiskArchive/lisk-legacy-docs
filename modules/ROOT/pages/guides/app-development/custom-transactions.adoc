...
:url_lisk_protocol_transactions_pool: {v_protocol}@lisk-protocol::network.adoc#pool
...
The `store` is available inside the `prepare()`, `applyAsset()` and `undoAsset()` methods, and provides procedures to get and set the data from the database.
