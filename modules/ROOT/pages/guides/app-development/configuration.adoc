...
This guide explains in a step-by-step process, how to configure a blockchain application built with the Lisk SDK.
...
Together with the `genesisBlockDevnet`, which defines the <<genesis_block, genesis block>>, it sets up a local development environment with 101 forging genesis delegates.
...
This genesis block can be used together with the `configDevnet` to quick start your application with a preconfigured devnet.

TIP: Go to Github, to see the complete file {url_github_genesis_block}[genesis_block_devnet.json^]
...
Specific constants for the blockchain application are set inside the `configDevnet.genesisConfig`.
...
TIP: To see a full list of all constants and their predefined values, please view the file {url_github_constants_schema}[application_config_schema.js^].
...
Please see the xref:{url_rpc_endpoints}[RPC Endpoints] and xref:{url_architecture_communication}[Communication Architecture] pages for more information about the API access options.
