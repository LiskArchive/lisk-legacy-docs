...
NOTE: All blockchain related data like blocks, transactions, account state and chain state are encoded using `@liskhq/lisk-codec`.

Especially, schemas are used in:
...
* *assets*, for the `schema`, which is defines the data structure and formats of the transaction asset.
* modules and/or assets, to *decode/encode* other specific data from the database, like blocks, transactions, account state and chain state data.

== Format

Schemas must be defined as the example below, which is a modified JSON schema.
