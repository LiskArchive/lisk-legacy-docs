...
The user who will later deploy the node on a server can then add the credentials to the config, to xref:{url_core_management_forging}[enable forging] for a particular delegate on this node.
====
...
Now, the new client node will receive transactions from the client app and vice versa.
The received transactions are visible in the logs of the lcient node (if log level is at least `info`).

.Logs of newly added client node
