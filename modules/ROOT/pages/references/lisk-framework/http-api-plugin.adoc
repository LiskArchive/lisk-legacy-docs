...
Defaults to only allow the local host.
<4> `cors`: Contains options for cross-origin resource sharing (CORS).
...
|Type|Name|Description|Schema
|**Body**|**data** +
__required__|Password for decrypting passphrase of a delegate for its corresponding address.|<<_forging_patch_data,data>>
|===
...
**Example** : `true`|boolean
|**password** +
__required__|Password for decrypting passphrase of a delegate. +
**Minimum length** : `5` +
...
|**generatorPublicKey** +
__required__|Hex encoded value of the public key of the delegate who forged the block.|string
|**height** +
...
Represents the total amount of tokens that the delegates voters voted this delegate. +
**Example** : `"1081560729258"`|string
|**username** +
