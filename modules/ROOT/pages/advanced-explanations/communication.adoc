...
A blockchain application communicates via <<actions>>, <<reducers>> and <<events>> which can be invoked via xref:{url_glossary_rpc}[Remote-Procedure-Calls (RPCs)] (actions, reducers) or subscribed to (events).
...
It is used especially in the xref:{}[lifecycle-hooks], to publish the events of the module.
