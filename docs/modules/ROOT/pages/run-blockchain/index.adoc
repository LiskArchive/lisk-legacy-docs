...
A sidechain has to always communicate with other blockchains via the mainchain.

Consider a sidechain *S1* with 20 nodes, out of which, a node is configured to act as a relayer node.
The job of this relayer node is to collect CCUs & CCMs from other nodes of *S1* and then pass them on to the receiving mainchain.
