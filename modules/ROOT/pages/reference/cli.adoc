...
The configuration for Lisk Core is stored in `~/.lisk/lisk-core/config` footnote:snap_footnote[Snap versions of Lisk Core store their everything in `~/snap/lisk-core/current/.lisk/lisk-core` instead of `~/.lisk/lisk-core`].
