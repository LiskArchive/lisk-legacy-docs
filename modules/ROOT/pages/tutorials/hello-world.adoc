...
Welcome to the step-by-step guide of creating the Hello World application with the Lisk Alpha SDK.
Firstly, a simple App can be used which showcases a minimal setup of a blockchain application with 1 xref:customize.adoc[custom transaction] type: the "Hello" transaction.
...
The default database name is `lisk_dev` The following command,  `createdb lisk_dev` should be used.
