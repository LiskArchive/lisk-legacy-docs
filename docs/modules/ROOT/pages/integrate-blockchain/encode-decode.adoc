...
To encode or decode a block with the ID `6533da00cec82c6d22b1c12f6cb4b6178fbfd9530e35d1018fcf4585085ec9c6`, then execute the script with the block ID as the parameter:
...
Executing the script will return the block in the different formats, as shown below:
...
.Example: Decoding/Encoding a transaction
[source,js]
----
const { getClient } = require('./api-client');
const assert = require("assert");
...
To display a transaction with the ID `130227fa63ac60edbbacb6dae709cf9304ab0181ef7ea28105764f6240d012f2` in the different existing formats, then execute the script with the transaction ID as the parameter:
