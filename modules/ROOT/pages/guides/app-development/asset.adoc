...
The file `nft.js` was already used in the guide xref:{url_guides_module_nft_utils}[], the functions `getAllNFTTokens()`, `setAllNFTTokens()`, `createNFTToken()` can simply be added, if not present yet:
