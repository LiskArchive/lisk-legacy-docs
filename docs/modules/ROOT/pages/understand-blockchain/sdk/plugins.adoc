...
Plugins offer useful features to the xref:{url_sidechain_glossary}[Sidechain Application] (sApp) and external services. Plugins are part of the off-chain logic, and are implemented inside the xref:{url_sidechain_client}[sidechain client].
