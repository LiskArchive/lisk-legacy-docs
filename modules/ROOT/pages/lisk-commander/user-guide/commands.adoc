...
<4> If true, displays output nicely formatted. Has no effect if json:false.
...
The later nodes serve as a fallback, if query to the first node was not successful.
...
In order to decrypt the encrypted message later your recipient will need your public key (to verify the message came from you), the nonce and the secret passphrase which matches the specified public key.
...
It has one required parameter which is the delegate username to be registered.
...
inlcude::partial$sensitive-inputs-warning.adoc[]
