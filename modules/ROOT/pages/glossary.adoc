...
:url_blockchain_app: introduction/blockchain-applications.adoc
:url_mainnet: introduction/blockchain-applications.adoc
:url_state_store: lisk-elements/chain.adoc#state-store
...
A reducer is classed as a function that determines changes to the state of an application. It can use the previous state and action to execute another state.
...
The xref:{url_state_store}[state store] is a data structure that holds a temporary state while processing a block. It is used to retrieve data from the blockchain, and exposes an interface to create, restore, and finalize snapshots. It is based on the following 3 states: Accounts, Chain and Consensus.
...
A transaction can have multiple purposes, e.g. a balance transfer or delegate registration, and posseses a common set of properties. Each transaction induces a specific operation on the accounts and is defined by a unique schema and transaction logic.
