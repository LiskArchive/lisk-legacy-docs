...
For the creation of the namehash output of the domain, aka node, reuse the `namehash.hash()` function of the `eth-ens-namehash` NPM package.
...
The function  `createLNSObject()` expects
.src/app/modules/lns/storage.ts
[source,typescript]
----
