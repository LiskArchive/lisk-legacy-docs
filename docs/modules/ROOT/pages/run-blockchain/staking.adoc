...
That means, the tokens used for staking stay in the account of the staker, but while they are used for staking, they are locked and cannot be used for any other purposes like for example token transfers.
...
IMPORTANT: The *minimum delegate weight required* to be selected as a block generator is 1000 LSK in the Lisk Mainnet.

Formally, the validator weight is defined as the following:

----
validatorWeight = minimum { 10 * selfStakes , totalStakes },
----

* `selfStakes` is the amount the validator voted for themselves, and
* `totalStakes` is the sum of all staked tokens (including self-stakes) for a specific validator.
...
To do this, send a corresponding Unlock transaction to the node.
...
//TODO: Test this with an account that has locked tokens
[tabs]
=====
Lisk Core::
+
