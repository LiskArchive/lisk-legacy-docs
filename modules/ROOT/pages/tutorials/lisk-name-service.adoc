...
If no errors are thrown, the application will start to add new locks every 10 seconds after the initial startup.
...
If the blockchain application was bootstrapped newly with `lisk init`, it still contains a default genesis block, which doesnt innclude the account schema of the new LNS module.
...
A lot of detailed information about this process can be found in the guide xref:{url_guide_genesisblock}[].

---

Stop the LNS application, if it is already running, and then start the LNS blockchain application again with the following command, to apply all the new logic that has been described above:
...
image:tutorials/lns/dashboard.png[Dashboard,200,100]
...
image:tutorials/lns/faucet.png[Faucet,200,100]
