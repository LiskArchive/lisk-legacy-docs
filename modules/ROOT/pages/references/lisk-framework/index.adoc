...
The application logic can be configured as desired in the xref:{url_reference_config}[Lisk SDK configuration]. Please see also the <<configuration>>.

[[modules]]
=== Modules

Modules are located in the `modules` folder.
These are the individual building blocks for the Lisk SDK.
...
Custom modules can be plugged into the Lisk SDK and may offer new features and capabilities for the application, whilst also replacing the default modules functionalities where necessary.
They extend the existing instance with a specific (and circumscribed), set of features as can be seen below:
...
The controller emits an event, `lisk:ready` which can be used to perform certain activities, that may be required to be performed when all the other modules are loaded.
...
By default, the modules will run in the same process as the controller, which loads the module.
...
Multiple modules can be defined by using commas, as shown below:
...
The life cycle of a module consists of the following events in the correct order as shown below:
...
This is used to subscribe to events occurring on the controller as shown below:
...
This is used to publish events to the controller, which will be delivered to all events subscribers as shown below:
