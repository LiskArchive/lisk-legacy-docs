= RPC API endpoints for Lisk nodes
...
A reference of all endpoints and events is provided by the Lisk Framework for a blockchain application.

TIP: For more information about the public, application endpoints, check out the xref:{url_general_endpoints}[] page.

== Module and plugin-specific endpoints and events

Modules and plugins often expose endpoints and events to the blockchain application. All endpoints and events registered with the application are available as RPC endpoints. For details about modules and plugins, see xref:{url_modules}[] and xref:{url_plugins}[].

== Usage
Endpoints and events exposed by Lisk are categorized based on various namespaces. These namespaces point toward various, modules, plugins, and default application-specific endpoints and events. To invoke an endpoint or to subscribe to an event, combine the name of an endpoint/event with its namespace. For example:

* `namespace_endpointName`
* `namespace_eventName`

For details, check out the endpoints and events discussed below.

== Endpoints

Endpoints are called to fetch specific data for an application. Lisk node endpoints are public and can be invoked via RPCs. These endpoints have been categorized based on various namespaces, as follows:
...
Events exposed by Lisk RPC node can be subscribed as explained in the xref:{url_subscribe_event}[] section. To subscribe to an event, it is necessary that a WebSocket or Inter-Process Communication protocol should be used. The default events exposed by the Lisk Framework are described below:
...
==== network_newBlock
Emitted when a new block is generated in a node.
...
Emitted when a new transaction is published on the node.
...
Emitted when the a new transaction is added in the transaction pool.
