...
A sidechain is actually a separate blockchain where the inclusion of a transaction in one of the blockchains can have an effect in the other blockchain.
Sidechains are aligned in parallel with the main blockchain.

In a nutshell, sidechains allow for an overall increase in transactions to be performed, and nodes containing assets or data from the parent blockchain to be securely used within a separate blockchain, namely a sidechain.
In turn, the advent of sidechains can increase the performance of an ecosystem of blockchains by taking some of the workload.
