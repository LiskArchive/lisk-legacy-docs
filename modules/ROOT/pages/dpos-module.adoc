...
This allows the sending of a register delegate transaction, which registers a delegate for the sender account with a given username.
...
This allows the sending of an unlock transaction, which unlocks token that have been locked after voting for a delegate, after unvoting this delegate.
...
This allows the sending of a proof-of-misbehavior transaction, and reports violations of the BFT protocol by a particular delegate.
...
This returns a list of all registered delegates, including their username and address.
...
<2> Address of the delegate as a hex string.

=== getUnlockings
This returns a list of delegate unvotes of a certain account, the height of the unvote, and the minimum height for unlocking the tokens again.
...
<1> Address of the account as a hex string.
...
[NOTE]
====
The DPoS used by Lisk can be construed as more of a middle ground between {url_wiki_pos}[PoS] and DPoS.
...
This technique enables a very energy efficient process of adding new blocks, which allows forging nodes to run even on machines with very limited processing capabilities, for example, such as a Raspberry Pi.
====
...
This will start the unlocking procedure and the LSK tokens will be ready for unlocking 2000 blocks later, (approximately 5 hours and 30 minutes).
...
Any misbehavior can therefore be reported by providing the two contradicting block headers.
