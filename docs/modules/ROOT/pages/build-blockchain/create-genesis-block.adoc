...
<3> `genesis_assets.json`: Contains details about the initial state of a sidechain such as account details for `token` and `pos` modules. Also contains the list of validators, etc.
...
For every new sidechain, a unique genesis block is required. As mentioned earlier, a default genesis block becomes available right after the client's `init` process. 

However, whilst running a public network, you may want to regenerate a genesis block for various use cases such as assigning different tokens to specific addresses of your choice or recreating the genesis block based on the needs of modules, for example, setting up the initial constraints required for a staking module, etc.
