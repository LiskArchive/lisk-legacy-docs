...
:url_tutorials_transport_1_client: tutorials/supply-chain/part3.adoc#client
...
Now try to start or re-start the `node`, `client` and `iot` application, exactly as performed earlier in xref:{url_tutorials_transport_1_client}[Step 1.3 in Part 2] of this tutorial.
...
Once all of the above steps have been completed, a simple and fully working proof of concept of a decentralised supply chain tracking system is now running on your machine.
