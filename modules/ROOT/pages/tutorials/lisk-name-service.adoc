...
All three actions are returning an <<lns-node-schema, LNS object>>, based on differeent input parameters like the address it os registered to, the domain name that is registered in the LNS object, or a hash of the LNS object, which is typically stored in user account under `lns.ownNodes`.

The main logic of the different actions is imported from the file `storage.ts`, and is explained in detail in step <<utility-functions-3>>.
