...
A fork can be defined as a change in the network. It is whereby an alternative version of the blockchain is created by generating two blocks on different parts of the network simultaneously. +
Generally speaking, a fork can be defined into one of three categories, a soft, hard, or temporary fork. A soft fork can occur when the blockchain protocol is changed into a backward-compatible manner. For example, a SW upgrade that is backward-compatible with older versions would result in a soft fork taking place. +
Alternatively, if the blockchain protocol is changed into a non backward-compatible manner. For example, the release of a new SW upgrade that is not compatible with the previous versions, which would require all users to upgrade to continue performing and validating new transactions, then a hard fork would occur. +
Finally, in the case whereby two miners mine a new block simultaneously, then the occurrence of a temporary fork will happen.
