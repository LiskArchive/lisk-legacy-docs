...
`HelloAPIPlugin` is a default plugins which comes with the Lisk SDK.
...
In addition to the hello module, we add a `HelloAPIPlugin`, which provides additional API endpoints.
These API endpoints allow the react client get information about the newly created hello properties of the application via API calls.
