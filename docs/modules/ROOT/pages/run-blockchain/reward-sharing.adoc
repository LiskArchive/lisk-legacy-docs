...
The set of validators that generate blocks, called generators, is chosen based on the total stake of each validator (self-stake and sum of staked amounts from stakers).
...
NOTE: Because validators need to self-stake for their accounts to become eligible to generate blocks, validators will receive additional rewards here, like their other stakers, too. For example, consider an active validator has 0% commission, and at the same time no other accounts are staking for the validator. In this case, the validator would still receive 100% of the block rewards.
...
In this example, the validator is set to keep *85%* of the rewards and share *15%* with their staker, hence the value of `85`.
+
...
However, a staker needs to manually claim their rewards, and that can be done via both Lisk Desktop and CLI.
...
. Once the transaction is executed and the block is finalized, the staker's account balance will increase as per the earned reward.
...
----
+
. xref:{url_posting_transaction}[Post the transaction] to the node for execution and once the block is executed, check your xref:{url_getbalance}[token_getBalance] endpoint.
