...
The on-chain architecture of the `Application` consists of the following components,
one is the `Node` and the others are the `Modules`.

The `Node` is the main entity in charge of performing the following actions on the blockchain:

* Forging blocks.
* Synchronizing with the network.
* Processing blocks from the network.
* Also broadcasting blocks and transactions to the network.

The `Node` is a private abstraction layer of the above-mentioned activities, which is the reason it is not visible to the SDK user.
...
Firstly, modules define an account schema to store the module related data in the account.
The definition of this schema is totally flexible and it is possible to define very complex data structures as well, if required.

Secondly, the `BaseModule` which every module extends from, offers various lifecycle hooks that allow each module to execute certain logic before or after transactions or blocks.
It also it defines the logic for the transaction assets of the module. Furthermore, it is possible to control the behavior of each module.

Finally, modules expose an interface.
...
Actions and Events are exposed to the plugins and external services.
More information about the exposed interface can be found in the xref:{url_explanations_communication}[Communication Architecture] section.

.When to create a custom module
[NOTE]
====
Modules are able to perform the following:
...
[NOTE]
====
Create a custom asset for every transaction type that you wish to use in the blockchain application.

Assets are able to perform the following:

* Define a schema for data sent through a transaction asset.
* Validate the data.
...
The steps listed above are exposed via the base module and base asset.
For further details see the xref:{url_references_framework_basemodule}[Lisk Framework reference].

=== Example: A hello module

An example of a simple implementation of a custom module is shown below:
...
<1> `name`(required): this is used for a key of the account schema if defined.
<2> `id`(required): this is used for a fieldNumber for the account schema, and as a `moduleID` when sending a transaction.
<3> `accountSchema`: this defines the account schema for the module.
...
<8> `beforeTransactionApply`: The code in here is applied before each transaction is implemented.
<9> `afterTransactionApply`: The code in here is applied after each transaction is implemented.
<10> `afterGenesisBlockApply`: The code in here is applied after the genesis block is implemented.
<11> `beforeBlockApply`: The code in here is applied before each block is implemented.
<12> `afterBlockApply`: The code in here is applied after each block is implemented.

=== Example: The hello asset

An example of a simple implementation of a custom asset is shown below:
...
<1> `name`(required): This is used for the UI.
<2> `id`(required): This is used as a `AssetID` when sending a transaction.
<3> `schema`(required): This defines the `asset` schema for the transaction.
<4> `validate` is used to validate the asset data before it is applied.
In the case whereby the validation fails, an error is thrown.
<5> `apply`(required): This defines a state change induced by this asset.
In the `HelloAsset`, it adds the hello string that was sent in the transaction to the senders account and increments the `helloCounter`.
