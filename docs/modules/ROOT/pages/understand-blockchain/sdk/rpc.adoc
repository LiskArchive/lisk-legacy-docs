...
xref:{url_sapp}[sApp] developed with Lisk SDK communicates to a node via RPCs. RPC-based interfaces i.e. IPC, WS, or HTTP allow applications to *invoke* and *subscribe* (WS and IPC only) to RPC endpoints and events. 
