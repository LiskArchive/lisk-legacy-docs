...
Then, look for the `outbox` store of the `interoperability` module.
...
|`MODULE_PREFIX`
| The first 8 bytes of the `key` value is the `MODULE_PREFIX`.
For example, in the above-mentioned snippet, the `MODULE_PREFIX` is: `83ed0d25`.

|`SUBSTORE_PREFIX`
|The last 4 bytes of the `key` value is the `SUBSTORE_PREFIX`.
