...
An example of how to sort the validator list can be found in the example script {url_github_exmaples_sidechaintegistration}[sidechainRegistration.ts^].
====
...
Taking this into account, we decide for a `sidechainCertificateThreshold` in the middle of minimum and maximum: 65
...
. `keys.json`: A file with the `blskey` values of the validators intending to sign the registration transaction.
.. This file can be generated for a specific validator with the xref:{url_sdk_cli_keyscreate}[keys:create] CLI command, by providing the validators passphrase.
...
== Next steps: Relayer nodes and app-registry

Relayer nodes are required to send cross-chain messages from sidechain to mainchain and vice-versa.

To enable cross-chain communication between sidechain and mainchain, a relayer node is required in both networks.

=== Setup mainchain relayer node

Please check the guide xref:{url_run_relayer}[] for a step-by-step explanation how to turn a mainchain node into a relayer.

=== Move LSK to sidechain

NOTE: This step is not necessary, if the default token for CCM fees (LSK) is changed to a native sidechain token.

To be able to pay the CCM transaction fees for CCM from sidechain to mainchain, it is necessary to move a sufficient amount of LSK tokens from mainchain to sidechain via a cross-chain transfer transaction.
...
=== Setup sidechain relayer node

Please check the guide xref:{url_run_relayer}[] for a step-by-step explanation how to turn a sidechain node into a relayer.

=== Register off-chain data in the app-registry

As a last step, it is recommended to register the sidechain related off-chain metadata to the {url_github_appregistry}[app registry^].
...
`app.json`::
Off-chain metadata of a sidechain, to be used by user frontends like Lisk Wallet.
