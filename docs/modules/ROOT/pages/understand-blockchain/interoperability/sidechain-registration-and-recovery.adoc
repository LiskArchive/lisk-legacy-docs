...
Any user can submit a <<recover-message>> command to recover several CCMs simultaneously.
...
An <<initialize-message-recovery>> command is responsible for setting the outbox root and the inbox size in the terminated outbox account.
...
TIP: All required information to recover messages can be obtained directly from the mainchain.
...
The LSK amount is returned to respective account address on the mainchain.
...
On the mainchain, users can recover...

* ... the balance of LSK they had on a terminated sidechain by submitting a transaction with a <<recover-state>> command.
* ... <<pending-ccms>> by submitting a transaction with a <<recover-message>>.
...
The users can recover the balance of any custom token they had on a terminated sidechain by submitting a transaction with a <<recover-state>> command.

.Recoverable modules
[NOTE]
====
State changes can only be recovered on a sidechain, if the module targeted by the CCM supports the recovery.
Modules expose a `recover` method if the recovery is supported, like for example the Token module, and the NFT module.
