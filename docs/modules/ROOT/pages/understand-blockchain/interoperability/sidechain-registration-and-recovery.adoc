...
As can be seen in the following schema below, this assumes that `trs` is a transaction with module ID `MODULE_ID_INTEROPERABILITY`, and the command ID `COMMAND_ID_STATE_RECOVERY_INITIALIZATION` is to be verified.
...
The command ID is `COMMAND_ID_STATE_RECOVERY` and the state recovery command schema can be seen below:
...
Finally, for processing a transaction `trs` with the module ID `MODULE_ID_INTEROPERABILITY`, and the command ID `COMMAND_ID_STATE_RECOVERY`, this implies the following logic as can be seen in the schema below:
...
==== Sidechain Terminated Message (where to put this?)
