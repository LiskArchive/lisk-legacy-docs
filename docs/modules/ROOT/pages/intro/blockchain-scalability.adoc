...
:url_imdea: https://networks.imdea.org/whatsnew/latest/
...
To summarise, Lisk running sidechains results in reducing the load on the mainchain, therefore increasing the overall scalability of the network.
...
For example, researchers from xref:{url_imdea}[IMDEA Networks] have just developed a new data structure known as xref:{url_setchain}[Setchain], which implements a grow-only set object whose elements are not ordered, unlike conventional blockchain operations, which can phenomenally increase scalability.
