...
Valid transactions trigger the corresponding command of a particular module which accepts this transaction type.
...
To be accepted by the blockchain application, the transactions must be transmitted in the expected format, including all the required properties of a transaction.
...
In case of a multisignature trasnaction, several accounts need to sign a transaction, before it is accepted by a node.
...
The following transaction types are included in every blockchain application by default.
