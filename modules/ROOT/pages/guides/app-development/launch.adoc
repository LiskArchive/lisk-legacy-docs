...
[NOTE]
====
This guide describes only the most important steps for releasing a blockchain application for a public Testnet.
...
To turn the Devnet into a public Testnet, the following thinngs should be considered:
...
* Create config for seed node
* Create public default config.json
* Connect another node with API enabled
...
Include the data of the new genesis delegates in the config(see xref:{url_guides_forging}[enable forging]).

[[config]]
=== Create a default configuration object

Update configuration that was passed to the node during the development to be prepared for the public Testnet.


Most of the configurations can remain the same as those already defined in the Devnet config options.
However, please note that there is one option that should be updated: **The seed node(s)**.

For example to add `1.2.3.4:5000` as a seed node, add an object with the 2 properties `ip` and `port` to the `network.seedPeers` list as displayed below:
