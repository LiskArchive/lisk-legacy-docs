...
:description: The Lisk Commander command reference lists all available commands of Lisk Commander as a technical reference. For each command this consists of all available options, a usage example and an example response.
...
This returns a randomly-generated mnemonic passphrase with its corresponding public/private key pair and Lisk address as shown in the code snippet below:
...
Gets the information about one or several accounts from the blockchain as shown in the code snippet below:
...
This shows the private account information for a given passphrase.
It displays the Lisk address, publickey and privatekey that belong to the entered passphrase as shown below:
...
Gets the block information from the blockchain as shown in the code snippet below:.
...
Prints the current configuration as shown in the code snippet below:
...
If the `api.nodes` are set to one or more multiple nodes, the Lisk Commander will ignore the `api.network` and will make all requests to the specified Lisk node.

When multiple nodes are specified, queries will always go to the first listed node.
The following nodes serve as a fallback, if a query to the first node was unsuccessful.
...
This displays the copyright notice as shown in the snippet below:
...
The 'gets delegate' information from the blockchain can be seen in the code snippet below:
...
The 'gets voters' information for given delegate(s) from the blockchain can be seen in the snippet below:
...
The 'gets votes' information for given delegate(s) from the blockchain can be seen in the code snippet below:
...
How to create a hash onion output to be used by a forger can be seen in the snippet below:
...
The command references can be seen in the snippet below:
...
This decrypts a previously encrypted message from a given sender public key for a known nonce using your secret passphrase as shown in the code snippet below:
...
This command uses the Lisk Elements passphrase module to encrypt a message you provide for a given public key using a randomly generated nonce.
In order to decrypt the encrypted message later the recipient will need your public key (to verify that the message came from you), including the nonce and the secret passphrase which matches the specified public key. This can be seen in the code snippet below:
...
Your specific passphrase is required here as shown in the code snippet below:
...
The public key, signature and message are required as shown in the code snippet below:
...
This command displays the Network identifier based on the payloadhash of the xref:{url_guides_config_genesis_block}[genesis block] of the network.
The Network identifier needs to be provided everytime a transaction is signed, which ensures that the transaction can only be broadcast in the dedicated network, as shown in the code snippet below:
...
Uses official Lisk seed nodes, if no other nodes are provided in the <<_config,config>>.

=== Forging

Update the forging status of a Lisk Core instance can be seen in the snippet below:
...
The 'get the network status' from a Lisk Core instance can be seen in the code snippet below:
...
In order to decrypt the secret passphrase later you will need both the IV and the password as shown in the code snippet below:
...
This command creates and signs a type 0 transaction, which will transfer a Lisk balance to a provided address if broadcast to the network as shown in the snippet below:
...
This creates a transaction which will register the account as a multisignature account if broadcast to the network, using the following arguments as shown in the snippet below:
...
This creates a transaction which will report misbehavior of delegate by providing 2 contradicting block headers as shown in the snippet below:
...
This creates a transaction which will unlock tokens voted for delegates and add them back to the sender balance, as shown in the snippet below:
...
The 'gets transaction' information from the blockchain can be seen in the snippet below:
...
Sign a transaction using your secret passphrase is shown in the snippet below:
...
This command verifies a transaction after being signed with the sign transaction command or create transaction command,  as shown in the code snippet below:
...
This displays the warranty notice as shown in the snippet below:
