...
Update the RPC configuration to enable the WebSocket API of Lisk Core.
To accomplish this, create a custom configuration file, for example by copying one of the default config files:
...
Add the following option to `custom-config.json`:
...
`--api-ws` enables the WebSocket API of the node, if not already enabled in the config.json file.

[source,bash]
----
lisk-core start --network mainnet --api-ws --config=/home/lisk/lisk-core/custom-config.json --overwrite-config
----
