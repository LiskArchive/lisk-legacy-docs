...
Connections in the https://github.com/LiskHQ/lips/blob/master/proposals/lip-0004.md[*Lisk P2P]* are established as _websockets_ [https://en.wikipedia.org/wiki/WebSocket[1]] using the _SocketCluster_ framework [https://socketcluster.io/#!/[2]]. Each node keeps a list of known peers and their IP addresses and initializes up to 20 outgoing connections to randomly selected peers, while accepting up to 100 incoming connections.
...
Nodes propagate newly received blocks in order to keep the network synchronized and to achieve https://docs.google.com/document/d/1MCIQVFWD9DXqIq4vt0u2ct8l5cEHtYy9bntvLpFnnho/edit#heading=h.blsa9bg5la9[consensus]. +
...
Transactions are propagated through the network in a way similar to blocks. Every 5 seconds, a bundle of 25 https://drive.google.com/a/lightcurve.io/open?id=1sCGxXm2HTaXjiezThr_LqAdpls6wPgh9R4YoLrFGas8[transaction IDs] are selected from the transaction pool and sent to all connected peers. Those peers check if they already have the corresponding transactions and can request any missing one from the node and include them in their transaction pool.

=== Transaction Pool

The transaction pool collects transactions that are waiting to be included in a block. When forging a new block, a delegate selects a set of transactions from the pool and includes them in the https://drive.google.com/a/lightcurve.io/open?id=1Eidrm7AMvp2Hq6SEXkZhuYiw9LimMUQmYDyZZ-LFZak[block payload]. Conversely, when a new block is received, transactions included in the block are removed from the transaction pool.
