...
Returns an account address in hexadecimal representation for a given Lisk32 address.
...
. `base32Address`(string): A Lisk32 address.
. `prefix`(string)(optional): Prefix for account addresses.
Default value: `lsk`.

==== Return value

`Buffer`: The account address in hexadecimal representation.
