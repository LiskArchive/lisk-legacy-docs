...
Therefore, when building a Sidechain application{fn_sidechain_glossary} (sApp) with the Lisk SDK, you can create custom cross-chain messages by defining the content of the `params` property as needed.
...
All additional parts of the sidechain protocol are defined by the sApp{fn_saap_glossary} developer.
...
To become a sidechain in the Lisk ecosystem, a sApp needs to perform the following two simple steps:
...
In this case we are going to use an example with two functioning applications.
As can be seen in diagram below the Muziki app is connected to the Lisk mainchain.
The Muzikie app is a decentralized music streaming platform and marketplace that enables users to stream and purchase music, concert tickets, and merchandise from a variety of artists.
To function as a sidechain, the Muzikie app also has its own set of applications and music smart contracts that manage music and its associated data.
Furthermore, also connected to the Lisk mainchain is the Enveti App, a decentralized social media platform that enables users to create and manage their own NFTs.

In order to connect the Muziki and Enevti sidechains, we can use a cross-chain communication protocol that allows them to exchange data and transactions.
For example, a user holding LSK tokens may decide to purchase an audio NFT on the Muzikie app, transfer it from the sidechain through the Lisk mainchain to the Enevti app, and then sell this NFT for ENVT tokens.
Finally, that same user could then sell/convert their ENVT tokens back to LSK tokens. Conversely, it is also possible for LSK tokens to be converted/sold to ENVT tokens.

So to perform this action, the user would first have to sell/convert their LSK tokens to MZK tokens, then select the NFT for sale.
This would be followed by initiating the transaction on the Muzikie app by specifying the amount they want to pay, and providing their Lisk wallet address as the recipient address.
Once the transaction is confirmed on the Muzikie sidechain, the NFT would then be transferred to the user’s Lisk wallet.

The user could then initiate a transfer of the purchased NFT from the sidechain to the Enevti app.
