...
const app = new Application(genesisBlockDevnet, { <3>
    app: {
        label: 'my-label',
        minVersion: '0.0.2',
        version: '2.3.4',
...
<2> Passes the config json `configDevnet`
<3> Sets the required values for label, protocol and version, overrides consoleLogLevel, uses default values for everything else
