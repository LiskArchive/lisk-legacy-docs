...
This page explains the three domains of a blockchain and provides an introduction to blocks and transactions.

== The three domains of a blockchain

From a high-level perspective, there are three domains of a blockchain which are explained below:
...
A detailed description covering the main properties of a block is provided in the following sections.

.Block JSON schema
[%collapsible]
====
Blocks are serialized and deserialized in accordance with the following JSON schema.
...
The most important properties of the block header are as follows:

* `timestamp`: The time the block was created as a Unix timestamp.
...
Besides the above listed properties, each block header contains various additional technical properties that are required for the blockchain application to function.
To view a description of the additional properties, expand the boxes shown below, and also see the JSON schema for an overview of all properties included in the header.
Check {url_lip55}[LIP 0055^] for additional information about the block header.

.Remaining properties of the block header
[%collapsible]
====
* `version`: The block header version must be equal to the value of a block of the previous protocol plus one.
...
This is important for cross-chain certification and included in certificates.
...
[%collapsible]
====
Block headers are serialized and deserialized in accordance with the following JSON schema.
...
NOTE: Inserting the assets root rather than the full assets allows the possibility to bound the size of the block header to a fixed size, while still authenticating the content of the block assets.
...
To be accepted by the blockchain application, the transactions must be transmitted in the expected format, including all the required properties of a transaction, and pass the transaction & command verification steps explained in the <<block-processing>> description.
