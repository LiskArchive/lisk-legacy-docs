...
:url_setup_docker: setup/docker.adoc.adoc
...
The xref:{url_setup_binary}[Binary installation] is an easy and automated way to set up Lisk Core, which includes almost completed automated update scripts; together with a selection of tools to assist with seamlessly maintaining a Lisk Node.
...
xref:{url_setup_docker}[Docker] adds support for additional platforms upon which to run a Lisk node.
...
An installation performed from xref:{url_setup_source}[Source] enables a developer to work on the newest codebase for Lisk Core, which may not have been tagged for a release yet.
