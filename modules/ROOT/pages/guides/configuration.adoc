...
To configure the application, pass a config object as parameter when creating a new `Application` instance.

During local development of the application it is recommended to import and use the `configDevnet` object from the Lisk SDK.
Together with the `genesisBlockDevnet` it sets up a local development environment with 101 forging genesis delegates.
...
<2> Optionally override certain options in `configDevnet`.
<3> Create the application instance and pass the `genesisBlockDevnet` as genesis block and `configDevnet` as config.

TIP: To view the pre-defined default configuration values, please see the xref:{url_reference_config}[list of configuration options].

[[config_options]]
== Configuration options

The default configuration options are located in:
...
TIP: Go to the xref:{url_reference_config}[configuration reference], to get a complete list of all available config options.
...
These constants allow to quickly adjust basic properties of the blockchain, for example time that elapses until a new block is forged, the network epoch time, or the block rewards.

In the alpha version of the Lisk SDK, not all available constants are configurable by the user, yet.
In future versions, more constants will become configurable.
...
The Lisk SDK exposes a object `genesisBlockDevnet` holds all important information to spin up a local devnet.
