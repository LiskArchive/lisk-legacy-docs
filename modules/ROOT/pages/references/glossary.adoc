...
Generally speaking, a fork can be classed into one of three categories, a soft, hard, or temporary fork. A soft fork can occur when the blockchain protocol is changed into a backward-compatible manner.
For example, a software upgrade that is backward-compatible with older versions would result in a soft fork taking place.
Alternatively, if the blockchain protocol is changed into a non backward-compatible manner.
For example, the release of a new software upgrade that is not compatible with the previous versions, which would require all users to upgrade to continue performing and validating new transactions, then a hard fork would occur.
