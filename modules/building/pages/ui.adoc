...
A more comprehensive description can be found in the documentation at {url-antora-docs}.

=== Prerequisites

To preview and bundle the default UI, you will need to have the following software installed on your computer:
...
Next, ensure that you have Node.js installed (which also provides npm).
...
To make Node.js 10 the default in all new terminals, type the following:
...
Verify the Gulp CLI is installed and in your PATH by running this command:
...
In preview mode, source maps are enabled automatically, so no other requirements are necessary to use them.
