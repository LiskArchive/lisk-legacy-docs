...
DPoS works in a similar fashion to PoS, however, one of the key differences is that it utilizes a **delegation and voting mechanism**, where users deploy their staked collateral to vote for *validators*.
Every user account can register as a validator by spending a certain amount of tokens.
A predefined number of validators with the most votes are allowed to add new blocks to the blockchain in turns.
A validator who has enough votes to be allowed to forge is called an **active validator**.
The block generation process is divided into **block generation rounds**.
A block generation round lasts until every active validator has generated exactly one block.
After each round, the list of active validators is calculated again based on their current number of votes.
To allow this mechanism to be both efficient and effective at performing transaction validations, various different components of DPoS exist.
In this system, generally users, or as they are known in PoS and DPoS, *validators* are voted in based on their reputation.

DPoS utilizes a unique election system that is able to select users that can perform block verification.
In this system, generally, validators are voted in based on their reputation or (financial) incentives that they offer to their voters.
Validators can also be thought of as witnesses or users producing blocks.
With DPoS, it is possible to vote on validators by entering your tokens into a staking pool and linking them to a specific validator.
In DPoS a limited number of validators exist (usually from around 20 to 100), and the validators are voted in by other users.
...
In addition, it is this limited number of validators that oversee the governance of a blockchain deploying the DPoS consensus mechanism.
Each user who holds a minimum of one token/coin with the DPoS blockchain has the ability to vote specifically for the validators that they want to perform the transaction validations.
Dependent on these votes, certain validators are allowed to add blocks to the blockchain in a specific order.
...
This is realized in DPoS, as there are only a limited number of validators, which enables a consensus to be reached much faster than PoW.
...
- An incentive to behave and conform to the rules/Security: Should any malicious activity on the network be discovered, the participants can vote to have the offending validator removed immediately, therefore providing a good incentive for validators to behave correctly, hence enhancing the security.
...
- Democratic system: With DPoS all validators are elected democratically resulting in each validator being able to have their say.

- Improved distribution of rewards; The whole process can be considered the most democratic method of voting, as it does not rest on the users that possess the highest amount of tokens, as just because a particular user has many tokens, that does not authorize this user to validate and confirm transactions.
All the users that maintain tokens in their accounts are able to select a group of validators to perform this task.
...
- Validators could create cartels: With DPoS this could be achieved by certain validators concentrating the role of validation between a small number of users, resulting in less resiliency and decentralization.
