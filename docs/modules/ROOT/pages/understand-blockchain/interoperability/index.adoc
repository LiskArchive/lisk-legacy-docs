...
Therefore, when building a xref:{url_sidechain_glossary}[Sidechain application] (sApp) with the Lisk SDK, you can create custom cross-chain messages by defining the content of the `params` property as needed.
...
* *sidechains*: Sidechain applications built with the Lisk SDK.
...
All additional parts of the sidechain protocol are defined by the xref:{url_sapp}[sApp] developer.
