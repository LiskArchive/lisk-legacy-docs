...
NOTE: When using a MAC with the Apple Silicon (M1 processor), please adhere to the relevant following setup instructions below:
...
rm -rf ~/.nvm
----
. Add a new session and set the iTerm to open in the **Intel** mode and then, set a shortcut key for opening iTerm in the aforementioned mode.
After that, open a terminal in Intel mode.
...
+
--
For Redis and Apple Silicon chips use the following command:
...
For Apple Silicon add the following lines to your `~/.zshrc`
...
==== Optional
. Check the mode in which the terminal is running.
+
[source,bash]
----
...
. Restart the terminal and ensure you are running it in the Intel mode.
+
[source,bash]
----
$ arch
...
=== Install Rosetta, Homebrew, and NVM
...
Now it is possible to add these binaries to your path in the `.zsh_profile`
...
=== Install Python on an M1 Mac
To install Python on an M1 Mac.
[source,bash]
----
brew install pyenv
