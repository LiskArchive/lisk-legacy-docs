...
When the first cross-chain update containing messages is sent to the sidechain, the parity of the properties in both the registration message and the the interoperability store are verified.
...
On the mainchain in the Lisk *Mainnet*, the own chain account is present by default, and set to an object with properties:
