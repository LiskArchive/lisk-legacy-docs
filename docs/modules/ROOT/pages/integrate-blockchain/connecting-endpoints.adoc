...
Apart from that, xref:{url_intro_modules}[Modules] and xref:{url_intro_plugins}[Plugins] of a bockchain client can also expose endpoints as described in the guides about xref:{url_endpoints_modules}[implementing endpoints for a module] and xref:{url_endpoints_plugins}[creating an endpoint for a plugin], respectively.
