...
With the latest version of Lisk Core, there will be new configurable settings and the following modules listed below in the following sub-section will also be used.
...
In figure 1 above, the elements in violet are created by Lisk Core v3, inclding the state snapshot.
The snapshot block element is shown in green, and is created by the migrator tool.
Finally, the elements in yellow are created by Lisk Core.
