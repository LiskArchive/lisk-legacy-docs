...
To ensure that our plugin is efficient, and doesn't look for events in the already checked blocks of the sidechain, we would like to store the height of the last block that was checked for a `newHello` event.
...
The `\_saveEventInfoToDB()` function will take the _decoded data_, the _block height_ where a `newHello` event is found, and an incremented _counter_ value and will store the aforementioned in their corresponding data structures.
