...
:url_kibana: https://www.elastic.co/kibana/
...
:url_log_files: configuration.adoc#_file_log_stream

[[log_monitoring]]
== Log Monitoring

The xref:{url_log_files}[log files][[Logging]] provide a chronological list of activities that happened on a node.
...
We use {url_newrelic}[New Relic] to monitor the activities inside of the application.
...
The first thing you need to do is register an account at {url_newrelic_rpm} if you have not already done that.
...
{url_apache_bench}[Apache Bench] is a generic benchmarking tool to measure the performance of HTTP servers.
...
To know fine-grained details of this dashboard, please read {url-newrelic-apm-intro}.
