...
For a better overview, create a new file `nft_token.js` in the `nft-module/` folder.

Now open `nft_token.js` and define the functions related to the NFT token, which will be used in the assets and the module in the next steps.
...
.nft/blockchain-app/nft-module/nft_token.js
[source,js]
----
...
class CreateNFTAsset extends BaseAsset {
  ...
  async apply({ asset, stateStore, reducerHandler, transaction }) {
    ...
    // 6.update sender account with unique nft id <3>
    senderAccount.nft.ownNFTs.push(nftToken.id);
    await stateStore.account.set(senderAddress, senderAccount);

    // 7.debit tokens from sender account to create nft token <4>
