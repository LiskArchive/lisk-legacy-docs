...
Any misbehavior of a delegate is indicated by contradicting block headers between two forged blocks of a particular delegate.
This can be reported by providing the two block headers which have contradicting values.

.Detecting Contradicting Block Headers (Source: {url_github_lip14}[LIP 14^])
[source,js]
----
