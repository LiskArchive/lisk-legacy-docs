...
To create the forging configuration data,
...
[source,bash]
----
$ lisk-core forging:config --output ./forging_config.json
...
[source,bash]
----
$ ./bin/run forging:config --output ./forging_config.json
? Please enter passphrase:  [hidden] # <1>
? Please re-enter passphrase:  [hidden]
? Please enter password:  [hidden] # <2>
? Please re-enter password:  [hidden]
...
.Example of forging_config.json
[%collapsible]
====
[source,js]
----
...
Merge the forging config with the custom config which was created in <<creating-a-custom-config-file,step 2.1>> above, to add the delegate information to the application configuration:

[source,bash]
----
TEMP_FILE=$( mktemp )
jq --slurp '.[0] * .[1]' custom-config.json custom-forging.json >$TEMP_FILE
mv $TEMP_FILE custom-config.json
----

Open `custom-config.json` to verify it was updated correctly.

Beside adding the delegate config data, the other options in the config can be adjusted as desired.
Find examples of config options that might be interesting to update right below.
