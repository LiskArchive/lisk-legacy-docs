...
All properties of a transaction object are described on the xref:protocol:transactions.adoc#transaction-properties[Lisk protocol > Transactions] page. The input data for the different <<module-asset, module assets>> is stored inside the <<transaction-asset>> of a transaction.
