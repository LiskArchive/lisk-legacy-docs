...
:url_references_commander_commands_asset: {docs_sdk}references/lisk-commander
...
For a complete overview of all available options of the `lisk generate:asset` command, see the xref:{url_references_commander_commands_asset}[Lisk Commander command reference].

In our example, we choose `createHello` as command name, and `hello`  as module name:
