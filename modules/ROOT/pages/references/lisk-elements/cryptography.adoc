...
:url_pbk_glossary: glossary.adoc#pbkdf2-password-based-key-derivation-function-2
...
This converts an Buffer to a hexadecimal string.
...
Returns an account address as Buffer for a given Lisk32 address.
...
`Buffer`: The account address as Buffer.
...
. `address`(Buffer): An account address as Buffer.
. `prefix`(string)(optional): Prefix for account addresses.
Default value: `lsk`.
