...
But before you start creating a store, you need to decide if the data is on-chain or off-chain data.
...
If it is not needed or desired to synchronize the data with other nodes, it should be stores in on off-chain store instead.
