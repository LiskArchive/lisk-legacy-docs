...
Next, we need to get the blacklist of words from the xref:{url_guides_module_config}[module config], because we will need it in the next stepp during the command verification.
...
Then, call the method at the bottom of the `init()` function of the module:
...
Now, the blacklist which is defined in the config is available in the command.
