= Transactions
:description: In this section of the Lisk protocol, user accounts forming the underlying database are covered.
...
:url_appendix_signature_scheme: appendix.adoc#signature-scheme
:url_consensus_bft: consensus-algorithm.adoc#lisk-bft
:url_consensus_bft_punishment: consensus-algorithm.adoc#punishment-of-lisk-bft-protocol-violations
:url_consensus_voting_and_weight: consensus-algorithm.adoc#delegates-voting-and-delegate-weight
:url_transactions_balance_transfer: transactions.adoc#balance-transfer
...
For example, in Lisk Mainnet, for a <<balance,balance transfer>> transaction with a size of 133 bytes, the minimum fee is 10^-5^ LSK/byte × 133 bytes = 0.00133 LSK.
...
A <<balance,balance transfer>> transaction must make the balance of the receiving account equal to or greater than 5 × 10^6^ (0.05 LSK in Lisk Mainnet).
