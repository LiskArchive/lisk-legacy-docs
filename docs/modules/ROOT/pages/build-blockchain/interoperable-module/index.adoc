...
Once the CCM was received and executed an the node, it is possible to request the reaction on sidechain 1 with the endpoint `hello_getReactions` which we implement in section xref:{url_build_executeccm_endoint}[Creating a new endpoint for getting reactions for a Hello message].

[source,bash]
----
./bin/run endpoint:invoke hello_getReactions '{"address":"lskdxdcwe42wpw4kzeceb6fpkcvrp3k25n62ur7bq"}
----
...
To completely reset the interop example, and start from fresh, there is a very handy command which will perform all necessary steps for you:
