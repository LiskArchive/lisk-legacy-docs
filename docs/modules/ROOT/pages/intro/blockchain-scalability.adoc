...
By deploying the *cross-chain certification* methodology, this results in an efficient and scalable solution of transferring information between chains.
In effect, by being able to run each application on a separate sidechain this results in increasing scalability.
More information can be found on cross-chain certification xref:{url_cross_chain}[here].

Secondly, its use of the DPoS consensus mechanism as explained in more detail on the xref:{url_blockchain}[What is blockchain] page.
