...
However, a staker needs to claim their rewards, that can be done via both Lisk Desktop and CLI.

=== Claiming via Desktop
To claim rewards via Lisk Desktop, a staker must login to their account on Lisk Desktop and perform the following steps to collect earned rewards:

. As soon as the staker earns a reward, they will be notified about it by Lisk Desktop.
The *Claim rewards* option is available in the pop-up notification and also in the Stakes page on the staker's account. 
+
image:run-blockchain/reward-sharing/claim-rewards-0.png["Claim reward Notification",300]
+
. You can click directly on the *Claim rewards* option available on the pop-up notification.
The pop-up notification will disappear after a few moments, so let's try claiming rewards via the Stakes screen.
Click on the *Stakes* button to continue the reward claim process.
+
...
+
. Lisk Desktop will display an *Transaction summary* to the staker. Click on the *Confirm* button to proceed.
+
...
. Once the transaction is executed and the block is finalized, the staker's account balance will increment as per the earned reward.
