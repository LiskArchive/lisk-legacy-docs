...
PM2 comes bundled with the Lisk Core Binary distribution and will be available after <<_update_path_environment_variable, Updating the PATH>>
environment].
For more information, how to manage your Node with PM2, go to the xref:administration/source.adoc[Source Administration] page.
...
As you can see from the usage guide, `input_file` and `from_version` are required. If you skip `to_version` argument changes in `config.json` will be applied up to the latest version of Lisk Core.
...
In case you want to create your own, just run the following script.
It will:
...
NOTE: Note, that this process may take a long time, depending on the size of your snapshot.
