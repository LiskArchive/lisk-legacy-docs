...
:page-previous: docker.html
:page-next-title: Application commands
:page-previous-title: Docker image setup
:v_sdk: v3
...
An installation performed from source code enables a developer to work on the latest codebase for Lisk Core, which may not have been tagged for a release yet.
...
+
--
Running PostgresSQL inside a Docker container will setup the correct version of PostgresSQL, and containerize it away from any existing versions you may have locally on your machine.
...
. Install NVM by following these {url_nvm_instructions}[official instructions].
...
This section covers how to install Lisk Core from Source code.
When completed, you will have a functioning node on the Lisk Network.
If it is required to upgrade your current Lisk Core installation, please see xref:{url_upgrade_source}[Update Source code].
...
You can view a xref:{url_environment_variables}[list of available environment variables] here.
...
If you are not running Lisk locally, you will need to follow the xref:{url_config_api}[API access] guide to enable access.
