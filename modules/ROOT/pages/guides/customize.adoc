...
Transactions are an essential part of blockchain applications that are created using the Lisk SDK, because they define the set of actions that can be performed by users in the network.
...
TIP: {url_github_base_transaction}[See the BaseTransaction on Github].
