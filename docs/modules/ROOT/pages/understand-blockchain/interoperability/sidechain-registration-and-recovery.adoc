...
Specifically, this occurs when the sidechain has been inactive for too long.
...
Once a sidechain is terminated in the ecosystem, the users of this sidechain cannot have any cross-chain interaction with it.
Therefore it will not be possible to send, or receive any (fungible or non-fungible) tokens, messages or custom information either from or to the sidechain.
...
* Liveness Condition (meld this into this paragraph)
...
It must contain a non-empty certificate which follows the schema defined in LIP 0061; furthermore, if this first CCU also contains a non-empty inboxUpdate, the certificate is only valid if it allows the sidechain account to remain live for a reasonable amount of time.
