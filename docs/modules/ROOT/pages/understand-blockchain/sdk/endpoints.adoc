...
It  provides an interface to *subscribe* to all events and to *invoke* endpoints of the blockchain application and its' modules & plugins.
...
If input data is required, it is provided as a second argument. All endpoints can be invoked via <<the-api-client>>. The following example shows the invocation of an endpoint with and without additional arguments.
