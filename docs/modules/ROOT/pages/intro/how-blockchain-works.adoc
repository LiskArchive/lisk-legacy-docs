...
Lisk uses what is known as an unstructured P2P network, whereby the nodes randomly connect to each other, and are all considered as equal participants in the network.
...
This increases the robustness and scalability, coupled with increasing security.
Lisk utilizes the unstructured partial network methodology, and the xref:{p2p_architecture}[Lisk P2P architecture] is covered in more detail in the *Lisk Elements* section.
