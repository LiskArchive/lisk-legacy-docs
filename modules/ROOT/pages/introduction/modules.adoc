...
* xref:{url_token_module}[]: xref:{url_protocol_accounts_transfer}[Token transfer]
...
* xref:{url_keys_module}[]: xref:{url_protocol_accounts_multisignature}[Multisignature group registration]

To add support for a new transaction to the application, it is required to implement a new <<assets,asset>> and to add it to a module.
