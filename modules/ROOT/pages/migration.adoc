...
:url_admin_binary_rebuild_from_genesis: administration/binary.adoc#rebuild_from_genesis
:url_configuration_forging: configuration.adoc#forging_enable_disable
:url_distributions: index.adoc#distributions
:url_setup_binary: setup/binary.adoc
:url_snapshots: index.adoc#_snapshots
:url_upgrade_vs_migration: index.adoc#upgrade_vs_migration

[IMPORTANT]
====
Migration of a Lisk Node is only necessary during a hard fork.
For normal software updates that don’t invoke a hard fork, please perform a normal upgrade, see xref:{url_upgrade_vs_migration}[Upgrade vs Migration].
====
...
If you don’t want to use the automated script, you can read a detailed explanatio of <<migrate_manually, how to migrate manually>> below.
...
We introduced a tool to perform the migration called *Lisk Bridge*, available at {url_lisk_downloads}[downloads.lisk.io].
...
It’s a wrapper script that invokes `installLisk.sh upgrade` at a specific block height - and thus is intended for users using the xref:{url_setup_binary}[Binary installation].
...
[IMPORTANT]
====
*Important note for delegates:* After automated upgrade, you still need to enable forging again manually, like described in xref:{url_configuration_forging}[configuration section].
====
...
Don’t panic! Counting from the migration height, you have 2 full forging rounds time to upgrade your node manually by following the steps described in <<migrate_manually, Migrate manually>>.
If 2 full forging rounds have already passed since migration, your Node will be probably on a fork after the upgrade.
To resolve this, rebuild your version of the blockchain xref:{url_snapshots}[from snaphot] or xref:{url_admin_binary_rebuild_from_genesis}[from genesis block].
...
. Go through the default xref:{url_upgrade_vs_migration}[upgrade process].
