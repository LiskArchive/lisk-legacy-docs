...
The hash onion can be generated with Lisk Commander the following way:
...
[CAUTION]
====
Do not activate forging for the same delegate on multiple nodes at rthe same time.
...
Invoke the following action to *disale the forging* for a delegate:
...
. Stop the old node.
. Display the forging information of the old node.
+
[source,js]
----
const info = await apiClient.invoke('app:getForgingStatus');
----
