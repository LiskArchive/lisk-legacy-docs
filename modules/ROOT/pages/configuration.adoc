...
:v_sdk: master
:url_admin_docker_config: administration/docker.adoc#config
...
:url_framework: {v_sdk}@lisk-sdk::lisk-framework/index.adoc

[IMPORTANT]
====
If you are using the Docker distribution, Lisk Core is configured in a slightly different manner.
Please go to the xref:{url_admin_docker_config}[Docker Administration] page to find out more about the docker-specific configuration of the Lisk Core.
====
...
To connect to another network, specify the `network` when starting the Lisk Core, as described in <<command_line_options,Source Administration>>.
