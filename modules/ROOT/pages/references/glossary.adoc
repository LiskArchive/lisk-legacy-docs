...
=== Account Address / Base32 Address Format

On user interfaces, an account address is represented in a user-friendly, human readable representation that uses a Base32 encoding and includes a checksum that detects typing errors. Account addresses are always 41 characters in length. They contain decimal digits and lower-case letters and finally start with a prefix.
In the case of the Lisk networks, this prefix is ‘lsk’. An example address in this format will look like the following: ‘lsk24cd35u4jdq8szo3pnsqe5dsxwrnazyqqqg5eu’.
For further details regarding other address representations this can be seen here in the different data structures xref:{url_data_structure}[table].
