...
If you are using a different distribution of Lisk Core 2.1.6, like Docker, Commander or Source, you need to xref:{url_core_2_install}[install the application/binary distribution].
...
After the lisk-migrator script has finished and the announced snapshot height has passed, there is no reason annymore to run Lisk Core 2.1.6 and it is recommended to stop it.
...
You can verify that the node is running correctly by running:
...
This will return some general node information like:
----
...
The `0 0 0` stand for the three variables `HEIGHT`, `MAXHEIGHTPREVIOUSLYFORGED`, `MAXHEIGHTPREVOTED`.
These three variables need to be set to `0` for every delegate who starts forging for the first time in the network, which is true for every delegate participating in the migration.

[TIP]

====
When a delegate has already forged in the network, they need to check which values to use for `HEIGHT`, `MAXHEIGHTPREVIOUSLYFORGED`, `MAXHEIGHTPREVOTED`.
