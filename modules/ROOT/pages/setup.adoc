...
For more information refer to the diagram on the xref:getting-started/interact-with-network.adoc[Interact with network] page.
====
...
[NOTE]
====
To install the required pre-requisites, it is necessary to have a user with sudo rights on the server.
The `lisk` user *does not need* any `sudo` rights to run the node.
====
...
To manage the start/stop process of the app in the background, install https://github.com/Unitech/pm2[PM2] as shown below:
...
To install the NPM package https://www.npmjs.com/package/lisk-sdk[lisk-sdk], run the following command:
