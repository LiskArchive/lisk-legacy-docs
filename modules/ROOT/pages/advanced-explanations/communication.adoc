...
The xref:{url_lisk_sdk}[Lisk SDK] offers two different RPC (Remote-Procedure-Call), API modes: `ipc` (Inter-process communication) and `ws` (WebSocket).
...
A blockchain application communicates via <<actions>> and <<events>> which can be invoked (actions) or subscribed to (events) via RPCs (Remote-Procedure-Calls).
