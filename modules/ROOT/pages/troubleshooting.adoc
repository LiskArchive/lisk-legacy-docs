...
If you want to enable forging for a particular delegate on your node, you need to store an object with the delegates’ public key and encrypted passphrase in that section as described in the xref:{url_config_forging}[configuration:forging] section.
--
====
...
Instead, the password is passed so you use it to encrypt your passphrase and the encrypted passphrase is stored in your xref:{url_config}[config].
* *Password* is always referring to the secret word/s you use to encrypt your passphrase symmetrically like described in the xref:{url_config_forging}[Forging section].
====

Should you have any further queries please reach out to one of the team or the Lisk community on {url_lisk_discord}[Lisk Discord]
