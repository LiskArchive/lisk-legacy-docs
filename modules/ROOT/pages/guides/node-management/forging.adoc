...
For this example, we reduce the value of the `count` to  4000, with a default distance of `1000` this will only produce 5 hashes in the hash onion.
Generally though, it is recommended to use high values for the `count`, so the delegate has enough hashes to forge for a long time.
...
For more information about the configuration of the xref:{url_lisk_sdk}[Lisk SDK] check out the xref:{url_guides_config}[configuration guide].
--
=====
