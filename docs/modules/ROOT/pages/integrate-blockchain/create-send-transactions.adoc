...
NOTE: When planning to run the same transaction through a sidechain client, the same CLI commands as in the general Lisk CLI. Just replace `lisk-core` with the application's CLI tool or use `./bin/run` building the application from the source code.
