...
The sidechain operates as a separate transactional chain alongside the mainchain and can be invaluable when large batch transactions are required to be processed, thus resulting in reducing the load.
...
DAG is a unique methodology which is built on the concept of an intertwined network of parallel nodes and blocks, whereby each block contains a transaction with more than one parent root.
...
To summarise, DAG’s fundamentally new design concept, has the ability to address the scalability issues specifically with regard to large networks requiring to transfer high volume data rates at speed without compromising security.