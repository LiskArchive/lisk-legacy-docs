...
It is then verified by the following block, which contains what is known as a 'hash' of the preceding block, which is used to authenticate the transaction. (Hashing will be explained in further detail later). In other words, the block of data containing the transaction (this is also known as block confirmation), will then be added to the distributed ledger, which completes the verification.
. *The block creation*:
