...
It is most likely, because the `validate()` or `apply()` function of the asset isn't implemented yet.
...
. it throws an error, if the hello message equals some illegal statement
...
If all tests pass, it verifies that the `validate()` function behaves exactly as expected.
...
As a reminder, the `apply()` function of the asseet `createHelloAsset` is shown below:
...
. it updates the hello message in the sender account with the specified hello string
. it increments the hello cpunter by +1
...
As the context is the same for every test,  it is convenient to prepare everything in or before the `beforeEach()` hook and directly call the `apply()` function with the context in each test.
...
<2> `CreateHelloAsset`: useed to creeate a valid test transaction with a hello asset.
...
. it publishes a new event for each applied hello transaction.
. it doesn't publish a new event for each applied other transaction (not hello).
...
. it sets the hello counter to zero, after the genesis block is applied
...
. it returns the absolute amount of sent hello transactions, when the action is invoked.
...
If the logic and the tests of the asset & module was implemented correctly, all tests should pass:
