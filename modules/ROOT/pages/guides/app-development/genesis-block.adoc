...
To replace the genesis block in the blockchain application, it is required to copy or move it to `~/.lisk/<YOUR_APP>/config/default`

 cp config/default/genesis_block.json ~/.lisk/<YOUR_APP>/config/default/genesis_block.json
...
Assuming no custom config is used, the following commands can be used to update the configuration (please update `<YOUR_APP>` to the corresponding application name).
 ...
 $ tmp=$(mktemp)
 $ jq '.forging.delegates = input' ~/.lisk/<YOUR_APP>/config/default/config.json config/default/forging_info.json > "$tmp" && mv "$tmp" ~/.lisk/<YOUR_APP>/config/default/config.json
 $ jq '.forging += input' ~/.lisk/<YOUR_APP>/config/default/config.json config/default/password.json > "$tmp" && mv "$tmp" ~/.lisk/<YOUR_APP>/config/default/config.json
