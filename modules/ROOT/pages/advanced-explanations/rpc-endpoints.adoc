...
:url_modules_sequence: references/lisk-framework/sequence-module.adoc
:url_modules_keys: references/lisk-framework/keys-module.adoc
...
The RPC endpoints page provides a reference of all available application RPC endpoits, a.k.a. <<application-actions, actions>> and <<application-events,events>>.

It is also mentioned where to find references for the <<module-and-plugin-specific-actions-events>> and gives short examples <<how-to-query-the-api,how to use the WebSocket and IPC channels>> with the API client.
[TIP]
====
...
For more information about the configuration of the RPC endpoints check out the xref:{url_guides_config}[configuration guide].
