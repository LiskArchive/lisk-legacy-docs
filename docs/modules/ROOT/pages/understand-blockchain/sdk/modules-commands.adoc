...
:url_understand_valid_invalid_txs: understand-blockchain/index.adoc##valid-vs-invalid-transactions
...
. an <<on-chain-stores,on-chain store, a.k.a module store>>
. an <<off-chain-store>>
...
A module can define one or multiple on-chain- or module stores, to store data in the blockchain that is part of the blockchain state.
...
IMPORTANT: Different to xref:{url_understand_rpc_events}[RPC events], Blockchain events are part of the on-chain logic: Each block includes the **event root in the block header**, which is the root of a Sparse-Merkle-Tree of all blockchain events, that are occured in that particular block.
...
The only mandatory property to define in the event is the `schema`, which defines what custom data will be emitted by the event.
...
`topics` are the second property, that can be defined by the developer, when adding/ publishing a blockhain event.
Topics are similar to tags and labels, categorizing the event with a list of the relevant keywords.
...
You can add utility functions like `log()` and `error()` to omit duplicate code and/or to simplify the publishing of the event later.
