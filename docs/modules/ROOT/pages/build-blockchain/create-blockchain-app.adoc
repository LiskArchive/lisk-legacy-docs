...
NOTE: The `init` command can be executed anywhere on the disk to bootstrap a blockchain application, however, to accomodate both *frontend* and the *blockchain application*, we have to create the aforementioned folders.
...
Alternatively, you can also run the application without the default modules by,
replacing 
