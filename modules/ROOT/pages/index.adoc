...
:description: The Lisk Core overview provides an introduction about Lisk Core and the requirements necessary to run a node in the Lisk network.
...
+
--
Use xref:{url_setup_commander}[Lisk Commander application] to conveniently install and manage Lisk Core.
--
Docker image::
+
--
xref:{url_setup_docker}[Docker image] adds support for running a Lisk node with other platforms.
For example, running a Lisk node inside of a Docker image in Windows, and then connecting it via a custom node on Lisk Desktop to Lisk Core, without the need to rent an additional server.
--
Source code::
+
--
...
The installation from the xref:{url_setup_source}[Source code] enables a developer to work on the latest codebase for the Lisk Core, which may not yet have been tagged for a release.
--
====
...
* xref:{url_upgrade_binary}[Update application]
* xref:{url_upgrade_commander}[Update Commander application]
* xref:{url_upgrade_docker}[Update Docker image]
* xref:{url_upgrade_source}[Update Lisk source code]
