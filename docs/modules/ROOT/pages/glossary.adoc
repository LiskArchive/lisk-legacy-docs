...
A module can be defined as an individual component.
The contents of a module contain blockchain-related business logic, and are able to perform state changes on the related blockchain.
For example, the Lisk SDK modules can define state machine hooks, block generation hooks, and commands to add the related logic to the state machine.
