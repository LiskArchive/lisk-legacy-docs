...
Then clone the {url_github_sdk_examples}[lisk-sdk-examples on GitHub] and navigate into `hello_world` folder.
...
You should now see a basic frontend for the Hello world application, that allows users to:
...
This is ok for proof of concept application with not much data.
For production networks, implement a solution that scales with a big amount of data.
====
