...
[NOTE]
====
If the existing data needs to encoded/ decoded before posting, check the guide xref:{url_guides_decoding}[]
====
...
IMPORTANT: xref:{url_service}[Lisk Service] needs to be installed in oder to post a transaction via the Lisk Service HTTP API.
...
IMPORTANT: The xref:{url_framework_httpapi}[] needs to be enabled on the node in oder to post a transaction via the node HTTP API.

cURL is one of the tools that can be used to send HTTP API requests to Lisk Core:
