...
Each validator generating a block has to set the hash onion value.
For every block that a validator generates, a hash onion is consumed.
...
. Create a bash script file, for example, `generateHashOnion.sh`.
+
.generateHashOnion.sh
[source,bash]
----
# REPLACE ADDRESS (MANDATORY)
# REPLACE HASHCOUNT (Optional, cover 5 years at 200000)

ADDRESS="lskcrm3k2t4te2umb6djnbofohzvtmfgbz6h25d4y"
HASHCOUNT=200000
...
----
+
. Add the following command to create a hash onion file.
The file will be created in the path mentioned in the `JSONFILE` variable.
+
.generateHashOnion.sh
[source,bash]
----
lisk-core hash-onion --count "$HASHCOUNT" --output "$JSONFILE"
----
+
. Next, add the `ADDRESS` to the hash onion file:
+
.generateHashOnion.sh
[source,bash]
----
...
[source,bash]
----
lisk-core endpoint:invoke random_setHashOnion --file "$JSONFILE"
----
+
. Once the hash onion is set, the script should retrieve the recently set hash onion and its usage stats by invoking the following endpoints:
+
.generateHashOnion.sh
[source,bash]
----
lisk-core endpoint:invoke random_getHashOnionSeeds "{\"address\":\"$ADDRESS\"}"
lisk-core endpoint:invoke random_getHashOnionUsage "{\"address\":\"$ADDRESS\"}"
----
+
. In case you want to delete the recently created file, add the following line to the script as well.
.generateHashOnion.sh
+
[source,bash]
----
