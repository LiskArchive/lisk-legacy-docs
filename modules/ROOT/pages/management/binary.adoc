= Lisk Core Binary Command reference
Mona Bärenfänger <mona@lightcurve.io>
:description: The Lisk Core Binary Administration describes all relevant commands to manage Lisk Core through the Command-line.
:toc:
...
If Lisk Core has been installed with the Binary package and you wish to use additional tools such as `pm2` and `psql`, the corresponding PATH variables need to be set by typing the following command:
...
PM2 is bundled with the Lisk Core Binary distribution and will be available after <<_update_path_environment_variable, Updating the PATH>>.
environment].
...
The interactive terminal for postgreSQL is bundled with the Lisk Core Binary distribution and will be available after <<_update_path_environment_variable, Updating the PATH environment>>.
...
Lisk provides official snapshots of the blockchain, which can be seen here: http://snapshots.lisk.io.
