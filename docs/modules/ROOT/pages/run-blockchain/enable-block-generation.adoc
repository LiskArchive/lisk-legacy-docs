...
. If you are trying to export the hash onion seed after invoking the `random_getHashOnionSeeds` endpoint, copy the response shown in the link in the previous point no.5 above, and store it securely, so if necessary it can be used to import hash onions on a different node.
. Stop the old node.
...
. Ensure the node is fully synchronized with the network. This process ensures that the new node is up-to-date with the latest blocks and transactions in the network.
Hence, the height of your node should be equal to the current network height.
+
[source,bash]
----
lisk-core node:info
