...
In case `lisk-cryptography` is used server-side, it is beneficial to speed up the application by using the {url_github_sodium}[sodium-native] library.
...
* `byteLength`: The integer to convert to a buffer.
...
[source,js]
----
encryptPassphraseWithPassword(plainText, password, [iterations])
----
