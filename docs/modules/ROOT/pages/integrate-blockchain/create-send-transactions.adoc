...
This guide describes different ways of creating and sending transactions to Lisk nodes.

== Transaction formats

A transaction can be represented in different formats.

Below is an overview of all existing transaction formats and their use case.

=== Buffer

The representation of the transaction binary data as Buffer.
...
It is sometimes used as input or output format in the CLI to when sending or creating transactions via the CLI (see xref:{url_sdk_cli}[] and xref:{url_core_cli}[]).
...
Used as input / output format for functions in xref:{url_references_elements}[], for example the xref:{url_references_apiclient}[].
...
Used for example for displaying the details of a transaction in the UI to the user.
...
[NOTE]
====
The properties of a transaction are described in detail in the Lisk Protocol at the xref:{url_protocol_txs}[Transactions] page.
====
...
[TIP]
====
In case it is desired to have the transaction in a different format before sending, there are functions to convert the transaction between the different formats conveniently, see xref:{url_integrate_decoding}[] for more information.
====
...
Any running node with enabled API can be used to send transaction object, see xref:{url_sdk_cli}[] and xref:{url_core_cli}[].
...
If the xref:{url_sdk_httpapi}[] is enabled on a node, it is possible to send the transaction to the node via HTTP request:

[source,bash]
----
curl -X 'POST' \
  'http://raw.githubusercontent.com/api/transactions' \
