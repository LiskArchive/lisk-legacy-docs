...
The command `generate:asset` previously created the asset `createHello` which already contains skeletons of all important components of an asset.
...
NOTE: The minimum and maximum values for the different properties which are defined in the schema do not need to be validated again in the `validate()` function.

In this example, we want to validate that it is not possible to create a hello message with some illegal statement.

If any account sends a `createHello` transaction, with `asset.name` equal to `Mewtwo`, it will throw the error `Illegal NFT name: Mewtwo`
