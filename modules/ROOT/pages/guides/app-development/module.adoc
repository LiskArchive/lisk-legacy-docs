...
As example, we use the module name `myModule` and the module ID `1000`:
...
The file `nft_module.ts` contains the module skeleton.
The file `nft_module.spec.ts` contains the related unit teest for the new module.

Let's first take a look at the module skeleton in `nft_module.ts`:
...
Let's now takee a look at the different parts of the module skeleton:
