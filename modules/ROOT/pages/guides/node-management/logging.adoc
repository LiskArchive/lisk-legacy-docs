...
By default, the generated log files are saved inside of the `logs` folder of the Lisk SDK.
...
Now navigate into the logrotate config directory and create a new logrotate file for the Lisk SDK as shown below:
...
After customizing and saving the config to suit your requirements, it is recommended to test it by performing a dry run using the following command listed below:
