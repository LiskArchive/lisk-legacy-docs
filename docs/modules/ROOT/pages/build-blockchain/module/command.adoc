...
The blacklist can be retrieved from the module config, which was defined in guide xref:{url_guides_module_config}[].
...
Right now, there is only a way to post the hello message.
Also the counter is created and incremented, but there is no way for an external service to request the data.
...
=== Verifying the command verification

For this, create a transaction, similar to how it is done in the previous section <<try-the-new-command-out>>.
But in this case, we want the transaction to be invalid, to verify it is using the values defined in the custom module config, that we created in guide xref:{url_guides_module_config}[].
