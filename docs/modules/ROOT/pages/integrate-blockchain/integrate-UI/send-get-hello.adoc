...
Whilst building xref:{url_helloClient_overview}[Hello Client], we created `HelloModule` to send hello messages on the sidechain. 
...
TIP: The relevant files discussed in this guide are {url_sendHello}[sendHello.js], {url_getHello}[getHello.js], and {url_messsageTimeline}[messageTimeline.js].
...
To do the aforementioned process, let's update the `sendHello.js` file in the *hello_frontend/src/components/* directory.
...
It will take in the information such as a passphrase, hello message, fee, etc., and will create a transaction using `apiClient`.

Upon clicking on the *Submit* button, the *send hello* transaction will be created and then it will be posted to the node.
...
The UI is now able to send hello messages on the sidechain, however, we want the UI to display a response to the user as well.
...
Everything is in place now, let's test the *Get Hello* page. 
