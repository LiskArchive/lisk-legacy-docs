...
A blockchain application communicates to the blockchain by requesting the RPC endpoints of a blockchain node.
...
The three modes of communication i.e. IPC, WS, and HTTP allow blockchain applications to communicate with Lisk nodes.
...
A blockchain application communicates via interfaces i.e. <<endpoints>> and <<events>>.
...
== Events
The Lisk apps emit RPC events and Blockchain events, which each follow very different concepts.
