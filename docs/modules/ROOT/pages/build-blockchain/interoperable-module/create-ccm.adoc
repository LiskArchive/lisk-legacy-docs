...
Export the types and interfaces as shown in the example below:
...
Go to the file `react/commands/react_command.ts` and import the constants, schemas, and types defined above.
...
Update the methods to expect the `interoperabilityMethod` as an argument, and assign it to the private property `_interoperabilityMethod` of the `reactCrossChain` command.
...
When a user posts a `reactCrossChain` transaction on a sidechain that registered the React module, a corresponding CCM is sent to the mainchain by a xref:{url_run_relayer}[relayer node], where it will be forwarded to the designated receiving sidechain.
