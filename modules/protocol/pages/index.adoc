...
_This section contains a high-level overview of the Lisk protocol, including the set of rules followed by a blockchain created with the Lisk SDK using the default configuration.
...
However, a complete technical specification covering most parts of the protocol is provided in the respective https://github.com/LiskHQ/lips[Lisk Improvement Proposals]._
...
In an [#index-account_based-1]#*account-based*# blockchain, the validity of a new transaction is checked against a database of user accounts. An account is a portion of the database identified by the account [#index-address-1]#*address*#, as opposed to [#index-UTXO_based-1]#*UTXO-based*# blockchains (such as Bitcoin, see https://en.wikipedia.org/wiki/Bitcoin[Wikipedia page]), where there is no concept of an account and the validity of a transaction is based on a database of past referenced transactions.
The cost of processing the transaction and updating the blockchain is specified in the transaction fee and is paid using [#index-tokens-1]#*tokens*#.
...
The validity of a block (and all transactions within it), is determined against the state of the account.
...
The Lisk protocol defines an account-based blockchain with a set of default transactions that is used to modify the accounts state.
...
Therefore, every account property except the address and every transaction of the Lisk protocol belong to one of these four respective modules.
...
. xref:{url_consensus}[*Consensus algorithm*]: This section covers how the delegates that are eligible to create blocks are selected and how they reach an agreement on new blocks.
