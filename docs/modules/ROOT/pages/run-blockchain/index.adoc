...
In the Lisk ecosystem, a node can be of the following four types.
...
=== Relayer nodes
In the Lisk ecosystem, nodes from different chains can communicate with each other as well and this communication is done via relayer nodes.
...
Consider blockchain A with 20 nodes, out of which, a node is configured to act as a relayer node.
The job of this relayer node is to collect CCUs & CCMs from other nodes of blockchain A and then pass them on to the receiving blockchain B.
Once configured, a relayer node can directly connect with the RPC and validator nodes of the receiving chains.
