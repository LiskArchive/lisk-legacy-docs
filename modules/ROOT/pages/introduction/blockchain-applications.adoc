...
* ...modules, check out the xref:{url_introduction_module}[] introduction.
...
A typical network of blockchain application might look like this:

image:intro/node-network.png[]

.Shared on-chain logic, individual off-chain logic
****
It is important to note, that all nodes in the network need to share the same on-chain logic, while the off-chain logic of the application may differ from node to node.
****
...
[loweralpha]
. Use your favorite framework/ language and communicate to the node via the <<communication-interfaces>>.
An example is the frontend for the {url_github_srs}[SRS example appllication^] , which has been developed with React.js.
. Create a new xref:{url_introduction_plugin}[plugin] for the blockchain application.
An example implementation of a frontend as plugin is the {url_github_lns_dashboard}[LNS dashboard plugin] for the example application Lisk Name Service.


.a) Standalone UI
image:intro/standalone-ui.png[]

.b) UI plugin
image:intro/plugin-ui.png[]
