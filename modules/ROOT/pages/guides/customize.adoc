= Create a custom transaction object
Mona Bärenfänger <mona@lightcurve.io>
:description: The Lisk customization page explains how to create custom transaction objects and how to register them with the blockchain application.
:toc:
:v_sdk: v3.0.2
...
The Lisk SDK provides a <<interface,BaseTransaction interface>> from which developers can extend from, to create custom transaction objects.
...
Do not use these to register custom transaction objects.
====
...
The type is a unique identifier for your custom transaction object within your own blockchain application. This can be thought of as the hallmark of a transaction. Set this constant to any number, except 0-12, which are reserved for the default transactions.
...
It is also possible to extend from one of the default transactions or other custom transaction objectss, in order to extend or modify them.

In most cases though, this is not recommended because updates in the logic of the inherited transaction can break the logic of the custom transaction object.
...
Getters are used inside of the `applyAsset()` and `undoAsset()` functions of a custom transaction object.
...
Setters are used inside of the `applyAsset()` and `undoAsset()` functions of a custom transaction object.
...
While implementing a custom transaction object, it is necessary to complete some of these steps.
