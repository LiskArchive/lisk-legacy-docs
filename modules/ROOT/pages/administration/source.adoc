...
:toc:
:url-admin-binary: administration/binary.adoc#_create_snapshot
...
[TIP]
====
For creating xref:index.adoc#_snapshots[snapshots] in the most convenient manner, it is recommended to use Lisk Core from the xref:{url-admin-binary}[binary distribution].
