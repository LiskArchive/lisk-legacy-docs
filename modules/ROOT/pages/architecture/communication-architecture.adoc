...
The `Application` exposes an IPC socket that can be used by internal components like modules and plugins, and by any external service such as other scripts in JS, a tool in Rust, or a Python daemon.
...
The API client is a Lisk Elements NPM package that can be imported into any JS client application.
...
TIP: More information regarding the API Client can be found on the xref:{url_rpc_endpoints}[] page.

=== How to invoke actions

xref:{url_rpc_actions}[Actions] are invoked based on their alias.
If an input is required, it is provided as a second argument.

.Examples: How to invoke different kinds of actions with the API client
[source,typescript]
----
...
<4> How to invoke an action that requires some input.
<5> Example of how to invoke an action of the monitor plugin.
...
TIP: More information regarding available events and actions can be found on the xref:{url_rpc_endpoints}[] page.
