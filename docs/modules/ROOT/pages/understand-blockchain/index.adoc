...
To serve this purpose, Lisk protocol accepts a configuration that is part of the state machine.
...
The chain stores the transaction data into a data store.

A chain is also responsible for fetching blockchain's data from the data stores.


===== P2P
The P2P domian handles sending and receiving data from nodes. 
...
===== Data Stores
Each blockchain node keeps data on various data stores inside their Lisk protocol instance. This data is of two types: on-chain and off-chain.

* *On-chain* data includes but not limited to state data of the chain, account balance, nonce, multi-signature keys, generators' information, and the sparse merkel tree, etc.
The blocks, transactions, events and assets are also part of the on-chain data among various other properties.

* *Off-chain* data includes but not limited to node information, peer list, random hash, etc. 
It also contains information for generators i.e. last generated block, encrypted keys, etc.
Lisk also maintain off-chain data about the legacy chains.
legacy data consists of blocks from depriciated versions of the protocol.
