...
Certain data is stored as a `Buffer` in the database, however, in other parts of the application the data is expected to be in the format of a hexadecimal string.

In these cases, it is necessary to convert between the `Buffer` and the `String` as described below:
