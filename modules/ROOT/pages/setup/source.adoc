...
:page-next-title: Source commands
...
WARNING: If you have other versions of PostgreSQL installed on your machine, ensure to stop them before starting the Docker container.
...
If all of the above has been followed correctly, this should now be enough to set up the database ready to use with Lisk Core.
...
In case it is required to access Postgres inside the container via the CLI, execute the following command:
...
Install {url_pm2}[PM2] for managing start/stop of the app process in the background by executing the following command:
...
Due to this in order to run Lisk, there are two options available which are shown below:
====
...
The result `PONG` should now be visible.
--
====
...
If it is required to upgrade your current Lisk Core installation, please see xref:{url_upgrade_source}[Update from Source].
...
You are now ready to move on to the xref:{url_core_config}[Configuration] documentation if you wish to configure your node further, e.g. to enable forging.
