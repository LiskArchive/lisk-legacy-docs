...
Lisk is currently well-placed in this regard, firstly by its interoperability solution that allows each applicatino to run on its own sidechain

its use of the DPoS consensus mechanism as explained in more detail on the xref:{url_blockchain}[What is blockchain] page.
