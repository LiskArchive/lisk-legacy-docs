...
TIP: It is recommended to use Postgres with Docker image for a straight forward and fast setup of Postgres.
...
To successfully perform the command shown below, install Docker as described in the Setup page of xref:{url_docker_setup][Lisk Core Docker image distribution].
...
To manage the Docker image container, use the following commands displayed below:
...
If you wish to upgrade your current Lisk Core installation, please see xref:{url_upgrade_source}[Upgrade from Source code].
