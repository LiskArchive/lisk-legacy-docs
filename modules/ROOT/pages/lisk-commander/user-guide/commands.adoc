...
<1> If false, this displays this output in table format.
<2> These are the custom nodes which will connect to the lisk-commander.
<3> Thi is for the network in use, do Main for Mainnet, Test for Testnet.
...
When the `api.nodes` are empty, the lisk-commander will connect to the official Lisk Seed Nodes depending on the network specified in the `api.network`.

If the `api.nodes` are set to one or multiple nodes, the lisk commander will ignore the `api.network`, and will perform all requests to the specified Lisk node.
...
OPTIONS
  -j, --[no-]json  Prints output in JSON format. It is possible to change the default behaviour in your config.json file.
...
These following commands are related to Lisk delegates.

=== Get Delegate

Gets the delegate information from the blockchain:
...
ARGUMENTS
  USERNAMES  Comma-separated username(s) to acquire the information.
...
This gets the voter's information for given delegate(s) from the blockchain:
...
This gets the voteing information for the given delegate(s) from the blockchain:
...
This displays the command reference:
...
These commands are related to user messages:
...
OPTIONS
  ...
  --[no-]pretty
      Prints JSON in pretty format rather than condensed. This has no effect if the output is set to table. It is possible to change the
      default behaviour in your config.json file.
...
OPTIONS
  -j, --[no-]json
      Prints output in JSON format. It is possible to change the default behaviour in your config.json file.
...
This signs a message using your secret passphrase.

The following commands below are used to sign a message.
Your original passphrase will be required here.
...
This command below verifies a message after being signed with the sign message command.
The public key, signature and message will be required:
...
OPTIONS
  -j, --[no-]json
      Prints output in JSON format. It is posssible to change the default behaviour in your config.json file.
...
This command is related to the Lisk nodes.

This uses the official Lisk Seed Nodes, if no other nodes are provided in the <<_config,config>>.

=== Forging

This updates the forging status of a node:
...
This gets information about a node:
...
This command is related to the Lisk passphrases.

=== Decrypt Passphrase

This decrypts your secret passphrase using the password which was provided at the time of encryption.
...
This encrypts your secret passphrase under a password.

This command below uses AES-256-CBC to encrypt your secret passphrase under a password you provide using a randomly generated initialisation vector (IV).
In order to decrypt the secret passphrase later both the IV and the password will be required:
...
This command relates to signatures for Lisk transactions from multisignature accounts.

=== Broadcast Signature

This broadcasts a signature for a transaction from a multisignature account.

The following command below broadcasts a signature to the network, and takes one of the following required parameters:
...
Accepts a stringified JSON transaction as an argument as shown below:
...
This command relates to the Lisk transactions.

=== Broadcast Transaction

The following command broadcasts a transaction to the network via the node specified in the current config.

This accepts a stringified JSON transaction as an argument, or a transaction which can be piped from a previous command.

If piping ensure the entire command chain is quoted out to avoid piping-related conflicts in your shell:
...
The following command below creates a transaction object:
...
This creates a transaction which will transfer the specified amount to an address if broadcast to the network.

The following command below creates and signs a type 0 transaction, which will transfer a Lisk balance to a provided address if broadcast to the network:
...
This creates a transaction which will register a second passphrase for the account if broadcast to the network.

The following command below creates and signs a type 1 transaction, which will register a second passphrase for the account if broadcast to the network:
...
This creates a transaction which will register the account as a delegate candidate if it broadcasts to the network.
...
This creates a transaction which will cast votes, (and/or unvotes) for delegate candidates using their public keys if broadcast to the network.

The following command below creates and signs a type 3 transaction, which will cast votes or unvotes for delegates if broadcast to the network.
This command requires at least one of the –votes and/or –unvotes options.
...
This creates a transaction which will register the account as a multisignature account if broadcast to the network, using the following arguments:
...
This gets the transaction information from the blockchain:
...
This command below signs a transaction using your secret passphrase:
...
This verifies a transaction has a valid signature.

The following command below verifies a transaction after being signed with the sign transaction command or create transaction command.
...
The following command below displays the warranty notice:
...
OPTIONS
  ...
  --[no-]pretty    Prints JSON in pretty format rather than condensed. This as no effect if the output is set to table. It is possible to change the default behaviour in your config.json file.
