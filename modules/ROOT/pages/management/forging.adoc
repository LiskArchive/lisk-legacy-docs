...
Finally, as also mentioned in the Forger info data link above, the values used are global and not local on your machine, therefore as long as the node is synched then the value given by Lisk Core is universal.
// It is not recommended to use the values from the lisk blockchain explorer
