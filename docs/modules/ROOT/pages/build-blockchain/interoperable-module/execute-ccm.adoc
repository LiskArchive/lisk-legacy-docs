...
In this example, we store the reactions for a Hello message und the sender address of the Hello message, because it is a unique identifier for a Hello message.
...
To execute and test the implemented corss-chain command, please refer to section xref:{url_build_interop}[Testing interoperable modules].
