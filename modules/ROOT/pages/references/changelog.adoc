...
The registration of a multisignature account has significantly changed in SDK 4.0.0.

Besides all mandatory properties of a transaction (type, senderPublicKey, etc.), the registration transaction requires the following:
...
Once the account is registered as a multisignature account, every outgoing transaction requires for every public key a signature in `mandatoryKeys`.
The array may be empty.
. The `asset` property needs to contain the property `optionalKeys`.
The value of this property is an array of pairwise distinct public keys.
This array must be ordered lexicographically.
Once the account is registered as a multisignature account, every outgoing transaction requires signatures for some public keys in `optionalKeys`, (the number of required signatures depends on the `numberOfSignatures` property and may also be zero).
The array may be empty.
. The sets of mandatory and optional keys, specified by `mandatoryKeys` and `optionalKeys`, must be disjointed.
their combination must have at least one element and at most 64 elements.
. The asset property needs to contain the `numberOfSignatures` property.
** The value must be an integer between 1 and the total number of public keys of the account, (the sum of the optional and mandatory keys).
** The value must not be less than the length of the value of `mandatoryKeys`.
** The value specifies how many signatures are necessary for every outgoing transaction from the registered multisignature account, where signatures for both the mandatory and optional keys are counted.
...
The signatures corresponding to `mandatoryKeys` are appended first, followed by those corresponding to `optionalKeys`, whereby the order of signatures is the same as the order of public keys in the respective array.
...
Hence, this results in this address having to be used as the `recipientID` for balance transfers to this multisignature account.
...
This is the case even if this key does not belong to either the set of mandatory or optional keys.
...
The items of this array are byte buffers of either a length of 0 or 64.

For Multisignature registration transaction, `signatures` also contains additional signatures corresponding to the public keys participating in the multisignature account.
Specifically, `signatures` is an array composed of a signature corresponding to the `senderPublicKey`, followed by signatures for the public keys in `mandatoryKeys` and `optionalKeys`, whereby the order of `signatures` is the same as the order of the public keys in the respective array.

For other transactions, `signatures` contains exactly one element.

The registration of a multisignature account is updated in the following manner as shown below:
...
TIP: For more detailed information about the new changes introduced, please refer to {url_github_lip17}[LIP 17: Make multisignature accounts more flexible, prevent spamming, and prevent signature mutability^].

=== Update: Vote transaction

The voting process now has certain changes which can be seen in the code snippet shown below:

For more detailed information about the new changes introduced, please refer to {url_github_lip23}[LIP 23: Introduce vote locking periods and new vote weight definition^].
...
In v4.0.0 xref:{url_bigint}[BigInt] is used to handle large numbers, such as account balances and transfer amounts.
...
The `applyAsset()` function of a custom transaction has become asynchronous, therefore it is now necessary to adjust the function in your implementation to something similar as shown below:
...
Due to this change, it is recommended to mutate the received account instance directly.
...
Static fees are replaced with dynamic fees. For more details please see the {url_github_lip13}[LIP 13: Replace static fee system by dynamic fee system^].
...
The specified fee must always be equal or higher to the defined minimum fee of the corresponding transaction type as shown below:
...
[TIP]
====
New accounts always start with a nonce equal to 0.
After sending the first outgoing transaction, which should therefore include the nonce = 0, it will then increment by +1 automatically.
====
...
For more information about the reasoning behind this change, please see {url_github_lip15}[LIP 15: Enable transaction invalidation by using nonces instead of timestamps^].

== Updated signature of `.sign()`: network identifier and passphrase

The network identifier, which had to be specified when creating the transaction object in SDK 3.x, is now specified in the `sign()` method of the created transaction instance as shown below:
...
NOTE: This type of potential attack becomes more relevant with SDK v4.0.0, as the dynamic fees allow much lower fees for nearly all default transactions, and therefore the transaction fees alone are not sufficient anymore to prevent this.
...
Due to the changes that have been introduced for the structure of transaction objects, the genesis block and it's transactions were newly created for Lisk SDK version 4.0.0
...
It is utilized to vote for the forging genesis delegates in a convenient manner during development.
...
This can be used to validate that the private key belongs to the owner, however it does not provide access to the owners private key. +
...
This is derived from 101 (max possible votes) - votesUsed (already used votes). +
**Example** : `8`|integer
|===
{set:cellbgcolor!}
...
__required__||object
|**blockId** +
__optional__|The ID of the block in which this transaction is included. +
**Length** : `1 - 20` +
...
By forging a new block on a chain, all former blocks and their contained transactions in the chain are confirmed by the forging delegate. +
...
**Example** : `"968ba2fa993ea9dc27ed740da0daf49eddd740dbd7cb1cb4fc5db3a20baf341b"`|string (publicKey)
|{set:cellbgcolor:green}**votes** +
__required__|All votes that the voter for the queried delegate voted for.|< <<_vote,Vote>> > array
|===
{set:cellbgcolor!}
