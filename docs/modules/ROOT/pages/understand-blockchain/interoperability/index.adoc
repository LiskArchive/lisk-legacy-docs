...
As shown by the image above, a Lisk blockchain can become interoperable with any other blockchain in the Lisk ecosystem including the Lisk Mainchain, simply by registering itself to the mainchain.
...
This means that any sidechain could be used as the mainchain, as long as it integrates the required interoperability modules.
