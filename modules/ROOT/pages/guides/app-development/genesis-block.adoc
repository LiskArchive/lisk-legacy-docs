...
[source,bash]
----
./bin/run genesis-block:create --output  /home/USERNAME/.lisk/<YOUR-APP>/config/default
----
...
After completion, the new genesis block will be copied to `config/default/`, whereby it automatically replaces the outdated genesis block.

After running the above command, the following 4 files should be generated:
...
Update the configuration file of the node where you'd like to enable forging for the genesis delegates of the network.
...
IMPORTANT: Keep the file `accounts.json` somewhere safe/private.
