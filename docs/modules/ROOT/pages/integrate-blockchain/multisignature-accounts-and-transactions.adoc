...
When using this script for several members, the `paramsStr` variable should be updated to the params that were returned from running `node mutisig.js PRIVATEKEY`.
This way all required signatures will be added to the parameters one after another.

When all required signatures are added, the parameters can be used to xref:{url_integrate_createtxs_apiclient}[create] a multi-signature registration transaction from the account that should be converted to multi-signature account.
