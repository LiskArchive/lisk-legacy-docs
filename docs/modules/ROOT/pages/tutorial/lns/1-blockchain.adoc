...
Open the file `storage.ts` which was previously created in the <<utility-functions>> step, and define a new function `updateLSNObject()` which updates a certain LNS object in the database, based on the provided parameters.
...
Now add the assets that were created previously in the <<creating-the-lns-module-assets>> step.
