...
The two most important roles are <<RPC enabled nodes>> and <<Validator nodes>>, illustrated in the diagram below:
...
This topic is covered in more detail on the xref:{url_enable_block_gen}[Enable block generation page].

If starting to enable block generation for the first time, it is highly recommended to first check out the dedicated guide: xref:{url_validator}[How to become a validator].
...
Staking can be defined as the process of holding and locking up a certain amount of LSK tokens to participate in validating transactions, in order to determine who secures the network.
