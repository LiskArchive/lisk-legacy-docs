...
:url_glossary_rpc: references/glossary.adoc#rpc-remote-procedure-call
...
<<reducers>> of modules can be invoked inside of the xref:{url_intro_modules_lifecycle}[lifecycle hooks] and xref:{url_intro_modules_assets}[Assets] of another modules via the `reducerHandler`.
...
<<events>>, <<actions>> and <<reducers>> are identified by their alias.
...
A blockchain application communicates via <<actions>>, <<reducers>> and <<events>> which can be invoked (actions, reducers) or subscribed to (events) via xref:{url_glossary_rpc}[Remote-Procedure-Calls (RPCs)].
...
Reducers are part of the module-internal request / response API, and are invoked via xref:{url_glossary_rpc}[Remote-Procedure-Calls (RPCs)].
...
* Modules
* xref:{url_intro_modules_assets}[Assets]
