...
Additionally delegates can be punished by anyone in the network, if they forge a block with consensus votes that contradict with the Lisk BFT (see the xref:{url_protocol_punishment}[protocol documentation] for details).
In order to avoid that a delegate is punished, a node operator has to take a lot of care when enabling forging on a node.
This means that they have to ensure the following:
...
* Never activate forging without porting over the `forger_info` data (see xref:{url_guides_forging_safely}[Savely enabling forging on a new node]).
