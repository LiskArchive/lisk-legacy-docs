...
<2> Here starts the first test, with a short and precise description of what the test is about.
...
----
If the function got called with the expected parameters, we know that the sender account was updated correctly.

In the second test, we want to verify that the transaction gets rejected, in case the sender has already a hello string in their account.
Therefore, we check if the expected `TransactionError` is returned when `applyAsset()` is executed.
