...
export const defaultConfig = {
	...
	// Modules hold a group of module-specific configs which is passed to a particular module.
	"modules": {},

	// Plugins hold a group of plugin-specific configs which is passed to a particular plugin.
	"plugins": {},
};
...
For more information, see xref:{url_run_key_path}[key derivation path] or {url_key_derivation_lip66}[LIP 0066: Introduce tree based key derivation and account recovery^].
...
=== modules
The `modules` holds module-specific configuration options of the blockchain client.

WARNING: The `module` configurations should only be changed by the blockchain developer, and not by the node operator!
If a node operator changes any of the module-specific configurations, it might cause the node to a fork.

Please check the available documentation for the respective module to learn about the available configuration options.

=== plugins
The `plugins` holds plugin-specific configuration options of the blockchain client.
...
==== generator
The `generator` configuration is used to read xref:{url_run_reys_create}[validator keys] from a file, which are then stored in the database.
...
| mandatory
|
| [{"startHeight": 123,"snapshotHeight": 123,"snapshotBlockID": "0f4ea1c3cfb61b99d387b26aaadf57936a528e5c713c6e55aa06f4d621b7e6f0"}]
