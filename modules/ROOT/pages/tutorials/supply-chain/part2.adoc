= Part 2: Track a packet on the blockchain
:description: This part of the Lisk supply chain tutorial describes the implementation, IoT application, & the client application for tracking a packet on the blockchain.
:toc: preamble
:imagesdir: ../../../assets/images
:experimental:
:url_github_transport_alarmtx_solution: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/transport/transactions/solutions/light-alarm.js
:url_github_transport_alarmtx31: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/transport/transactions/light-alarm.js#L44
:url_github_transport_index25: https://github.com/LiskHQ/lisk-sdk-examples/blob/v4/archive/3.x/transport/iot/light_alarm/index.js#L30
...
The goal of Part 2 is to implement a simple application that tracks sensor measurements on the blockchain.
Therefore once the IoT application is started, it will immediately send a `LightAlarmTransaction` to the network, whenever the sensor detects light.
...
It is also possible to 'require' the solutions directly in your application by just changing the following code snippet:
...
This application needs to be installed on different independent nodes, and will setup and maintain the blockchain which is used to store the data of the packets, the carrier, and the users.

A client application::
This will display information from the blockchain to the user.
A frontend is required which should display a list of packetIDs, consisting of the carrier, the sender, the recipient, and a status field, (`pending | ongoing | alarm | success | fail`).
...
[[step1_1]]
== 2.1 Implement the LightAlarm transaction
...
In this next step only the `validateAsset` function needs to be implemented.
...
**Implement your own logic for the `validateAsset()` function** {url_github_transport_alarmtx31}[here at line 44^].
...
Now **copy the code from your local computer** at `transport/transactions/light-alarm.js` (which was previously prepared in <<step1_1, step 2.1>>) to the Raspberry Pi.
...
Next, **insert the code snippet listed below** ,and save the `index.js` file.
...
// Check config file or visit localhost:4000/api/node/constants to verify your epoch time, (OK when using /transport/node/index.js).
...
In this part 2 of the workshop, only the `Initialize` and `Packet&Carrier` pages will be used.
====
