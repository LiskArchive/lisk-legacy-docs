...
However, IOTA is probably the most commonly known DAG. Their DAG named Tangle, completely removes miners from the verification process, as in order to broadcast each transaction, two previous transactions have to be validated first in order for the transaction to be processed.
This in turn increases decentralisation as all the users have to reach consensus.
