...
This section describes the two important parts of the Lisk protocol which together are referred to as the consensus algorithm.

The first part of this section describes the protocol for deciding who is allowed to add a new block to the blockchain.
...
A *delegate* is an account that performed a xref:{url_transactions_delegate}[delegate registration transaction].
...
Depending on the votes cast, every delegate has an associated [#index-delegate_weight]#*delegate weight*#, which is an indicator of the support for this delegate in the network.
...
where `selfVotes` are the number of votes that the delegate used to vote for themselves, and `totalVotes` are the sum of all vote amounts (including self-votes), for that specific delegate.
...
The computation of 101 active delegates, as well as the standby delegate selection, is based on the delegate weights from the two previous rounds.
...
This is to avoid the situation where a delegate who is not running a node leads to frequently missed block slots.
...
In the Lisk-BFT protocol delegates choose the chain where the block at the tip of the chain has the largest `maxHeightPrevoted` value (and the largest `height` value in case of a tie).
...
This means that both the delegate as well as the accounts voting for that delegate are punished by their tokens being locked for an extended period of time.
