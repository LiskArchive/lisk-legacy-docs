...
These servers have a <<node_app,node application>> installed that handles all communication between other nodes in the network.
...
When setting up a new node, each new node will initially connect to the seed nodes when bootstrapping for the first time.
