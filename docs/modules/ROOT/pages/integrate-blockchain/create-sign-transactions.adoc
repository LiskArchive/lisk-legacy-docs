...
The steps apply to all the applications developed with the Lisk SDK, including Lisk Core.
...
// [TIP]
// ====
// For more in depth information regarding enabling IPC, WebSocket, and HTTP APIs, please refer to the following page xref:{url_lisk_node}[Communicating to a Lisk node via RPC].
// ====
// TODO: Uncomment the above TIP when the docs beta version is released.

=== via CLI

The application CLI can be used to create a sendable transaction object to any running node. 
