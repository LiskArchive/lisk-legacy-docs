= P2P
:description: How to install, update and use the P2P library including, actions, events & some relevant examples.
:toc:
...
[source,bash]
----
npm install @liskhq/lisk-p2p@0.5.0
----
...
- `p2p.getDisconnectedPeers()`: get all the disconnected peers that are part of the network which are not connected to you.
...
Listen to various events on the network, in order to observe the network activities more closely and take appropriate actions if necessary, as shown below:
...
Check [examples](examples/) folder for a few examples to demonstrate P2P library usage with certain use cases.
