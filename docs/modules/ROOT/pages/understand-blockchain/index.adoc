...
Unlike Ethereum, where smart contracts have to share the same chain for their blockchain needs, Lisk provides the opportunity for each application to have its own sidechain.
...
However, the Genesis configuration and the configuration for the on-chain logic such as Modules must be the same across the network of a blockchain application.
