...
However, the password is sent in plaintext so it can be used to encrypt the passphrase which is stored in your xref:configuration.adoc[config].
* *Password* is always referring to the secret word(s) used to encrypt the passphrase symmetrically, as described in the xref:configuration.adoc#_forging[Forging section].
====

For any further queries please reach out to one of the team the Lisk community on https://lisk.chat/[Lisk Discord]
