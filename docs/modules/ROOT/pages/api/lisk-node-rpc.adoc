...
A reference of all RPC endpoints and events provided by a sidechain client through the Lisk Framework.
...
Checks, if the encoded `header` values passed to the endpoint contradict each other and return a corresponding boolean value.
...
==== `generator_setKeys`
Used for setting keys of a validator account. Request can have either a `plain` key object or an `encrypted` one.
...
==== `generator_setStatus`
Used for setting the status a generator.
