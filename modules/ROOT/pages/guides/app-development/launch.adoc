...
For example, there can be an additional application that connects a particular IoT (Internet-of-Things), device to the blockchain.
====
...
[IMPORTANT]
====
The current version of the xref:{url_lisk_sdk}[Lisk SDK] does not yet support the possibility to create its own xref:{url_protocol_genesis_block}[genesis blocks].
...
Add the code for the customized `node` application (including the transactions), to a public code repository.
...
* {url_github_hello_transaction}[transactions^]: A folder containing all required transactions.
