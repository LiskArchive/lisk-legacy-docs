...
This guide explains in a step-by-step process how to enable forging on a node as a delegate.
...
Must match the value in the `forger_info` data.
<7> Optional: If true, this overrides the `maxHeightPreviouslyForged` and `maxHeightPrevoted` values in the forger_info data.
