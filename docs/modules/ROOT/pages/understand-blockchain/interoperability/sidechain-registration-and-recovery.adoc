...
:url_bls: glossary.adoc#lisk-sdk
...
Therefore each new sidechain occupies a certain namespace in the ecosystem.
Additionally, every newly registered sidechain can increase the size of every cross-chain update command posted on the mainchain (due to the increasing size of the xref:{url_outbox_rootwitness}[outboxRootWitness] property of the command).
...
image:understand-blockchain/interop/chain-interoperability-store.png[ ,align="center" ,500]

.Figure 1: Overview example of the interoperability store.

With regard to the Lisk mainchain, the ‘own chain’ entry exists by default in the mainchain state, whereas there is one entry per registered sidechain for the other four stores created by a sidechain registration command.
With regard to sidechains, the ‘own chain’ and mainchain account entries are unique in the state tree and are created by a mainchain registration command.
...
* `genesisBlockid` - The ID of the genesis block ID  (add link - defined in LIP0034 ) is computed from the SHA-256 digest of the serialized bytes of the sidechain genesis block.
...
.. `keys`: An array of {url_bls}[BLS public keys^].
...
The mainchain account possesses a similar structure as depicted in the chain interoperability store above in figure 1.



==== mainchain validators

onwnchainID, own name, etc from lip 43
dd
