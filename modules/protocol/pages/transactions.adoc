...
|*delegate misbehavior report*|*DPoS*|Report any misbehavior of a delegate
|===
...
This is an integer that accounts for the number of transactions from the sender account.
...
This is the fee to be spent by the transaction. This fee has to be equal to or greater than a minimum value for the transaction to be valid. The minimum value is calculated as the size of the transaction object multiplied by a constant, `minFeePerByte`, given by the protocol. The value of `minFeePerByte` is 1000 (10^-5^ LSK/byte in the Lisk Mainnet). Note that for *delegate registration* transactions there is an extra summand of 10^9^ (10 LSK in the Lisk Mainnet), added to the minimum fee to account for the name space used by this transaction.

For example, in the Lisk Mainnet, for a <<transfer,token transfer>> transaction with a size of 133 bytes, the minimum fee is 10^-5^ LSK/byte × 133 bytes = 0.00133 LSK.
...
This is the public key of the account issuing the transaction.
Note that this public key xref:{url_accounts_keys}[does not necessarily sign the transaction].
For simplicity, the account issuing the transaction is called the sender account in the SDK documentation.
...
This is an array with the signatures of the transaction.
If the account associated to the `senderPublicKey` does not have the xref:{url_accounts_keys}[`keys`] property, the object containing the six properties, `moduleID`, `assetID`, `nonce`, `fee`, `senderPublicKey`, and `asset`, has to be signed by the private key corresponding to the `senderPublicKey`.
...
Further details regarding the signing process are provided in the xref:{url_appendix_signature_scheme}[Appendix] section.
...
A <<transfer,token transfer>> transaction must make the balance of the receiving account equal to or greater than 5 × 10^6^ (0.05 LSK in the Lisk Mainnet).
...
Each one induces a specific operation on accounts and is defined by a unique schema and transaction logic.
The specific data is given in the `asset` property previously mentioned.
The key points and useful information for each of these transactions are mentioned in the following subsections.
...
This transaction transfers the number of tokens specified in the `amount` property from the xref:{url_accounts}[account] corresponding to the `senderPublicKey`, i.e. the sender account, to the account specified in the `recipientAddress`.
...
This transaction registers the sender account as a xref:{url_consensus_voting_and_weight}[delegate] with the name given in the `username`. A valid name contains only characters from the set `[a-z0-9!@$&_.]` and has to be at maximum 20 characters in length.
...
This is accomplished by specifying the Lisk xref:{url_accounts_address}[address] of the voted delegate in the `delegateAddress` together with the amount of support given to this delegate in `amount`.
...
The maximum number of votes that can be cast in a single transaction is 20 and `amount` has to be a multiple of 10^9^ (10 LSK in the Lisk Mainnet).
...
This transaction [#index-unlocked-2]#*unlocks*# the tokens specified in `amount` that were previously unvoted for the delegate specified by the `delegateAddress` by a vote transaction at the height given in the property `unvoteHeight`.
This transaction is only valid if it is issued after the unlocking period has been completed since the `unvoteHeight`.
For a regular vote, the unlocking period is 2,000 blocks (around 5 hours).
...
The total number of keys required for every future outgoing transaction from the account is given in the `numberOfSignatures`.
...
This transaction can be utilized to report any misbehavior of a certain delegate.
