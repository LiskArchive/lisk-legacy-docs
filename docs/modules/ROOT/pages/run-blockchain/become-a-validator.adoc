...
:url_references_cli_txsend: {docs_sdk}application-cli.adoc#transactionsend
...
//TODO: clarify if it is safe to import the plain test as well, or if only encrypted part shoud be imported via CLI
.config/keys.json
[source,json]
----
{
...
How to <<enabling-block-generation-for-the-first-time>> is explained in the following step.
...
== Enabling block generation for the first time
...
.. Never use outdated <<validator-data>>.
...
<1> Replace `lskqaxxmj78frvgpjgwvf4yqjjkcrr9yhn2sxxwm3` with your validator address.
