...
:description: This section covers how to write and run a test script, and furthermore test a custom transaction object.
...
When the logic inside a custom transaction object becomes more complex, it becomes complicated to verify that the custom transaction object logic is working as expected.
...
In order to explain how to write unit tests for custom transaction objects, the `applyAsset()` method of the {url_github_hello_world}[Hello World app^] is used here as example.
It can be used as a template to write unit tests for your own custom transaction objects.

NOTE: To test the complete logic of a custom transaction object, it is required to write unit tests for all methods that are implemented in the custom transaction objects: `validateAsset()`, `applyAsset()` and `undoAsset()`.
...
Is writing unit tests really enough to ensure the functionality of a custom transaction object?
