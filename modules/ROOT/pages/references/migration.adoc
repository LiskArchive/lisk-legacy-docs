...
Beside getting a more descriptive name, the chain ID is also constructed differently now:
...
The module developer can now easily define which data should be returned by the endpoint for the particular module by adjusting the newly introduced `metadata` attribute.

.Main benefits of this change
[%collapsible]
====
Improved user experience::
* Users can now query all relevant metadata about a module in a consistent manner.
Improved modularity::
...
The `init()` function of the command can be called inside the `init()` function of the module to pass config options to a command, if desired.
