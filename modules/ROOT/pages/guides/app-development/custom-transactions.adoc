...
The fees for every transaction can be defined dynamically, though there is always a minimum amount that needs to be paid for a transaction.
...
It's recommended to override the values of `MIN_FEE_PER_BYTE`, `NAME_FEE` properties of a custom transaction depending on the use case.
