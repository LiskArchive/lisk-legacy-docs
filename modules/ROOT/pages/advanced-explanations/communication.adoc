...
<<reducers>> of modules can be invoked inside of the xref:{url_intro_modules_lifecycle}[lifecycle hooks] and xref:{url_intro_modules_assets}[Assets] of another module via the `reducerHandler`.
...
Actions are part of the request / response API, and are invoked via .
