...
NOTE: To view the predefined default config values, please check out the <<_list_of_configuration_options,list of configuration options>>

=== Overwrite specific values

.Example of how to override the default config values is shown below:
[source,js]
----
...
TIP: Click here to jump to the https://github.com/LiskHQ/lisk-sdk/blob/development/sdk/src/samples/config_devnet.json[configDevnet file on Github].

.Import the example config from lisk-sdk as shown below:
[source,js]
----
...
<1> Require the lisk-sdk package.
<2> Passes the config json `configDevnet` as a config object for the new application instance.
Start the node with a fully functioning Devnet.
...
.Example of how to override the example config object is shown below:
[source,js]
----
const { Application, genesisBlockDevnet, configDevnet} = require('lisk-sdk'); <1>
...
A complete list of all the available configuration options of the Lisk SDK can be found below:
...
Inside `app.genesisConfig` the specific constants for the blockchain application are set.
...
TIP: To see a full list of all constants and their pre-defined values, please check the following file: https://github.com/LiskHQ/lisk-sdk/blob/development/framework/src/controller/schema/constants_schema.js[framework/src/controller/schema/constants_schema.js].

=== Structure

include::../../ROOT/partials/config-object.adoc[]
...
The genesis block is not forged by an actual delegate like all of the other blocks which come after the genesis block.
Instead it is defined by the blockchain application developer, when creating the `Application` instance of the blockchain app. (see the following section: <<_modify_default_values,Modify default values>>).

TIP: Go to Github, to see the complete file: https://github.com/LiskHQ/lisk-sdk/blob/development/sdk/src/samples/genesis_block_devnet.json[genesis_block_devnet.json]

In the near future, a genesis block generator to create genesis blocks will be included within the Lisk SDK.
For the Lisk SDK, the exposed `genesisBlockDevnet` can be used as a template, and hence customized to your requirements.

It is possible and also recommended to customize the genesis block to suit the use case of your specific blockchain application.
...
<9> Null, as the genesis block has no previous block by definition.
