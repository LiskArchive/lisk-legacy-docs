...
:url_github_lip09: https://github.com/LiskHQ/lips/blob/master/proposals/lip-0009.md
...
For more details, see the https://lisk.io/documentation/lisk-core/3.0.0/management/[Lisk Core management docs] or the descriptions at the respective endpoint shown in this section.
...
* Voters : Voters related API endpoints.
...
If no timestamps are provided, the timestamps from the Lisk epoch to the current date will be used.
...
==== Description
Returns all current constants data on the system, e.g. The Lisk epoch time and version.
...
|Type|Name|Description|Schema
|**Body**|**data** +
__required__|Password for decrypting passphrase of delegate with the corresponding public key.|<<_updateforgingstatus_data,data>>
|===
...
|**400**|Malformed query or parameters.|<<_paramerrorresponse,ParamErrorResponse>>
|**409**|An error related to processing of request.|<<_processingerror,ProcessingError>>
|**429**|Too many requests, exceeded rate limit.|<<_requestlimiterror,RequestLimitError>>
|===
...
|**isDelegate** +
__optional__|The value indicating if the account is a delegate.|boolean
|**keys** +
...
Consists of both the fees and the reward. +
...
The networkId that the node is connecting to, see {url_github_lip09}[LIP-0009^] for more details. +
...
This is dependant on the slot height. +
**Example** : `"500000000"`|string
|**supply** +
...
|**networkId** +
__optional__|The network identifier as per {url_github_lip09}[LIP-0009^]. +
...
|**delegateAddress** +
__required__|Lisk address of the delegate that the queried account unvoted. +
**Example** : `"12668885769632475474L"`|string (address)
|**unvoteHeight** +
...
**Example** : `"1000000000000"`|string
|**delegateAddress** +
__required__|Lisk address of the delegate that the queried account voted for. +
**Example** : `"12668885769632475474L"`|string (address)
|===
