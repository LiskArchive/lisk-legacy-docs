= Consensus algorithm
:description: The consensus algorithm section of the Lisk protocol illustrates how the delegates eligible to create blocks are selected and how they reach agreement on new blocks.
...
An account can vote for any delegate using a xref:{url_transactions_vote}[vote transaction].
...
image::unif_diagrams/forgerList.png[forgerList]
...
. Shuffle the list using `randomSeed2`.
...
For details, please refer to the Lisk-BFT research paper ({url_arxiv}[arXiv]).
...
The signed block headers can be included by anyone as part of a xref:{url_transactions_pom}[proof-of-misbehavior transaction] and submitted to the network.
