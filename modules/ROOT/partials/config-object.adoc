...
<15> Milestone 3.
<16> Milestone 4.
<17> Starts rewards at first block of the second round.
<18> Distance between each milestone.
<19> Minimum loglevel, that should be logged in the log file.
<20> Defines the name and path of the log file.
...
<31> Specifies the minimal log level for database logs.
<32> Relative path of the database log file.
<33> Contains options for the cache component.
<34> Sets the number of databases for Redis to use.
...
<59> Time to wait for the response from the server before timing out.
