...
[TIP]
====
Lisk also supports invoking endpoints from within the sidechain client. For more information, check out the xref:{url_sync_store}[syncing and storing] example for the `HelloInfoPlugin`. Specifically, the following:
...
Any endpoint within the Lisk ecosystem can be invoked via Lisk's CLI as well. For more information, check out the xref:{url_cli}[CLI reference page].
