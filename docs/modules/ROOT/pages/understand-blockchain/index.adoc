...
[#sidechain-client]
Sidechain client::
The sidechain client is the central piece of software that is needed to establish and maintain a blockchain network between many different independent servers.
Servers with running sidechain clients are called *sidechain nodes* in this context.
For example, xref:{url_lisk_core}[] is the blockchain client of the Lisk Mainnet.
The xref:{url_lisk_sdk}[] offers a lot of helpful tools to build a fitting sidechain client for your application.
Check out the guide xref:{url_build_index}[] for more information on the topic.

[#middleware]
Middleware::
A middleware can prepare, aggregate and enrich data from the blockchain network, so it can be easily used and digested later by the application frontend.
Lisk provides a ready-to-use middleware, which can be easily configured for any sidechain application:
xref:{url_lisk_service}[Lisk Service] is an enriched node API that provides a convenient endpoints for sending or getting data from the Lisk blockchain.
...
Although a middleware is *optional*, it will mst probably become essential for bigger blockchain applications in the long run.

[#ui]
User Interface (UI)::
The frontend, or <<frontend-ui-integration,User Interface of sidechain applications>> is highly customizable.
Any UI that is able to communicate over HTTP or WS to a server will work.
Use your preferred exising Frameworks to develop an accessible and intuitive UI for users which fits your individual use case.
...
A sidechan client relies heavily on its state machine to mutate the state of a sidechain.
...
In the context of the Lisk blockchain, a *transition* of states is triggered through *blocks* and their containing *transactions* which are added to the blockchain.

*Modules* facilitate the state changes states in a sidechain client.
Sidechain developers can create their own custom business logic for the sidechain here by creating their own modules or reuse existing ones, and registering them to the client.
...
However, the Genesis configuration and the configuration for Modules must be the same across the network of each a sidechain client.
