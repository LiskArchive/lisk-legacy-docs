...
:description: Lisk Commander command reference lists include accounts, blocks, config, copyright, delegates, hash onion, forging, passphrases, transactions & a usage example.
...
This command displays the network identifier based on the payload hash of the xref:{url_guides_config_genesis_block}[genesis block] of the network.
The network identifier needs to be provided every time a transaction is signed, which ensures that the transaction can only be broadcast in the dedicated network.
