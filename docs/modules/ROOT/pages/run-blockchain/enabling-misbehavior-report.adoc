...
:fn_lip24: footnote:witness[The {url_consensus_bft_punishment}[Punishment of Lisk-BFT protocol violations^] LIP provides the details regarding the implications of {url_rep_mis_transaction}[Report Misbehavior^] transaction.]

The {url_plugin_report_misbehavior}[Report Misbehavior plugin^] provides automatic detection of the validator's misbehavior and sends a `reportMisbehavior` transaction to the running node.
This transaction contains the information necessary to prove that the validator who signed the {url_blocks_header}[block headers^] has violated the {url_consensus_bft}[Lisk-BFT protocol^] and is deemed to be punished{fn_lip24} once proven guilty.
