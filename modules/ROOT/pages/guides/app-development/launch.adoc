...
Beside this, seed nodes require no further special config options.
Especially, it is recommended to leave the public API disabled on a seed node.
...
It is recommended to provide additional application specific documentation for users, for example by updating the README.md with all important steps to spin up a node.
...
As the seed node should have RPC API disabled, add a new node to the network with API enabled.
This node has mainly the purpose of providing blockchain data to external tools and services via the API.
Forging should bbe disabled on this node.

To restrict the node API to specific IPs, see the guide xref:{url_guides_securenonforging}[]

TIP: Using this initial architecture it is convenient to test if the WebSocket connection between the seed node and the new node is established correctly.
...
NOTE: The network will become decentralized for the first time when at least 51% of the delegates are actively forging in the network are independent delegates.
...
. Perform a self-vote.
. Optional: Convince other accounts to vote for your delegate, to increase the delegate vote weight if necessary.
