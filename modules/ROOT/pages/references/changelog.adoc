...
:description: Developer changelog information including the transaction types, dynamic fees & updated genesis block for the devnet, API changes, delegates and nodes.
:toc:
...
:url_config_reference_3x: v3@lisk-sdk::references/config.adoc
...
Once the account is registered as a multisignature account, every outgoing transaction requires a signature for every public key in `mandatoryKeys`.
The array may be empty.
. The `asset` property needs to contain the property `optionalKeys`.
The value of this property is an array of pairwise distinct public keys.
This array must be ordered lexicographically.
Once the account is registered as a multisignature account, every outgoing transaction requires some signatures for some public keys in `optionalKeys` (the number of required signatures depends on the `numberOfSignatures` property and may also be zero).
The array may be empty.
. The sets of mandatory and optional keys, specified by `mandatoryKeys` and `optionalKeys`, must be disjointed;
hence their union must have at least one element and at most 64 elements.
...
Transactions in the Lisk SDK v3.x had multiple properties which can hold these signatures, `signature`, `signSignature` and `signatures`.
...
The registration of a multisignature account is updated in the following manner:
...
{url_github_lip25}[LIP 25: Introduce minimum balance requirement for accounts^] introduces a new constant `minBalance`, which is defining a minimum balance for all accounts in the network.
...
Due to the changes that have been introduced for the structure of transaction objects, the genesis block and its transactions were newly created for Lisk SDK version 3.0.
