...
This guide explains in a step-by-step process how to publish a blockchain application.
...
NOTE: The roles of the nodes are not mutually exclusive, so it is possible a node may fulfill several of the described roles, or alternatively it may fulfill none of them, depending on the configuration of the node.
...
+
Newly connected nodes will use the seed peers in their config as a starting point to discover the rest of the network.
...
It is mandatory to install the {url_github_hello_index}[node application^] on a server in order to run a <<nodes, node>> on a particular blockchain network.
...
The development and publishing of the client application are similar to a regular web application.
...
The following illustration below depicts how the setup looks during development:
...
The plan is to connect a second node with the public API which has forging disabled.
...
[IMPORTANT]
====
The current version of the Lisk SDK does not yet support the creation of its own xref:{url_protocol_genesis_block}[genesis blocks] yet.
...
It is advantageous to have the genesis delegates actively forging on the seed node, in case the network does not yet have enough real delegates who can take the forging spots.
...
To include the data of the new genesis delegates  that were created in the <<genesis_block, genesis block>> in the config, see the xref:{url_guides_forging}[enable forging page]).
...
* {url_github_hello_package}[package.json^]: A project file that lists all necessary dependencies, (this should include `lisk-sdk` as a dependency).
* {url_github_hello_transaction}[transactions^]: A folder containing all required custom transactions.
* `README` : A Readme file that describes the most important steps to setup the node.
...
IMPORTANT: Do not forget to open the corresponding xref:{url_setup_ports}[ports] for both HTTP and WS communication!

By default, the API access of a node is disabled.
So after a successful setup of the node, it is required to xref:{url_guides_config}[whitelist specific clients] in the config or alternatively to make the xref:{url_guides_config}[API access public] for everyone.
...
These new blocks are broadcast again to the new node, and the client app can then display the data based on the API calls that it sends to the new node.

.Log of the seed node with the forging genesis delegates:
image:forging_node.png[Forging node logs]

[NOTE]
.Broadcast irregularities are of no concern.
====
...
During the development of the blockchain application, one node was enabled for forging for all 101 genesis delegates.

After the release of the first version of the blockchain application, it is required that real delegates take the forging slots of the genesis delegates.
...
[NOTE]
====
If a delegate joins the network at a very early stage, it is likely that they will probably replace one of the genesis delegates.
