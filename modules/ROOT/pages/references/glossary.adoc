...
Lisk has a faucet plugin for the Lisk Framework that provides a user interface to receive an certain amount of tokens on request.
...
A validator can be described as a user who is authorized to secure the network. In a DPoS system there are a fixed number of validators that are able to vote on who will validate transactions. The validators who receive the highest number of votes can then become delegates.
