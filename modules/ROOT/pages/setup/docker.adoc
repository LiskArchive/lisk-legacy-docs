...
Docker image adds support for additional platforms upon which to run a Lisk node.
...
include::partial$create-a-new-user-docker.adoc[]
...
To fully verify that your node is synchronized with the network, go to the {url_lisk_explorer_mainnet}[Lisk Explorer (Mainnet)] or {url_lisk_explorer_testnet}[Lisk Explorer (Testnet)] and compare the network height in the explorer with the height of your node.
...
To proceed to the next step, please see xref:{url_admin_docker}[Docker image management] to learn how to manage your node.
...
Note: this command don't start Lisc Core right now, but will enable autostart on reboot.
...
Note: these commands works only if Lisc Core started as a service.
