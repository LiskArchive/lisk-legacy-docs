...
This tutorial will teach you how to create a custom module that adds the feature to create and purchase NFTs in a blockchain application, and a custom plugin that provides additional NFT-related features.
...
|Examples
a|
* Real world: Fiat currencies such as  the US Dollar ($) or Euro (€).
...
First, create a new folder which will contain all the files for the NFT app:
...
These variables are available in the module and are explained in more detail in the upcoming sections <<the-apply-function>> and <<actions>>.
====
...
To create a new NFT, the following information is required:
...
The `validate()` function has access to the following:
...
The `apply()` function has access to the following:
...
* `reducerHandler`: This allows the user to utilize reducer functions of other modules inside the `apply()` function.
...
=== Module ID and name
It is recommended to start with the easiest ones, which are defining the module name and ID.
...
To implement it, we use the function `getAllNFTTokensAsJSON()`, which has been described in the <<nft-related-functions>> section.
...
Now that all on-chain logic for the NFTs is defined in the NFT module, add a corresponding NFT plugin, which will handle the off-chain logic for the NFT app.
...
Copy and paste the snippet shown below into the newly created file `db.js`.
...
Updates the NFT history based on the NFT transaction found in the block.
...
This tutorial is mainly about explaining how to build with the Lisk SDK, therefore other parts of the frontend app are not explained in explicit detail here.
You can find more information about how to build a React application in the {url_react_docs}[official React documentation^].
...
In the first terminal window, start the frontend application with the following command:
...
When starting the app for the first time, the page is relatively empty. However, once we have created the first NFTs, the home page will display all existing NFTs and their details.
...
Next we take a dive into the most important parts of the frontend app, regarding the blockchain related logic.
...
It then uses the `createNFTToken()` function that we defined in the <<create-nft>> section, to create the `createNFT` transaction and the `sendTransactions()` function from the <<api-related-functions>> section, to post the transaction to the blockchain application.
...
It then uses the `purchaseNFTToken()` function that we defined in the <<purchase-nft>> section, to create the `purchaseNFT` transaction and the `sendTransactions()` function from the <<api-related-functions>> section, to post the transaction to the blockchain application.
...
It then uses the `transfer()` function that we defined in the <<transfer-funds>> section, to create the `transfer` transaction and the `sendTransactions()` function from the <<api-related-functions>> section, to post the transaction to the blockchain application.
...
It then uses the `transferNFT()` function that we defined in the <<transfer-nft>> section, to create the `transferNFT` transaction and the `sendTransactions()` function from the <<api-related-functions>> section, to post the transaction to the blockchain application.
...
For the frontend the following components are defined here:
...
====
As you may be aware, the account address can be represented in different formats, such as `bytes`, `Buffer`,`hex` or `Base32`.
...
A list of all existing NFTs, rendered as a <<nft-component>>.
...
That's it! You should now have a complete example NFT blockchain application running, including the following:
