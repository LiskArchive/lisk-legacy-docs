...
To create a valid signature, an sidechain validators with enough weight equal or greater to the <<sidechaincertificatethreshold>> need to individually sign the mainchain registration message, including: ownChainID, `ownChainName`, `mainchainCertificateThreshold` and `mainchainValidators`.
