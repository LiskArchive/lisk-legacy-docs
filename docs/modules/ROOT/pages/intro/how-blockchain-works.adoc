...
DPoS can be classed as being less centralized than other consensus protocols such as PoS and PoW. However, with DPoS effectively a limited number of users retain control of the network. One criticism that is often levied at DPoS is that it sacrifices decentralization for scalability.
...
Furthermore, with this stake-weighted {url_voting-mechanism}[voting mechanism^] DPoS has the advantage of being able to execute transactions and verifications much faster than PoS or PoW, as described above, and also results in higher scalability.
