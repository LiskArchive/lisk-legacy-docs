...
<<events>> and <<endpoints>> are identified by their alias.
...
A blockchain application communicates via <<endpoints>> and <<events>> which can be invoked (actions), or subscribed to (events), via WebSocket.
