...
:description: Part 2 of the Lisk Transport tutorial describes how to implement a simple supplz chain tracking system. It also teaches how to implement a simple trust system and how to lock funds in an account.
:toc: preamble
...
NOTE: It is recommended to have a look at the xref:tutorials/transport.adoc#_general_procedure[diagram on the Introduction page], which will help the user to have an overiew of the workshop.

[TIP]
====
"Are there more of those... solutions?"::
https://github.com/LiskHQ/lisk-sdk-examples/tree/development/transport/transactions/solutions[Yes, there are more!], check out the fully working implementation of the other transactions in the `transactions/solutions/` folder.
...
class RegisterPacketTransaction extends BaseTransaction {
    ...
    applyAsset(store) {
        ...
        /* Check if packet account already has a status assigned.
           If it has, this means the package is already registered so an error is thrown. */
...
Now try to implement the https://github.com/LiskHQ/lisk-sdk-examples/blob/development/transport/transactions/register-packet.js#L160[missing logic] for `undoAsset()` by reverting the steps of the `applyAsset()` function.

**Important: To verify the implementation of `undoAsset()`, compare it with the https://github.com/LiskHQ/lisk-sdk-examples/blob/development/transport/transactions/solutions/register-packet.js[solution].**
...
**Important: To verify the implementation, please compare it with the https://github.com/LiskHQ/lisk-sdk-examples/blob/development/transport/transactions/solutions/start-transport.js[solution].**
...
_Why is it possible to cache two accounts at the same time?_
Please notice that the cache function accepts an array which allows it to pass in multiple query objects.
...
A further in depth explanation in the custom transactions deep dive article can be found on https://lisk.io/blog/tutorial/custom-transactions-statestore-basetransaction-transfer-transaction#6658[our blog].
...
**Important: To verify your implementation of `applyAsset()`, please compare it with the https://github.com/LiskHQ/lisk-sdk-examples/blob/development/transport/transactions/solutions/finish-transport.js[solution].**

== 2.3 Test out the full workflow with the client app

=== Check the status in the `lightAlarmTransaction`

At this point the entire workflow should be implemented with different packet statuses. If the the packet is currently `ongoing`  or `alarm`, then to send an alarm follow the instructions described below:

Insert the code snippet listed below in the `applyAsset()` function of https://github.com/LiskHQ/lisk-sdk-examples/blob/development/transport/transactions/light-alarm.js#L39[light-alarm.js], before the code that applies the changes to the database accounts.
...
Now try to start or re-start the `node`, `client` and `iot` application, exactly as performed earlier in xref:tutorials/transport1.adoc#_1_3_the_client_application[Step 1.3 in Part 1] of this tutorial.
...
Firstly, open the http://localhost:3000/post-register-packet[Register Packet] page and complete the form in order to register your packet in the network.
...
To perform this task, go to the http://localhost:3000/faucet[Faucet page] and enter the carrier address(`6795425954908428407L`), followed by the amount of tokens to be transferred to this account.
...
The carrier is required to post the transaction on the http://localhost:3000/post-start-transport[Start Transport] page, to initiate the transport.
...
This occurs due to the status check added in the <<_check_for_status_in_the_lightalarmtransaction, Check for status in the lightAlarmTransaction>>.
...
TIP: Move on to xref:tutorials/transport3.adoc[section 3: Next steps].
