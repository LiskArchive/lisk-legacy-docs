...
// Footnotes
:fn_weight: footnote:weight[See xref:{url_lip23_weight}[LIP23] for more details about the validator weight calculation.]
...
== Introduction
Any accounts with sufficient tokens can use them to "vote" with their tokens for one or multiple validators.
This voting process is also called *staking* in PoS blockchains.
An account performing a staking is also called *staker*.
...
NOTE: This definition implies that *at least 10% of the validator weight always consist of self-stakes*{fn_weight}.
