...
. xref:lisk-commander/user-guide/commands.html#_create_account[Create an own, private account on the network]
. xref:lisk-commander/user-guide/commands.html#_delegate_registration_transaction[Register a delegate]
...
. xref:{v_core}@lisk-core::configuration.html#_enabledisable_forging[Enable forging for the newly created delegate on the node]
