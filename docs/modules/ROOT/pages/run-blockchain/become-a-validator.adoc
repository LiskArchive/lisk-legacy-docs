...
:url_run_staking_selfstake: run-blockchain/staking.adoc#self-stake
...
:url_run_staking_validatorweight: run-blockchain/staking.adoc#validator-weight
...
To be able to become a validator for a blockchain, the following prerequisites need to be fulfilled:

. Owning an account on the respective blockchain.
. A sufficient balance to send the "Register Vaidator" transaction on the account.
...
It will be required everytime to enable block generation, in order to decrypt the generator keys in the node.
...
To conveniently create and send a transaction, first start the node, if it is not running already.
...
The parameters 2, 3, 4 are all generated in step <<creating-the-validator-keys>>.
...
Copy the transaction in binary format that was created in the previous step and post it to the node.
...
Import the validator keys which were generated in step <<creating-the-validator-keys>> into the node.
...
After the validator keys are imported to the node, it is possible to verify that the import worked as expected with th following command:
...
There are some situation though, where it is needed to get the seed hash onion back, for example when xref:{url_run_block_generation_move}[moving from one node to another].
...
This is particularly relevant for nodes who have been set up recently, because they might not have catched up with the current state of the network, yet.
To verify that you node is fully synched, request the `system_getNodeInfo` endpoint.
...
For each blockchain network, there is a specific *minimum validator weight* that is required for a validator to be considered for the block generation in general.
...
A good way to check what concrete validator weight is required to reach an active position, just check the validator weights of the currently 101 active validators.
For the Lisk Mainnet, this information can be obtained directly from the {url_lisk_wallet}[Lisk Desktop^], or from one of the various Lisk explorers like {url_liskscan}[^] or {url_liskobserver}[^].

NOTE: Please note, that even if you have not enough validator weight to be in one of the active validator positions, there is still a chance each round to be selected as one of the two random validators.
To be considered for the random selection, a validator just needs to have a weight equal or greater to the minimum validator weight of the network.
...
To verify the concrete weight of your or any other validator, it is possible to directly request the required information via RPC endpoint.

. <<check-validator-details>>
. Use the values `totalStakeReceived` and `selfStake` to calculate the validator weight, following the formula:
+
----
