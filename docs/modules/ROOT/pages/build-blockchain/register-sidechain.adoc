...
:url_exmaples_guides_createmessage: https://github.com/LiskHQ/lisk-sdk-examples/blob/292-chain-registration/guides/chain-registration/mainchain/messageCreation.ts
// Project URLs
...
In this example, we decide for the following threshold:
...
----
{
  ...
  "sidechainCertificateThreshold": "66"
}
----
...
The Register Mainchain parameters are created in an analog way as the Register Sidechain parameters hae been created previously.

[TIP]
====
To conveniently prepare the required parameters, check out the following script in the lisk-sdk-examples repository:
{url_exmaples_guides_createmessage}[messageCreation.ts^]
====
...
Now, paste the *complete list* that was returned by the `consensus_getBFTParameters` endpoint under the `"mainchainValidators"` key:
...
The individual signing of the mainchain registration by the sidechain validators is executed off-chain, similar to the process of creating multisignature transactions.
