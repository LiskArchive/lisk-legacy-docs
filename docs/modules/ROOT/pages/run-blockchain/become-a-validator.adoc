...
IMPORTANT: When attempting to migrate to a new or different node, it is crucial that the hash onions utilized during the configuration process correspond to the previous node.
Use the previously stored hash onion to successfully migrate information from the previous node to a newer node.
...
[source,bash]
----
./bin/run endpoint:invoke random_setHashOnion '{"address":"lskz45myuptyyjvzc2z7o5344quubyrv6nofffh8e","seed":"2aa20ce749838a5c6bac8bdf14dcebd8","count":1000000,"distance":1000}'
----
--
=====
