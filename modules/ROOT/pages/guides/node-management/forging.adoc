...
Before it is possible to enable forging on a node, it is required to register as delegate.
This is done by choosing a unique delegate name, and posting a corresponding xref:{url_protocol_tx_registerdelegegate}[Register Delegate] transaction to the network.
...
+
--
If the blockchain application was xref:{url_guides_setup_bootstrap}[bootstrapepd with Lisk Commander], it is possible to generate the relevant forging data conveniently with the command xref:{url_references_cli_forgingenable}[forging:enable].
