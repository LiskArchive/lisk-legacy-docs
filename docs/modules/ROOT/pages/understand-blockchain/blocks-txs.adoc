...
=== Block Rewards
Delegates receive a dynamic reward for generating a block.
...
+
----
101 * 10^8^ Beddows or 101 LSK = 101 * 100000000
----
...
To respect the maximum of 3153600 newly minted LSK per year, and to ensure it will not be profitable for any delegate if any other delegate misses a block, validators who generate blocks for the second time in the round, get the <<minimum-reward-for-active-validators,minimumRewardActiveValidators>> for the second block.
...
|Block generated for the first time on the current chain
|75%
