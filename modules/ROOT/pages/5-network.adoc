...
Connections in the https://github.com/LiskHQ/lips/blob/master/proposals/lip-0004.md[*Lisk P2P]* are established as _websockets_ (see [https://en.wikipedia.org/wiki/WebSocket[Wikipedia page]]) using the [https://socketcluster.io/#!/[_SocketCluster_]] framework. Each node keeps a list of known peers and their IP addresses and initializes up to 20 outgoing connections to randomly selected peers, while accepting up to 100 incoming connections.
...
On top of SocketCluster, lies the _Lisk RPC and Events_ layer, which manages interactions between nodes using Remote Procedural Calls (see [https://en.wikipedia.org/wiki/Remote_procedure_call[Wikipedia page]]).
