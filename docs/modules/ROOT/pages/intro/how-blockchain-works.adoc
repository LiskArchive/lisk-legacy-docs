...
Hashing the previous block and then adding the hash to the block header of the next block ensures immutability, and in the context of blockchain, the hashing function properties are used within the consensus mechanism.
Blockchains are also dependent on hashing, and the well-known SHA-256 (Secure Hash Algorithm), cryptographic hash function is used extensively in blockchain technology, including Bitcoin.
