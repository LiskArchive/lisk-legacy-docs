...
<4> Shutdown stops the application with the given code and message.
<5> NetworkIdentifer is used to sign blocks and transactions.
<6> getSchema returns all schema definitions that are registered to the application.
The schema is a type defined by a codec, which is a Lisk JSON schema.
...
<4> Optional. Apply.
<5> Optional. Additional validation function that can be implemented.
...
If a plugin is already registered with the same alias, it will throw an error.
<2> The package meta information.
<3> The supported configurations for the plugin with default values.
<4> The list of valid events which this plugin wants to register with the controller.
...
<5> The object of valid actions which this plugin wants to register with the controller.
Each action name will be prefixed by a plugin alias, e.g. `pluginName:action1`.
Source plugin can define the action while others can invoke that action.
<6> The method which will be invoked by the controller to load the plugin.
Ensure all loading logic is completed during the life cycle of load.
The controller emits an event `app:ready` which you can use to perform some activities which can be completed when every other plugin is loaded.
<7> The method to be invoked by the controller to perform the cleanup.
