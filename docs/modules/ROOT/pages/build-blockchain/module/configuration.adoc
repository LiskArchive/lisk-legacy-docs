...
As a final step, let's verify that the config options which we defined in `custom_config.json` in step <<define-a-custom-module-configuration>> are really used during the verification of a command.
