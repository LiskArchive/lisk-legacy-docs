...
The validator list for a block generation round is assigning a block generation spot for each validator.
Each active validator and the selected 2 random standby validators gets exactly one spot on the validator list.
The list is not ordered and validators get a new random position on the list for each round.
