...
. We assume the original account lost their credentials.
The owner of the original account uses a new account to rescue the funds in the original account.
. The rescuer initiates a recovery process.
. We assume the owner of the original account informs their friends that they lost their credentials and inform them about the new account address of the rescuer account.
...
In our example, we use the genesis account for the original account, so it is only necessary to send tokens to the rescuer account and to the 3 friend accounts.
