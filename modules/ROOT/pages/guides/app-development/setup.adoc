...
This will create a new blockchain application which uses `genesisBlockDevnet` as genesis block for the blockchain, and `configDevnet`  to configure the application with common default options to run a node in a development network.
...
Now it is possible to start a blockchain application with default configuration, that will connect to a local devnet.
