...
More specifically, within the Lisk documentation, whenever xref:{url_sapp}[sApps] are discussed, this is directly referring to applications that run by using sidechain, which are compatible with Lisk Protocol. 
...
===== Data Stores
Each blockchain node keeps data on various data stores inside its Lisk sidechain\'s instance. This data contains the following two types: on-chain and off-chain.
