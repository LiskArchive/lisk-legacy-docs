...
:url_hash_onion: node-management/forging.adoc#add-forging-data-to-config
...
The smallest unit of account. 100 million Beddows is equal to one <<LSK>>.
...
For further detailed information regarding forking in the Lisk network, see the following xref:{url_fork}[Lisk Mainnet hard fork process] description in the Lisk Mainnet Regenesis page
...
In the Lisk network it can be defined as an integer that accounts for the number of transactions from the sender account.
