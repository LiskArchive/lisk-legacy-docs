...
The event log keeps the blockchain events emitted from the latest and configured number of blocks.
...
A sApp{fn_saap_glossary} developed with Lisk SDK communicates to a node via RPCs. RPC-based interfaces i.e. IPC, WS, allow applications to subscribe to *RPC events*. 
...
With each block's execution, an event is generated and stored in the event log.
For example, modules such as `Reward` emit the `rewardMinted` event after each block's execution.
...
----
Once an event is retrieved from the event log, its `data` property can be decoded by using the `codec.decode()` function by passing the encoded data and the relevant schema to it.
