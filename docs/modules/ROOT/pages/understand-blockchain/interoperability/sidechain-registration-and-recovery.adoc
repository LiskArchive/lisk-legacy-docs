...
The life cycle of a sidechain consists of three states: <<sidechain-registration,registered>>, <<active-sidechains,active>>, and <<terminated-sidechains,terminated>>.
...
After a sidechain has been registered on the mainchain, it does not need to follow the <<liveness-condition>>, until the <<first-cross-chain-update-from-a-sidechain>> has been included on the mainchain.

=== Sidechain registration on the Lisk Mainchain

When a sidechain is registered, this creates new entries in the interoperability related data stores on the mainchain.

* a new <<chain-data-substore,sidechain account>>,
* a new <<channel-substore,sidechain channel>>,
* the <<chain-validators-substore,list of active validators>> on the sidechain.
* an <<outbox-root-substore, outbox>> for cross-chain messages.
* an new entry in the <<registered-names-substore>>.

//This results in a new entry being created in the 5 different substores of the interoperability module:
...
* and the <<chain-validators-substore,list of active validators>> on the Mainchain.
...
IMPORTANT: *The termination of a sidechain is permanent* and cannot be undone.

NOTE: Users can recover their tokens and CCMs from a terminated sidechain.
For more details on this topic, read the <<sidechain-recovery-process>> covered later on this page.
...
If a sidechain does not post a CCU tot he mainchain within 30 days, the sidechain account is terminated on the mainchain.
...
Whenever a sidechain has violated the liveness condition, any user can submit a liveness termination on the mainchain, and this will result in the sidechain being terminated.
...
Each sidechain maintains several data stores to store data related to cross-chain interoperability.

The following sections describe every data store of the interoperability module, explain what data is stored, and why it is stored.
...
The "own chain" data substore stores the name and ID of the respective chain.
...
It is present only on the mainchain.

Entries are created as part of the processing of the sidechain registration command.

== Sidechain recovery process
