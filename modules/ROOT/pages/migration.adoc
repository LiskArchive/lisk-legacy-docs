...
:description: The Lisk Core migration page describes how to migrate the node if a new update introduces a hard fork in the network, together with additional associated information.
:toc:
...
When the block height of the network reaches the predefined height, it will invoke the update on all nodes in the network at the same time.
...
The `<blockheight>` is the block height announced previously when the hard fork is going to occur.
...
Counting from the migration height, there are 2 full forging rounds of time available to upgrade your node manually by following the steps described in <<migrate_manually, Migrate manually>>.
If 2 full forging rounds have already passed since migration, your node will probably be on a fork after the upgrade.
In order to resolve this, rebuild your version of the blockchain xref:{url_snapshots}[from snaphot] or xref:{url_admin_binary_rebuild_from_genesis}[from the genesis block].
