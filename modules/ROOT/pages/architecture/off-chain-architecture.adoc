...
[NOTE]
====
Plugins enable the user to perform the following...
...
<7> Does not load the plugin, if it is disabled in the config.
...
<12> The method to be invoked by the controller to perform the cleanup.
...
* Forger plugin also provides webhook functionality, a webhook could be configured to monitor remote nodes.
