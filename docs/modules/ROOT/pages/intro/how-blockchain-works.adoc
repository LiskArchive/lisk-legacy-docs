...
Generally speaking the lower the hashrate (computing power), the higher the chance of a 51% attack exists. However, all consensus mechanisms are vulnerable to these attacks. This type of attack can corrupt the network as with such a high amount of mining power, they can mine faster than all other miners.
...
Most cryptocurrencies utilize asymmetric encryption. For example, Ethereum uses asymmetric cryptography with the public-private key pair as described further in this section.
...
Blockchains are also dependent on hashing, and the well-known SHA-256 (Secure Hash Algorithm), cryptographic hash function is used extensively in blockchain technology.
