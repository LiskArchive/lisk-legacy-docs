...
Create a new branch `docs-PRODUCT-x.y.z`, on the base of `docs-PRODUCT`, where `PRODUCT` can be `sdk`, `service`, `protocol` or `service`, and `x.y.z` is equal to the now outdated product version.
Like the version number indicates, this branch will store the docs that become outdated with the new release.

The branch `docs-PRODUCT` will be used as base for the new documentation.
