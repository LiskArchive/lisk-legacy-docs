...
* [x] What data is stored on-chain to facilitate interoperbaility across chains
* [x] The recovery process of data for terminated sidechains
====
...
. The <<chain-validators-substore,list of active validators>> on the mainchain.
. An <<outbox-root-substore, outbox>> for cross-chain messages.
...
== Active sidechains
If the mainchain receives the <<first-cross-chain-update>>, or vice versa, the status of a sidechain changes from `registered` to `active`.
...
On a sidechain, the channel substore is initialized as part of the <<first-cross-chain-update>>.
